...
def main():
    ...
    if frontend is not None:r.save(args.out/'frontend_identity.json',dict(parameter_sha256=before,train_encoded_sha256=r.sha_tensor(frontend.encode(data[0])),val_encoded_sha256=r.sha_tensor(frontend.encode(val[0])),feature_parameters=sum(p.numel() for p in frontend.features.parameters()),classifier_head_used_by_optics=False))
    if args.phase=='smoke':
        weights=len(data[1])/(2*torch.bincount(data[1],minlength=2).float());rows=[]
        for v in variants:
            model=g.build(v,cfg);encoded=frontend.encode(data[0][:8]) if frontend is not None else data[0][:8];assert encoded.shape==(8,1,100,100) and 0<=encoded.min() and encoded.max()<=1
            loss,_=g.loss_terms(model,model(encoded),data[1][:8],weights,cfg);loss.backward();grads={n:float(p.grad.norm()) for n,p in model.named_parameters()};assert all(np.isfinite(x) and x>0 for x in grads.values());assert frontend is None or all(p.grad is None for p in frontend.parameters());rows.append(dict(variant=v['id'],gradients=grads));del model;torch.cuda.empty_cache()
    ...
    for seed in args.seeds:
        for v in variants:
            ...
            assert frontend is None or before=={n:r.sha_tensor(p) for n,p in frontend.state_dict().items()}
