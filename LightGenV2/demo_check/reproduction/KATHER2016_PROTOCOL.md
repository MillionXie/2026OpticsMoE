...
训练期间不读取测试用于度量或选模；最终锁定36份权重，重放全部验证预测，然后统一测试。预先设定的跨设备重放要求为每张图argmax完全一致、每个分数最大绝对误差≤2×10⁻⁶、各指标差≤2×10⁻⁶，并保存实际误差；超限即失败，不能静默放宽。实际GPU UUID审计发现本轮设备是4090与A100，先前按nvidia-smi数字编号判断为3090/4090不准确。若跨设备重放失败，保留证据，按原训练GPU UUID精确重放全部验证结果后再测试，不改变权重或选模规则。
