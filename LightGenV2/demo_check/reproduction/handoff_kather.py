...
def main():
    ...
    if a.phase=='train':
        ...
        assert a.checkpoint;ck=torch.load(a.checkpoint,map_location='cpu',weights_only=False);cfg=ck['config'];arch=ck['arch'];model=m.build(arch,ck['depth'],cfg);model.load_state_dict(ck['model'])
