...
def main():
    ...
    seen=set(sets['val']);keep=[]
    for i,img in enumerate(x):
        h=hashlib.sha256(img.tobytes()).hexdigest()
        if h not in seen:keep.append(i);seen.add(h)
    assert len(keep)==11948
    report=dict(command=sys.argv,git_commit=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),data_sha256=hashlib.sha256(a.data.read_bytes()).hexdigest(),source_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),splits=info,train_val_exact_overlap=len(sets['train']&sets['val']),deduplicated_train_n=len(keep),deduplicated_train_support=np.bincount(y[keep],minlength=8).tolist(),training_sample_ids=[f'train_{i}' for i in keep],test_read=False,patient_groups_available=False)
    (a.out/'data_audit.json').write_text(json.dumps(report,indent=2),encoding='utf-8');print(json.dumps({k:v for k,v in report.items() if k!='training_sample_ids'},indent=2))
