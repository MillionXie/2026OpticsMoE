...
原图150×150 RGB，缩放后将四张50×50图按`[R,G;B,(R+G+B)/3]`拼成100×100振幅输入。不是RGB三波长传播，没有CNN、Qwen、电子分类残差。MoE为九路密集光学路由，每个循环包含一个局部专家层和一个全局层；2/4/6层分别对应1/2/3个循环，另有一个路由相位板。OEO为每层传播后测强度、全场LayerNorm、ReLU、Softsign、零相位振幅重编码，无可训练电子权重。

`selected_config.json`是验证集选出的共同训练配置；所有主层和路由相位同时训练。新实验最多60轮，比较延长训练、输入dropout、减轻探测器捕获辅助损失三种方案。未改变旧baseline，未按测试集差距选配置。详细架构、损失及结果见`reports/reproduction/`。
