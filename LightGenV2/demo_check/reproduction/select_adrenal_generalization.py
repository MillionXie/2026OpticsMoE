...
def main():
    p=argparse.ArgumentParser();p.add_argument('--runs',type=Path,nargs='+',required=True);p.add_argument('--out',type=Path,required=True);p.add_argument('--confirmation-runs',type=Path,nargs='*',default=[]);a=p.parse_args();assert not a.out.exists()
