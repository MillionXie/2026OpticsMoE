# Adapted from Hendrycks robustness CIFAR-C generator; see vendor_manifest.json.
# -*- coding: utf-8 -*-
...
def impulse_noise(x, severity=1):
    ...
    x = sk.util.random_noise(np.array(x) / 255., mode='s&p', amount=c, rng=int(np.random.randint(0, 2**31-1)))
...
def frost(x, severity=1):
    ...
    frost = cv2.imread(str(Path(__file__).parent / "frost" / Path(filename).name))
