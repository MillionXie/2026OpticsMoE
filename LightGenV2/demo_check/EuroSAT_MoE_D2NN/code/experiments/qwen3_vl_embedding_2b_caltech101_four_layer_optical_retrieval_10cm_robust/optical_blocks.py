...
class MoE4LanguageTwoBlockOpticalPath(nn.Module):
    ...
    def _encode_input_fields(
        ...
        # Copy all sequence lengths to the host in one operation.  Iterating
        # over CUDA rows and converting each scalar separately forces one
        # device synchronization per sample (240 syncs per batch across the
        # four optical blocks at batch size 60), while producing identical
        # integer lengths.
        lengths = [
            int(value)
            for value in (~padding_mask).sum(dim=1).detach().cpu().tolist()
        ]
