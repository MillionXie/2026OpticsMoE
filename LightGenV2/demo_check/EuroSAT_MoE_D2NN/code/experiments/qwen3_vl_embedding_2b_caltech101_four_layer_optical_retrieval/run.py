...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Caltech101 four-plane Vision+Language joint electronic/optical retrieval"
        )
    )
    ...
    try:
        if args.phase in {"train", "all"}:
            if args.resume_checkpoint is None:
                print(
                    "[initialization] joint random initialization: electronic mixers, "
                    "routers, optical phases/readouts and fusion gates are trainable",
                    flush=True,
                )
            train_optical_retrieval(
                loaded,
