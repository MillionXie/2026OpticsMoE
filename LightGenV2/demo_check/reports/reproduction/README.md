...
2026-09-17进行中：逐层OEO、多种子和输入覆盖对照；新增非MNIST的Kather2016组织图像八分类。训练尚未完成，不能把预实验验证成绩当最终测试结果。[完整技术说明](OEO_METHODS_20260917.md)、[Blood协议](../../reproduction/BLOODMNIST_MULTISEED_PROTOCOL.md)、[Kather协议](../../reproduction/KATHER2016_PROTOCOL.md)。逐像素核查发现原D2NN首层仅约4.5%相位面被直接照明，新增保持总输入功率的全孔径D2NN对照；此前单种子差距只对应原小输入设置，不能直接视为严格公平的架构优势。
