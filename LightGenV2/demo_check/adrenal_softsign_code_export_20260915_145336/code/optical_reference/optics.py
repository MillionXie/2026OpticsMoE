...
class DetectorArray(nn.Module):
    def __init__(
        ...
        det_steps_x=None,
        det_steps_y: int = 150,
        start_pos_x_per_row=None,
        ring_radius: float | None = None,
        ring_start_angle_deg: float = -90.0,
    ):
        super().__init__()
        if isinstance(grid_size, int):
            height = width = grid_size
        ...
        # Optional row-specific x starts allow unequal row lengths (for
        # example 3/3/4 detectors) to remain independently centered. Existing
        # fixed layouts keep using the scalar start_pos_x unchanged.
        self.start_pos_x_per_row = list(start_pos_x_per_row) if start_pos_x_per_row is not None else None
        self.ring_radius = float(ring_radius) if ring_radius is not None else None
        self.ring_start_angle_deg = float(ring_start_angle_deg)
    ...
    def _centers(self):
        ...
        if self.layout == "ring":
            ...
            if radius is None or radius <= 0:
                raise ValueError("ring detector layout requires a positive ring_radius")
            center_y = (height - 1) / 2.0
            center_x = (width - 1) / 2.0
            centers = []
            for index in range(self.num_classes):
                angle = math.radians(self.ring_start_angle_deg + index * 360.0 / self.num_classes)
                centers.append(
                    (
    ...
    def _build_masks(self) -> torch.Tensor:
        ...
        if self.layout == "fixed_2x2":
            ...
            for row_index, detectors_in_row in enumerate(self.n_det_sets):
                ...
                row_start_x = self.start_pos_x
                if self.start_pos_x_per_row:
                    row_start_x = int(self.start_pos_x_per_row[min(row_index, len(self.start_pos_x_per_row) - 1)])
                for col_index in range(int(detectors_in_row)):
                    x0 = row_start_x + col_index * (self.detector_size + int(gap_x))
        ...
        for index, (cy, cx) in enumerate(self._centers()):
            y0 = cy - half
            x0 = cx - half
            y1 = y0 + self.detector_size
            x1 = x0 + self.detector_size
            if y0 < 0 or x0 < 0 or y1 > height or x1 > width:
                raise ValueError(
                    f"Detector region {index} falls outside the grid: y[{y0}:{y1}] x[{x0}:{x1}]"
                )
            region = occupied[y0:y1, x0:x1]
            if bool(region.any()):
                raise ValueError(f"Detector region {index} overlaps another detector")
            masks[index, y0:y1, x0:x1] = 1.0
