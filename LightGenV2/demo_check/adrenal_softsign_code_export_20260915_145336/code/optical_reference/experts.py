"""Optical expert implementations with optional OEO ablation support."""
...
from .activations import activate, CANDIDATES
...
class StageGlobalOEO(nn.Module):
    """Configurable intensity LayerNorm + nonnegative activation + zero-phase re-encoding.
    ...
    def __init__(
        ...
        if str(cfg.get("type", "intensity_layernorm_activation")) != "intensity_layernorm_activation":
            raise ValueError("nonlinearity.type must be intensity_layernorm_activation")
        ...
        normalization_aperture = str(
            normalization.get("aperture", "nonlinear_enabled_expert_regions")
        )
        if normalization_aperture not in {
            "nonlinear_enabled_expert_regions",
            "full_active_panel",
        }:
            raise ValueError(
                "normalization aperture must be nonlinear_enabled_expert_regions or full_active_panel"
            )
        self.activation = str(cfg["activation"]["type"])
        if self.activation not in CANDIDATES:
            raise ValueError("Unknown OEO activation")
        if not bool(reencoding.get("zero_phase", True)):
            raise ValueError("OEO re-encoding must use zero_phase=true")
        if str(reencoding.get("amplitude_source", "activation_output")) != "activation_output":
            raise ValueError("nonlinearity.reencoding.amplitude_source must be activation_output")
    ...
    def forward(self, fields: list[torch.Tensor], enabled: list[bool], capture_fields: bool = False):
        ...
        activated = activate(normalized, self.activation)
        amplitude = activated
...
class D2NNExpert(OpticalExpert):
    ...
    def __init__(self, size: int, cfg: dict, optics_cfg: dict, scalar_cfg: dict | None = None) -> None:
        ...
        self.nonlinear_schedule = [
            bool(value) for value in cfg.get("nonlinear_schedule", [True] * self.num_stages)
        ]
        if len(self.nonlinear_schedule) != self.num_stages:
            raise ValueError(
                "expert_bank.d2nn.nonlinear_schedule must contain one boolean per D2NN layer"
            )
...
class FourierConvolutionBlock(nn.Module):
    """Finite-aperture centred Fourier transform with a local phase mask."""
    ...
    def forward(self, field: torch.Tensor) -> torch.Tensor:
        ...
        # A finite local Fourier aperture is explicit: frequencies outside
...
class HeterogeneousExpertBank(nn.Module):
    """Configured experts advanced synchronously through a shared stage count.

    The default four-D2NN experiment is fully optical between masks and does
    not instantiate OEO modules. The OEO implementation remains available for
    explicit ablation configurations.
    """
    ...
    def __init__(self, layout, bank_cfg: dict, optics_cfg: dict, nonlinearity_cfg: dict) -> None:
        ...
        self.vectorize_homogeneous_d2nn_requested = bool(
            bank_cfg.get("vectorize_homogeneous_d2nn", False)
        )
        self.vectorize_homogeneous_d2nn = (
            self.vectorize_homogeneous_d2nn_requested
            and all(isinstance(expert, D2NNExpert) for expert in experts)
        )
        stage_counts = {expert.num_stages for expert in experts}
        if len(stage_counts) != 1:
            raise ValueError(
                "All configured experts must expose the same number of stages; "
                f"got {sorted(stage_counts)}"
            )
        self.num_stages = next(iter(stage_counts))
        self.nonlinearity_enabled = bool(nonlinearity_cfg.get("enabled", True))
        normalization_cfg = nonlinearity_cfg.get("normalization", {})
        self.panel_wide_nonlinearity = (
            str(normalization_cfg.get("aperture", "nonlinear_enabled_expert_regions"))
            == "full_active_panel"
        )
        if self.nonlinearity_enabled:
            if self.panel_wide_nonlinearity:
                # Revised-D2NN-aligned definition: the nine expert regions
                # belong to one physical phase plane, so LayerNorm is applied
                # once over the entire 498x498 active panel, including gaps.
            ...
            # Do not even allocate affine gamma/beta when OEO is disabled.
            self.stage_nonlinearities = nn.ModuleList()
        self.max_fiber_modes = max((expert.num_modes for expert in experts if expert.expert_type == "fiber"), default=0)

    @property
    def execution_mode(self) -> str:
        if self.vectorize_homogeneous_d2nn:
            return "batched_homogeneous_d2nn"
        return "serial_expert_loop"

    def _forward_homogeneous_d2nn_stage_batched(
        self,
        stage_index: int,
        local_fields: list[torch.Tensor],
    ) -> list[torch.Tensor]:
        """Advance all homogeneous D2NN experts with one batched FFT call.

        The former path launched one phase modulation and one padded FFT/IFFT
        pair for every expert.  Stacking the expert axis produces [B,E,H,W],
        then flattens B and E only for the shared propagator.  Each expert keeps
        its independent phase parameters, and autograd distributes gradients
        back through the stacked phase tensor.
        """
        if not self.vectorize_homogeneous_d2nn:
            raise RuntimeError("Batched D2NN execution is not enabled")
        experts = list(self.experts)
        reference = experts[0]
        fields = torch.stack(
            [value.to(torch.complex64) for value in local_fields], dim=1
        )
        phases = torch.stack(
            [expert.phase_layers[stage_index].get_phase() for expert in experts],
            dim=0,
        ).to(device=fields.device, dtype=torch.float32)
        modulation = torch.exp(1j * phases).to(torch.complex64)
        fields = fields * modulation.unsqueeze(0)

        padding = int(reference.propagation_padding)
        if padding:
            fields = F.pad(fields, (padding,) * 4, mode="constant", value=0.0)
        batch, expert_count, padded_height, padded_width = fields.shape
        flattened = fields.reshape(batch * expert_count, padded_height, padded_width)
        propagated = reference.propagators[stage_index].propagator(flattened)
        propagated = propagated.reshape(batch, expert_count, padded_height, padded_width)
        if padding:
            size = int(reference.size)
            propagated = propagated[
                :, :, padding : padding + size, padding : padding + size
            ]
        return list(propagated.unbind(dim=1))
    ...
    def forward(
        ...
        for stage_index in range(self.num_stages):
            ...
            use_batched_d2nn = self.vectorize_homogeneous_d2nn and not (
                return_details and capture_outputs
            )
            if use_batched_d2nn:
                linear_outputs = self._forward_homogeneous_d2nn_stage_batched(
                    stage_index, locals_
                )
                linear_details = [None] * len(linear_outputs)
            else:
                linear_outputs = []
                linear_details = []
                for expert_index, expert in enumerate(self.experts):
                    ...
                    if local_output.shape != locals_[expert_index].shape or local_output.dtype != torch.complex64:
                        size = self.layout.expert_size
                        raise RuntimeError(f"Expert {expert_index} violated the [B,{size},{size}] complex64 stage interface")
            ...
            if self.nonlinearity_enabled:
                if self.panel_wide_nonlinearity:
                    if not all(enabled):
                        raise RuntimeError(
                            "full_active_panel OEO requires every D2NN expert schedule entry to be enabled"
                        )
                    linear_canvas = torch.zeros_like(field, dtype=torch.complex64)
                    for aperture, local_output in zip(self.layout.expert_apertures, linear_outputs):
                        linear_canvas[:, aperture.y0 : aperture.y1, aperture.x0 : aperture.x1] = local_output
                    active = self.layout.active_aperture
                    active_field = linear_canvas[:, active.y0 : active.y1, active.x0 : active.x1]
                    panel_outputs, oeo = self.stage_nonlinearities[stage_index](
                        [active_field],
                        [True],
                        capture_fields=capture_outputs,
                    )
                    nonlinear_canvas = torch.zeros_like(field, dtype=torch.complex64)
                    nonlinear_canvas[:, active.y0 : active.y1, active.x0 : active.x1] = panel_outputs[0]
                    locals_ = [
                        nonlinear_canvas[:, aperture.y0 : aperture.y1, aperture.x0 : aperture.x1]
                        for aperture in self.layout.expert_apertures
                    ]
                    oeo["application_scope"] = "full_active_panel"
                    oeo["physical_active_shape"] = [self.layout.active_size, self.layout.active_size]
                else:
                    locals_, oeo = self.stage_nonlinearities[stage_index](
                        linear_outputs, enabled, capture_fields=capture_outputs
                    )
                    oeo["application_scope"] = "per_expert_regions"
            else:
                # Purely optical bypass: preserve the complete complex field,
                # including phase, between the expert and global masks.
                locals_ = linear_outputs
                oeo = {
                    "enabled": enabled,
                    "enabled_indices": [],
                    "normalization_mean": None,
            ...
            if between_stage is not None and stage_index < self.num_stages - 1:
                stage_canvas = torch.zeros_like(field, dtype=torch.complex64)
                for aperture, local_output in zip(self.layout.expert_apertures, locals_):
                    stage_canvas[:, aperture.y0 : aperture.y1, aperture.x0 : aperture.x1] = local_output
                stage_canvas = between_stage(stage_index, stage_canvas).to(torch.complex64)
                if stage_canvas.shape != field.shape:
                    raise RuntimeError(
                        f"between_stage must preserve canvas shape {tuple(field.shape)}, "
                        f"got {tuple(stage_canvas.shape)}"
                    )
                locals_ = [
                    stage_canvas[:, aperture.y0 : aperture.y1, aperture.x0 : aperture.x1]
                    for aperture in self.layout.expert_apertures
                ]
    ...
    def parameter_report(self) -> dict:
        ...
        for index, expert in enumerate(self.experts):
            ...
            grid_size = self.layout.expert_grid_size
            value.update({"index": index, "row": index // grid_size, "column": index % grid_size})
    ...
    def nonlinearity_parameter_report(self) -> dict:
        if not self.nonlinearity_enabled:
            return {
                "enabled": False,
                "operation": "none_complex_field_bypass",
                "learned_gain": False,
            ...
            per_stage.append(
                {
                    ...
                    "routing_amplitude_reapplied": False,
                    "activation": "sigmoid",
                    "reencoding": "activation_output_as_zero_phase_amplitude",
