...
| P2 | **T07 商品检索（图搜图）** | ABO similarity10；商品train/val/test=120/40/40；新增2058商品仅训练；480查询×原120商品中心；同类为相关；单seed周期test及超参数选模 | Hit@1/5/10、Precision、positive Recall、mAP@10、NDCG@10↑ | **严格α>0.4、光Router Top2：Hit@1 79.5833%、mAP@10 0.76856490（4090）**；同best去光62.7083% | — | — | Frozen Qwen native2048D **Hit@1 95.2083%**；native64D **94.375%**（20260912复评，0训练参数） | 本轮未测 | 本轮未测 | — | **readout_subspace_20260912/evaluation固定权重复评完成**，TRAIN读出校准/复评bbb9876d；382/480，α0.428–0.440；干净训练99.9306%，81%目标未达。去光下降16.875个百分点；原线性头仅改weight/bias，全部光学相位/光路不变，无TF/attention或新增推理层。只比前版多1个命中，不宣称显著；保留率经TEST缓存选择，披露偏差。[任务与证据](tasks/t07_abo_image_retrieval/README.md) |
...
| P6 | T03 显著性分析 | SALICON train2014 10000；val2014 5000 作 public test；无 validation；单seed选模 | CC/SIM/NSS/AUC-Judd↑；KLD/MAE↓ | **光Router Top2、alpha≥0.4：独立CC 0.86204960、SIM 0.82407、NSS 0.96543**；训练DC20–30%，标准eval关闭随机噪声 | — | 本轮未测；旧模型5090D计算图估算5.654 ms/image，不冒充新版端到端 | 新同规格头Frozen Qwen：**CC 0.88968**；旧头复评0.88103、重新训练0.87899；历史D2NN 0.8346 | 新Qwen头未测；旧头历史mean 10.176 ms/image | 本轮未测；旧光学物理/墙上代理0.317/0.455 J；旧Qwen 117.837 W、1.199 J，不沿用作新头结果 | — | **SAM+空间CC蒸馏系数2完成50轮，best epoch5；同SHA已独立float64复评和完整重载5000张。**alpha=0.430722/0.441067，专家份额23.16–27.10%；现有E内空间DW增6912参数、头85412；相对系数.6的CC/KLD/SIM略改善，NSS/MAE略差。去光CC=.84117204；未达0.87。源码/权重SHA及可视化见[T03复现说明](tasks/t03_saliency/reports/reproduction/SAM_TRAINING.md) |
