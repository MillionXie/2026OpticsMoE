...
|---:|---|---|---|---|---|---|---|---|---|---|---|
| P1 | **T06 视频质量评价** | LGVQ；test 558；时间质量按 16 视频×4帧同负载、A100 原生 batch 16；空间质量为单视频4帧 | SRCC↑；同时报 KRCC/PLCC↑、RMSE/MAE↓ | **16×4 时间：SRCC 0.8044、PLCC 0.8180**；**单视频 Spatial-4：SRCC 0.6393、PLCC 0.6743** | — | A100 分段组合代理：时间质量 **20.382 ms/16视频**；空间质量 **11.558 ms/video**；均含 6 次物理光场和必要串行 CCD 后处理，非实验台端到端实测 | 冻结 Qwen3-VL-2B 五质量词头：时间 batch-16 SRCC **0.7688**、PLCC 0.7794；空间 SRCC **0.6908**、PLCC 0.7065 | A100：时间 batch-16 mean/median/P95 **405.308/397.072/398.773 ms/16视频**，吞吐 39.48 video/s；空间 **74.438/75.321/78.360 ms/video** | 光学 80.388 W 代理：时间 **1.638 J/16视频**、空间 **0.929 J/video**；时间 A100 batch-16 稳态 active mean **238.18 W**、active **96.537 J/16视频**（250 W 上界 101.327 J）；空间 **6.242 J/video** | 16×4 无全局专家坍缩，但帧路由样本变化率仅 8.2%；实测仍需 PCC、SSIM、gain-aligned NMAE、强度比与饱和率 | **A100 batch sweep 与正式复测完成。**batch 8 起达到约 95.6% 功率上限；选 batch 16 与光学负载一致，同负载加速 **19.89×**、active-energy 代理比 **58.92×**；空间任务 6.44×、6.72×；下一步测实验台端到端和整机功率 |
| P2 | **T07 商品检索（图搜图）** | ABO similarity-10；10 类、200 商品×12视角；gallery 120 个 train 商品 centroid；test 480 个未见商品视角；每 query 有 12 个同类相关项 | unrestricted R@1↑；同时报 P@K、positive Recall@K、mAP@10、NDCG@10 | — | — | — | 冻结完整 `Qwen3-VL-Embedding-2B`：**R@1 0.9521、R@5 0.9875、R@10 0.9979、mAP@10 0.8325、NDCG@10 0.8807**；category route 0.9208 | A100、batch 1；mean/median/P95 **49.174/46.157/68.927 ms/query**；第一个 Vision block→2048D→120 gallery+10 prototypes→排序 | A100 idle/active/peak **61.47/82.96/101.11 W**；active **4.079 J/query**，idle-subtracted 1.057 J，250 W 上界 12.294 J | — | **数据合同与 A100 frozen-Qwen baseline 已完成。**480 条 test 全量；2048D、224×224、零可训练参数；下一步在同一合同上迁移光学 MoE，不能与 T08 图搜文混用 |
| P3 | **T08 商品检索（图搜文）** | ABO easy100；100 商品；train 4800/test 2400；100 个官方英文标题为固定候选库；每 5 epoch 看完整 test 选 EMA best | R@1/R@5/R@10、MRR、median rank↓ | 强均衡版 **R@1 0.7983、R@5 0.9538、R@10 0.9858、MRR 0.8676** | — | A100 分段组合代理 **10.175 ms/query**；6 次物理光场 + 必要 CCD 后串行处理；非实验台端到端实测 | Frozen `Qwen3-VL-Embedding-2B`：**R@1 0.7358、R@5 0.9329、R@10 0.9583、MRR 0.8224** | A100、batch 1：首个 Vision block→2048D 归一化→100 标题排序；mean/median/P95 **43.963/42.099/57.060 ms/query** | 光学 80.388 W 代理 **0.818 J/query**；A100 active mean/peak **86.26/103.70 W**，实测 **3.792 J/query**，250 W 上界 10.991 J | — | **A100 正式复测完成。**光学组合时间加速 4.32×、active-energy 代理比 4.64×；光模型固定 224×224、64D，Qwen 主干冻结；硬件一致性尚未测 |
...
| P5 | T02 关键点检测 | LSP；固定 test 1000；单 seed；周期 test 选模 | PCK@0.2、PCKh@0.5↑；MPE/NME↓ | **DC20 光 Router Top-2：PCK 0.5773、PCKh 0.7363、NME 0.3488** | — | A100 分段组合代理 **6.259 ms/image**；3 次物理光场 + 必要 CCD 后串行处理；非实验台端到端实测 | 冻结完整 Qwen Vision + DeconvPoseHead：PCK **0.7226**、PCKh **0.8849** | A100、50 warm-up + 200 次：mean/median/P95 **18.016/17.010/25.418 ms/image** | 光学 80.388 W 代理 **0.503 J/image**；A100 active mean/peak **80.62/87.83 W**，实测 **1.452 J/image**，250 W 上界 4.504 J | — | **A100 正式复测完成。**1000 张 test 全量评估；Qwen 原生 Vision blocks 全部执行，只有 1,102,990 参数反卷积读出头可训练；组合时间加速 2.88×、能效代理 2.89× |
| P6 | T03 显著性分析 | SALICON train2014 10000；val2014 5000 作 public test；无 validation | CC/SIM/NSS/AUC-Judd↑；KLD/MAE↓ | **DC20 光 Router Top-2：CC 0.8291、SIM 0.8063、NSS 0.9283、AUC-Judd 0.7631** | — | A100 分段组合代理 **6.309 ms/image**；3 次物理光场 + 必要 CCD 后串行处理；非实验台端到端实测 | Frozen Qwen+头：CC **0.8810**、SIM 0.8327、NSS 0.9903、AUC-Judd 0.7730 | A100、50 warm-up + 200 次：mean/median/P95 **19.141/19.753/25.646 ms/image** | 光学 80.388 W 代理 **0.507 J/image**；A100 active mean/peak **81.05/93.30 W**，实测 **1.551 J/image**，250 W 上界 4.785 J | — | **A100 正式复测完成。**5000 张 public test 全量评估；组合时间加速 3.03×、能效代理 3.06×；实验台端到端未测 |
...
- T07 ABO similarity-10 图搜图 A100 frozen-Qwen baseline：
  `tasks/t07_abo_image_retrieval/reports/a100_frozen_qwen_20260907/README.md`
- A100 审计后正式总表、时间 batch sweep、核心时延、板卡功率与能耗（T06/T08/T07/T02/T03；不含 OpenMoji）：
  `tasks/t06_video_quality_assessment/reports/a100_audited_table_20260907/README.md`
- 旧 A100 batch-1 归档（保留用于口径追溯，不再作为时间一致性主表）：
  `tasks/t06_video_quality_assessment/reports/a100_cross_task_baselines_20260907/README.md`
