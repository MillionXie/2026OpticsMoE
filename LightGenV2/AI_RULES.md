...
25. 每项任务的 `reports/reproduction/README.md` 是唯一复现入口。每个报告数值须说明模型/训练过哪些部分、
    数据划分及标签处理、指标/选模口径、完整命令、依赖环境、源码 commit、权重和数据清单 SHA256。
    区分“固定权重复评”“重新训练复现”和“外部论文数值”；未实际复现的项目明确标待核验。
    原始日志/逐样本指标保留在对应 run，报告仅引用它们。优化不得覆盖原有 baseline 和历史结果。
