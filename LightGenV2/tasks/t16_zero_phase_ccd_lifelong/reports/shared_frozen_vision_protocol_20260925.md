...
本文件记录验证集候选和已经按验证选模后仅测试一次的 MoE A/B/C/D 阶段。原无视觉前端协议的数字不可拼入本协议。
...
正式 MoE 下三角的十格均已核验，均为选中 checkpoint 的一次测试集宏平均召回。三张顺序表统一以**行＝刚学完的阶段、列＝接受测试的任务**；“—”表示该任务尚未学到或该阶段尚未完成，绝不是 0%：
