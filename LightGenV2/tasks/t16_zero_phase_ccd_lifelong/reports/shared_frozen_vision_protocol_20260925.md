...
C 阶段从获选 B checkpoint 继续，开放第 9–12 槽并保留全部已见任务训练记录。其 `balanced_cycle` 调度先将每个任务的全训练集走完一遍，再循环短任务，使各任务每轮 batch 数相近；所以是**全量加重复**，不是固定 512 样本记忆。C/D 的测试须先按完整验证集选 checkpoint，尚未填表。D2NN full replay B 是额外对照；正式 D2NN 无 replay 下三角使用独立的 `--replay-mode none` 入口。
