...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        resume_value = _nested(raw, "abo_image_text.resume_checkpoint", None)
        if resume_value:
            resume_path = _resolve_from_config(config, str(resume_value))
            expected = str(_nested(raw, "abo_image_text.resume_checkpoint_sha256", ""))
