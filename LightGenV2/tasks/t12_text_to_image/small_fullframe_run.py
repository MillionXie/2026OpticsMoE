...
def main()->int:
    ...
    args=parser.parse_args();result=train_small_editor(task=args.task,data_dir=args.data_dir.resolve(),instruction_cache=args.instruction_cache.resolve(),output_dir=args.output_dir.resolve(),device=torch.device(args.device),epochs=args.epochs,batch_size=args.batch_size,learning_rate=args.learning_rate,seed=args.seed);print(json.dumps(result,indent=2));return 0
