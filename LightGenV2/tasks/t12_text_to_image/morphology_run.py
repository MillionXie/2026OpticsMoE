"""CLI for full-frame text-guided product morphology generation."""
...
from .morphology_data import ProductMorphologyDataset, build_morphology_instruction_cache
...
def main() -> int:
    ...
    for name in (
        ...
        "instruction-cache", "adapter-checkpoint", "warm-start-checkpoint", "output-dir", "config",
    ):
    ...
    if args.command == "cache-instructions":
        result = build_morphology_instruction_cache(
            args.output.resolve(), args.data_dir.resolve(), args.qwen_checkpoint.resolve(),
            torch.device(args.device), keep_layers=args.keep_layers, force=args.force,
        )
        ...
        result = cache_redesign_latents(
            ...
            image_size=args.image_size, device=torch.device(args.device), batch_size=args.batch_size,
            num_workers=args.num_workers, dataset_class=ProductMorphologyDataset,
            task_name="full-frame product morphology generation",
        )
    else:
        ...
        result = train_redesign_model(
            ...
            model_config=model_config, training_config=training_config, device=torch.device(args.device),
            seed=args.seed, dataset_class=ProductMorphologyDataset,
            task_name="full-frame text-guided product morphology generation",
        )
