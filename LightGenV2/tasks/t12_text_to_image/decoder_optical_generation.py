...
class DecoderOpticalGenerator(nn.Module):
    def __init__(self, config: DecoderOpticalConfig) -> None:
        ...
        self.to_rgb=nn.Sequential(nn.Conv2d(w[0],w[0],3,padding=1),nn.SiLU(),nn.Conv2d(w[0],3,3,padding=1))
    ...
    def forward_with_aux(self,reference:torch.Tensor,text:torch.Tensor)->tuple[torch.Tensor,dict[str,torch.Tensor]]:
        ...
        if self.config.task=="style":
            ...
            delta=torch.zeros_like(raw); mask=self.foreground_mask(reference).to(raw.dtype); output=torch.tanh(raw)
        return output,{"encoded":encoded,"decoder_generated":value,"delta":delta,"mask":mask}
