"""One-call text-to-image inference with Qwen and the compact Turbo student."""
...
def generate(
    *,
    ...
    unet = UNet2DConditionModel.from_pretrained(
        compact_unet, variant="fp16", torch_dtype=torch.float16, local_files_only=True
    ).to(device).eval().requires_grad_(False)
    ...
    report = {
        ...
        "inference_flow": "text -> frozen Qwen -> adapter -> Gaussian latent + one compact UNet call -> one VAE decode",
        "unet_calls": 1,
        "vae_calls": 1,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate with the compact one-step Turbo student")
