"""Text-to-image inference for the V0 parallel optical Turbo model."""
...
def _to_pil(value: torch.Tensor) -> Image.Image:
    array = ((value.float().clamp(-1, 1) + 1) * 127.5).byte().permute(1, 2, 0).cpu().numpy()
    return Image.fromarray(array)


def _load_optical_unet(checkpoint: Path, device: torch.device):
    payload = torch.load(checkpoint, map_location="cpu", weights_only=False)
    from diffusers import UNet2DConditionModel

    unet = UNet2DConditionModel.from_pretrained(
        Path(payload["compact_unet"]), variant="fp16", torch_dtype=torch.float16,
        local_files_only=True,
    )
    wrapper = attach_parallel_optical_mid(unet, OpticalTurboConfig(**payload["config"]))
    wrapper.load_optical_state_dict(payload["optical_state"])
    return unet.to(device).eval().requires_grad_(False), wrapper, payload
...
def generate(
    *,
    ...
    unet, wrapper, payload = _load_optical_unet(optical_checkpoint, device)
    ...
    for seed in seeds:
        noise = torch.cat([
            torch.randn(
                (1, 4, 64, 64), generator=torch.Generator(device=device).manual_seed(seed + index),
    ...
    report = {
        ...
        "inference_flow": "Qwen condition + Gaussian seed -> one hybrid optical UNet call -> one VAE decode",
        "unet_calls": 1,
        "vae_calls": 1,
        "iterative_loop": False,
        "optical_backend": payload["architecture"]["optical_backend"],
        "fusion_alpha": float(wrapper.fusion.alpha.detach()),
        "images": records,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate with the parallel optical Turbo model")
    parser.add_argument("--adapter-checkpoint", type=Path, required=True)
    parser.add_argument("--optical-checkpoint", type=Path, required=True)
    ...
    report = generate(
        ...
        optical_checkpoint=args.optical_checkpoint.expanduser().resolve(),
...
__all__ = ["_load_optical_unet", "generate"]
