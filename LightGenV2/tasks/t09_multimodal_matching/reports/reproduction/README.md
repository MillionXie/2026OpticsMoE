...
当前交接默认配置：图文采用CNN+phase dropout=0.05的已测版本；音文采用无CNN、intensity Softsign OEO，并已补做左右排布。OEO流程是有效面非负光强除以空间均值，经过u/(1+u)，再单位功率振幅归一化和相位清零；它不直接作用于复振幅。输入端模态分别归一到0.5功率，总输入功率为1，层间OEO归一化与此独立。最新左右排布结果和专家统计见任务README的“当前统一协议与保留版本”。

追加OEO对照：原平方光强+LayerNorm+ReLU+Softsign曾导致上下布局MoE零读出；centered_leaky_relu保留该结构但将ReLU负斜率设为0.1，左右音文测试MoE84.54%、D2NN84.49%。intensity Softsign的lambda=0.20均衡路由测试MoE83.62%、D2NN73.24%，前三专家接近均匀但第四路仍约1%；均衡项未纳入默认配置。
