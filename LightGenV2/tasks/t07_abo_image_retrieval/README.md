...
> **已启动，结果待训练完成**：`abo200_enrolled_fresh_20260913`（40×100，GPU0 RTX4090，PID909360）；`shape8_multiview_20260913`（20×100，GPU1 RTX4090，PID908314）。两者训练源码固定5f731fae，均先通过完整CUDA冒烟（含正常/去光）；冒烟PID901381/901382已退出。不是拿冒烟2步的数值当正式成绩。
> 新代码本地/服务器286测试通过。当前仅上述两张训练卡；Qwen基准已完成释放GPU3。每个run有status/execution/history，正常结束或异常退出释放CUDA；不得终止其他人的GPU4任务。原始run不删除，只存best/last。
