...
第二次联合课程`domain_joint_restart_20260912_gpu4`：从当前固定79.375%而不是旧78.75%开始，
沿用第35节joint配方、原cap250二视角池、教师坐标/辅助头、seed42，重新初始化优化器和16轮余弦调度。
不使用四视角池、局部视觉教师或新读出。它是增加训练预算的warm restart，不是独立重训或单因素性能证明。
源码f695014b；等待GPU4的seed123组正常退出并检查空闲后才运行；完整命令第40节。
光路、推理结构和480/120协议不变，尚无新结果。
