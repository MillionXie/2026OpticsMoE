...
> 新增只读训练诊断：每轮重用已有干净TRAIN bank，排除同一张照片，记录全图库Hit@1及最佳正/负余弦间隔；这是**轮首当前live**，不是轮末TEST。每轮12相位RMS变化也写入history；不新增模型前向、不改loss/推理/选择规则。本地334测试通过，正式启动前须同步源码并验证服务器。下一项计划为外部spin12轮→目标20轮，原轻量结构，命令第72节；数据尚未ready时不能启动。
