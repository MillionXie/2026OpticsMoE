...
> 本次只比78.75%多3个命中，不宣称显著性；单seed、周期test选模、跨run挑选。完整16轮联合课程仍在训练。
...
先只对训练图补充教师缓存：含原1440图共可复用旧缓存5013图，预计新增4371次前向，实际以缓存报告为准。
...
队列`domain_teacher_first_views4_20260912_gpu1`（监督2589495）等待joint课程结束后接续，不额外占GPU。
...
现已接续启动，监督2514340/训练2542439；第一轮live/EMA均78.125%，尚未超过固定78.75%起点。
实时进度以该run的status.json/history.json为准，不把启动PID当作仍在运行的保证。
