...
> **持续目标（2026-09-13）**：用户要求持续优化至上述目标、进一步争取81%，保持光路/Top2/alpha>0.4和专家不过度集中。强SAM目标对照已完成30轮，最终选回epoch0=78.125%，不是训练提升；PID1665320已退出。外部组12+30轮训练完成、最终评估中，训练候选最好77.0%（最后live，TRAIN83.8125%），不宣称大池成功。训练clean下降到80%上下、TEST无提升，提示强正则/增强导致判别能力损失；不能把gap变小本身当成功。
...
> 新源码`158cf6f295f3f61cbad765fa50a50b81bf6b239c`已推GitHub，本地/服务器各300项测试通过。弱SAM完整CUDA冒烟完成（两步、有限梯度，78.25%仅比起点多1张，非正式提升），PID1951330已退出。正式A=`abo200_mild_adamw_20260913`已在物理GPU0 RTX4090启动，PID1986961；正式B待外部组退出后复用GPU1，不用已被其他任务占用的GPU3。工作树`.worktrees/t07_mild_pair_20260913`固定源码，最多两卡。
