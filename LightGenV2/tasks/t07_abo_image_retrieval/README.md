...
光路、推理结构和480/120协议不变；新配置本地158项测试通过，尚无新性能结果。
...
逐位置视觉教师`standalone.vision_teacher`（已接入独立profile，未获得正式训练结果）：只在独立缓存生成进程中
...
正式队列`domain_vision_patch_20260912_gpu2`（监督2542040）已接受，等待GPU2的256维读出组完成；
等待时不占CUDA，不重复启动。尚无本组正式性能结果，不能用接口检查替代性能验收。
该依赖已完成；完整5556图视觉缓存生成成功，教师进程2550104已退出释放CUDA，学生2551456已接续。
缓存/源模型/processor身份与实际尺寸见该run的`build_vision_teacher_cache/artifacts/final_report.json`，
不再使用4图smoke缓存。新学生尚未有可验收的完整结果。
