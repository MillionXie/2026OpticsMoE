...
最新中间观测：扩大池组第16轮EMA为76.25%（366/480，RTX3090），尚未完成最终同权重复评；
仍未达到389/480目标，不用该中间值覆盖正式交付依据。
...
教师缓存子进程2226915已在GPU1实际启动（不是仍在等待），读取5556张训练图；
缓存和蒸馏训练尚未完成，不能宣称已带来提升。下方75.2083%仍是三组后续实验的共同起点。
