...
准备训练方法`domain_distill_joint_merger`：仅解冻原有V→L接口最后的4096→2048线性层
...
本地166项测试通过，包括默认merge逐值不变、解冻范围、混合精度初始等价、FP32微小更新/重载及光学张量不变。
服务器真实权重/输入和初始指标仍需验证；尚未宣称正式训练已启动或性能提升，命令第43节。
...
新配置尚无可验收性能结果；正在运行状态以实时PID/status为准。
