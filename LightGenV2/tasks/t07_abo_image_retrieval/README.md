...
> **新一轮方案（2026-09-13，启动状态另行记录）**：`sku_regularized`保持原光路/推理参数量，从当前78.125%权重做对照：A仅目标集30轮；B先在额外1,986 SKU/7,944图上做12轮同SKU预训练、再目标集30轮。B多12轮计算，非等计算量对照。预训练不是复用旧任务训练权重，而是当前协议权重继续做外部实例学习；冻结前端不解冻。操作见COMMAND第66节。
