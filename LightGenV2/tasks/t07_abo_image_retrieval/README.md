...
> **持续目标（2026-09-13）**：用户要求持续优化至上述目标，保持光路/Top2/alpha>0.4和专家不过度集中。强SAM目标对照已完成30轮，最终选回epoch0=78.125%，不是训练提升；PID1665320已退出。外部组当前目标22/30轮，已评估最好76.875%，不提前宣称大池成功。训练clean下降到80%上下、TEST无提升，提示强正则/增强导致判别能力损失；不能把gap变小本身当成功。
> 下一组`sku_mild_adamw`/`sku_mild_sam`从同一78.125%权重各20轮，除SAM rho=0/.002外完全一致：lr-scale=.1，电子decay=.01，无额外phase dropout，无缩放/位移/模糊，亮度/对比度仅.95..1.05；既有硬件噪声作用于10%训练batch、router噪声仍关闭，均衡辅助损失为上轮的1/4。光学结构与推理参数不变，见COMMAND第67节。启动状态后续记录。
