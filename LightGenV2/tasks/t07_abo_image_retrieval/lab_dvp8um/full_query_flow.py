...
STAGE_CALIBRATION = {
    'vision_router': ('none_normal', 'flip_v'),
    'vision_expert': ('h_inverse', 'rot270'),
    'vision_global': ('none_normal', 'rot270'),
    'language_router': ('h_inverse', 'flip_v'),
    'language_expert': ('hv_inverse', 'rot270'),
    'language_global': ('hv_inverse', 'rot270'),
}
...
def main():
    ...
    rows = [row for row in protocol['rows'] if row['split'] == 'query']
    if len(rows) != 800:
        raise ValueError(f'Expected 800 queries, got {len(rows)}')
    ...
    write(out/'run_contract.json', {
        ...
        'scope': '800 physical queries against fixed 1600 simulated gallery',
    })
    ...
    predictions, metrics = evaluate(descriptors, rows, protocol, bank)
    ...
    report = {
        ...
        'query_count': len(rows), 'gallery_count': 1600,
        ...
        'scope': 'All 800 physical query descriptors against the fixed 1600-image simulated gallery.',
    }
