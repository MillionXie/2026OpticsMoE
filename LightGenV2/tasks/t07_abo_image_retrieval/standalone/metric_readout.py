...
def run(args):
    ...
    verification = args.source_run / 'verification'
    ...
    identity = dict(source_commit=source_commit(), command=sys.argv, pid=os.getpid(),
        ...
        extra_inference_parameters=0, inference='Same linear64 layer, Wnew=A@W, bnew=A@b',
    ...
    def save(name, step):
        candidate = fold_metric(payload, matrix.detach())
    ...
    try:
        for step in range(args.steps + 1):
            if step:
                ...
                loss = metric_loss(matrix, train_z, train_labels, idx, args.anchor)
                loss.backward()
                torch.nn.utils.clip_grad_norm_([matrix], 1., error_if_nonfinite=True)
                optimizer.step()
            if step % args.eval_every == 0 or step == args.steps:
                with torch.no_grad():
                    vectors = F.normalize(z @ matrix.T, dim=-1)
                ...
                entry = dict(step=step, cached_test=metrics, train_hit_at_1=train_hit,
                    matrix_delta_frobenius=float((matrix.detach()-torch.eye(64)).norm()),
        ...
        removal, _ = rank_instances(F.normalize(caches['remove_optical']['vectors'].float() @ best_matrix.T, dim=-1), rows)
