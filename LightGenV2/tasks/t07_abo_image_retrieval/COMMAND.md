...
## 54. 仅训练时随机缩减近纯白留边（已提交，不重复启动）

源码1dab4190已同步GitHub，两端228项测试及真实训练224图/梯度检查通过；监督437698已提交GPU5 RTX3090。
...
## 55. 训练时仅投影冲突的教师梯度（运行中，不重复启动）

源码8be14e72已同步GitHub，两端240项测试及真实8图CPU反向检查通过；监督454819/学生454823已在空闲GPU4 RTX4090启动。
初始完整复评79.375%；前8轮约95%–99%批次触发投影，日志证明开关生效，但暂未超过当前最佳。
