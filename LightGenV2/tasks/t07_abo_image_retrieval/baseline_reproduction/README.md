...
| Qwen native2048 Hit@1 | 457/480 = **95.2083%** | 685/800 = **85.625%** |
| 均匀随机选Top1的命中概率 | 12/120 = 10% | 8/1600 = 0.5% |
...
本次独立脚本已经在RTX4090从原图重跑旧native：64维94.375%、2048维95.2083%，与历史一致。
新版也从原图重跑：64维85.125%与历史一致；补充2048维85.625%，同类别诊断99.125%。
因此本次新协议增加维度只提高0.5个百分点，不能单靠64维截断解释新旧协议的全部差异。
执行源码 `91b2d02d18fe2f9d9bce33ac6978b0eaa778750b`，不是只重算旧缓存。
两次使用同一空闲GPU0 RTX4090顺序执行，PID3428835/3438359已结束后才交付；
交付版仅新增ZIP源码身份校验、文档和打包核验，不改变上述推理/计分逻辑。
...
本次两协议已从图像复跑，原始证据位于 `evidence/independent/`。
回归388项通过（原任务377+本目录11）；不把单次可复现结果当多seed统计显著性。
