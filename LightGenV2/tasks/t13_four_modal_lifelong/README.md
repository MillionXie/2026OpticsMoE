...
纯 CCD 窗口分类或全光系统。训练时另用固定 CCD 能量区给光学相位提供辅助梯度；它不参与
最终预测。光学阶段结束后冻结传播层，在完整训练集上重新拟合电子 MLP，并只用验证集选择
MLP 轮次。测试输出始终来自这个 MLP，辅助窗口精度单独记录，不能冒充最终精度。
...
|Physical Concepts 视频文|现有 5784 / 1056 / 1352 个匹配对|2|CC BY 4.0；1024 个 quadruplet 初步包，组间互斥；前端显式编码相邻帧差分|
