...
> **已废弃，不得作为最终实验引用。** 这张表使用了多层电子头、训练后校准以及对 Speech / Physical
> 最终标签监督训练的冻结前端；后两项也已接近饱和。当前正式合同改为单个线性读出、Speech
> 8 候选和 Physical 10 候选，见同目录 `CURRENT_SINGLE_TASK_PROTOCOL_20260922.md`。
