...
2026-09-17用户指定[第65轮实测适配记录及恢复权重](HARDWARE_EPOCH65.md)：200条选模留出集修改格0.8575、整场景0.725。已按原100轮日程重放至65轮并重载核验，全部1000条分区/分组指标与该轮历史误差0；选定PT为`head_replay_epoch65_20260917/last_checkpoint.pt`，不是自动best。源码`7fdd931e`。原PT未保存，不声称权重逐位一致；原自动best及全部历史不变。

2026-09-16追加：末端读出头已从100轮续训至200轮（恢复AdamW，保持1e-6，不加热）。best仍为epoch50，留出集修改格/整场景0.8775/0.745；epoch200为0.8600/0.730，未改善。源码`440b1a8f`，任务退出码0，best重载1000条指标误差0。完整记录见[200轮逐轮结果](../../runs/hardware/head_adaptation_800_200_e200_20260916/00_逐轮结果.md)。800/200划分不变，200条参与选模；不能称为独立最终测试。原100轮结果及CCD均保留。

2026-09-15新增 [SHS单电脑实测指引](../../COMMAND_SHS.md)：固定87.15%主版本，语言三层在前、视觉三层在后，
在师弟电脑联控两块SLM和高速相机。完整1000条六层实测已完成：修改格0.8830、整场景0.6050；不与下方历史98%混用。

2026-09-16离线去光与末端适配已完成，详见[任务协议与命令](../../README.md)和
[全部逐轮结果](../../runs/hardware/head_adaptation_800_200_20260916/00_逐轮结果.md)。
源码`de8084b1`；固定原模型`routerfill_shared`epoch40，checkpoint SHA在任务README与run/config中记录。
同权重去光（两个模态光支路关闭，电子系数恢复为1，无重训）修改格0.4065、整场景0.2730。
实测1000条四操作分层随机800/200，seed20260916，无重新采集；只训练343316个下游电子读出参数100epoch。
epoch50按200条留出scene-exact优先选best：修改格0.8775、整场景0.7450、IoU0.8358333333、F1 0.9522550505。
该200条用于选模，不是独立最终测试；全1000条混合800条适配样本，更不能列为独立test成绩。
本地run包含原始逐样本去光结果、split、所有轮次、config/source manifest、best/last、SHA256传输清单。
冻结前端/光学状态未改，缓存epoch0精确复现；best重载1000条指标误差0。任务退出码0，不再占用硬件。
...
新增实际结果：[2026-09-08三组全量复评、去光、相位及电子冗余检查](RESULTS_20260908.md)。
该报告明确区分本次测量与下文历史值；禁TF/CCD非线性合规问题尚未通过新模型修复。
