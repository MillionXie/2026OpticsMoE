...
if __name__ == '__main__':
    ...
    parser.add_argument('--shs-offline-output', type=Path, help='Additive offline head-adaptation ZIP, no hardware runtime overwrite')
    args = parser.parse_args()
    if args.shs_offline_output:
        from LightGenV2.tasks.t04_semantic_interaction.shs_package import build_offline_update
        result=build_offline_update(args.shs_offline_output)
    elif args.shs_control_update_from:
        ...
        if not all((args.run_dir,args.data_dir,args.output_dir)):parser.error('run-dir/data-dir/output-dir required')
        result=build(args.run_dir,args.data_dir,args.output_dir)
