...
相位通过独立子进程中的SDK常驻线程、保留RGBA缓冲、消息泵与周期重写，不用GUI，不改VCom/固件/时序电压。
独立进程隔离两家SLM SDK的全局图形状态，不代表物理显示必然正确，仍须相机对照。
换相位等待期间持续读取并丢弃相机过渡帧，不能只sleep后丢固定几帧。
新振幅连接预热10秒；当前相位换层等待10秒（每层开销，不是每幅输入都等待10秒）。
...
GPU环境已验证Python3.12、Torch2.8.0+cu126、RTX4060；相位SDK需要pywin32（本机312）。
...
师弟4060已完成固定权重1000条test仿真复评：修改格准确率0.8700、整场景0.6880；
原报告分别0.8715、0.6900，跨设备实测值不得写成原报告值。前4条六CCD边界回放的最大logit误差均为0。
证据在部署目录`release.json`；本地统一查看入口为任务`runs/hardware/shs_single_pc_20260915/00_查看这里.md`。
硬件仍需通过光场重复性检查后才启动正式数据集。探测成功/SDK返回不等于已完成六层实测。
