...
## 20260915：显式加入部分原test的部署适配（独立协议）

用户授权把一部分原test加入训练。新参数 `adapt.py train --test-adapt-fraction 0.2` 固定seed=20260914，
从原558条test随机取112条，与2250条原train组成2362条训练数据；剩余446条仅逐epoch选模、不反传。
完整558条会另报为 `original_test_all_mixed`（混合训练/留出，不能称独立test），112条适配与446条留出分别报告。
默认fraction=0，原协议不变；原0.620022版本保留，并在完全相同446条上重算作为对照，不能跨分母比较。
...
四组各100轮已完成，SRCC最佳第32轮EMA=0.6200222298（PLCC=0.6415857913），尚未达到0.64；上一轮SRCC=0.6172733980保留。
所有学习率/batch/权重约束/排序对照统一放在 `runs/hardware/spatial_train2250_20260914/readout_tuning064/`，最佳权重位于其 `rank_stratified/original_train2250_test558/`。
