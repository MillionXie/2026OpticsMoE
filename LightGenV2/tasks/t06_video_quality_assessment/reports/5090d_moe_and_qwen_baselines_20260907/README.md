...
时间一致性的冻结 Qwen 对照固定采用原始方案二证据：单视频 4 帧均值 65.433 ms，16 个
视频顺序执行为 **1046.928 ms**。因此，只比较既往六层光学硬件总时长时为
`1046.928 / 9.084 = 115.25×`；与当前完整光学 MoE 计算图临界路径比较则为
`1046.928 / 28.744 = 36.42×`。前一个数值不含 router、CCD 后处理、场重建和任务头，
只能标为“光学六层硬件时间比”；后一个才是本报告的完整计算图时间比。后续独占 GPU
复测得到的 985.691 ms 属于另一测量轮次，不再替换这条已经用于老师表格的固定证据。
