...
相对上一正式版本 s523，s586 的 SRCC 观测增量为 `+0.001132`。对同一 558 个
测试视频做 2,000 次成对 bootstrap，增量的 95% 区间为
`[-0.00150, +0.00411]`，因此它应表述为“当前观测最优”，不应表述为统计显著
提升。相比之下，同一 s586 checkpoint 开启光学相对绕过光学的增量为
`+0.117298` SRCC。
...
Transformer block、VGG、RGB 旁路、相位量化、k 空间滤波及像素扰动。按照前序
消融的正式选择，训练与评测均固定加入 20% 未调制直流功率分量。
