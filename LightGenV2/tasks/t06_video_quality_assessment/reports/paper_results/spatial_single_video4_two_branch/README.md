...
视觉 router 的四专家选择占比为 22.67%、23.48%、27.64%、26.21%，没有专家坍缩。这里纠正旧解释：language 阶段接收的是四个随视频变化的图像摘要 token 与固定 prompt 的拼接序列，因此固定选择同一对专家应如实记为 language router 集中，不能只归因于 prompt 固定。
