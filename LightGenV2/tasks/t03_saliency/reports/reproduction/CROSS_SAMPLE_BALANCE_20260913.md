...
本地逐行核对两份已独立复评CSV的5000个sample_id及顺序完全一致：本run的
`candidate_recheck/per_image_cc.csv`与`qwen_aligned_head_50_20260912_seed42/independent_test_recheck_original_batch/per_image_cc.csv`。
光模型减Qwen的平均CC差为-.0123457711891145；光模型2211张更高，
345张落后超过.1，69张领先超过.1。只用于已选择权重的描述性差距，不是独立盲测显著性证明。
不要把“光模型在若干图上胜出”解释成已追平。历史100轮同头Qwen的.88968469仍保留，
且当前光模型蒸馏教师来自该100轮模型；50轮基线不是相同总训练计算预算的严格控制。
