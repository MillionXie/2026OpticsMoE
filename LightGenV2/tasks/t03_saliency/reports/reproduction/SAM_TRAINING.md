...
配对已以源码`fbe09b5c915c1f3b3b0832416215c3a0a49e970e`通过236项CPU测试（51.68秒，13条既有警告）
并推送GitHub后启动：PID/PGID484872，GPU3 UUID `GPU-4d8bfdb9-8777-05a6-3811-ab18ff4eadfd`，
固定工作树`.worktrees/t03_baseline50_20260912`，run为`moe_alpha40_asam010_20260913_seed42`。
最初打算使用已释放的GPU0，但启动前检测到别人的新任务，检查直接中止且没有创建run；
改选当时空闲GPU3，不结束别人进程。本任务共用GPU1和GPU3，不额外使用第三张卡。
后续发现GPU3也出现另一任务的分配，需监控显存/吞吐；不把共享时延作为独占GPU测速数据。

半径.5组第一轮CC=.86179219，尚未超过正式.86204960。
其`midrun_state_audit.json`核验该轮EMA SHA `4a9120e9fcf003de1d017338e67dd1fa806ab65fc51fcf0a4063f413368756208`：
alpha=.430992/.441208，同头85412、六张相位479364、全部tensor键/shape及既定几何合同不变。
状态审计不代替完整测试、路由分布或真实运行图检查。

### 大半径组提前停止

半径.5组第1/5/10轮CC=.86179219/.86063447/.85986539，持续退化。
已于2026-09-13停止PID/PGID476516，保留best第1轮、last第10轮；不是完成50轮。
仅处理核验过UID/PGID/工作树/完整命令的本组父进程及5个子进程，不删除任何权重或数据。
具体停止与权重完整性记录在run内`manual_stop_report.json`和`stopped_checkpoint_integrity.json`。
其第1轮EMA的64张train随机抽查：四专家份额21.88%/27.34%/18.75%/32.03%，
共出现5种Top2组合，最多一种占37.5%，未有专家完全闲置；这是小样本诊断而非全测试集平衡结论。

半径.1组第1/5轮CC=.86107074/.86130864，第10/15轮降至.8608744351/.8606780324，
同期训练CC上升，未改善泛化。已于UTC 2026-09-12 18:03（北京时间09-13 02:03）停止。
保留best第5轮、last第15轮；不是完成50轮，不替换正式候选，不删除文件。
核验UID、命令、cwd和PGID后仅终止484872组；父进程及五个子进程均确认退出，GPU3无本任务分配。
CPU重载两个checkpoint，90/180个张量中的浮点张量均有限，详情在本run的`manual_stop_report.json`。
best SHA256 `b720c90c3f641fcb555c36886242e2cb9c4b25f4c590e32fc761fd7b480de503`；
last SHA256 `52a5e3051486b41856baa7201d3e2a4b0db45d5d2b4a7273248ea45101d5bb6b`。
`midrun_state_audit.json`对best核验alpha=.43085524/.44116246、同规格头85412、光参数479364；
相对正式87ad的物理相位RMS差：router .00085088 rad，四专家/全局约.01439至.01523 rad。
张量键/形状和配置合同不变；此状态审计不替代独立全测试或真实硬件验证。
...
后续原batch8在第5/10轮CC=.86172517/约.8614连续回落，已停止，保留best第1轮、last第11轮；
父进程561115和五个子进程均退出、GPU3释放，两个checkpoint CPU重载有限值检查通过。
不是完成20轮，不删除首轮独立复评证据。下一项仅训练均衡估计的配对及真实更新审计见
[跨样本均衡估计](CROSS_SAMPLE_BALANCE_20260913.md)，不增加推理参数或解冻前端。
