...
最终源码`d55eaf17c8d20770e0ece9df976110b1e5896cce`，已推送GitHub分支
`experiment/salicon-alternating-20260914`；289项CPU回归通过（51.83秒，13条既有警告）。
真实8图三阶段单步检查通过：只训光时795147个电子参数逐位不变；只训电时479364个光参数
逐位不变；联合时两侧更新，所有原有冻结参数不变；24个原生TF钩子调用0次，头85412参数。
原始坐标单步更新RMS：第一阶段phase .00383552/router .00066208；第二阶段光两组为0，
电子/CCD/头/FFN分别约1e-5/2e-5/2e-5/5e-5。不是物理相位弧度或新准确率。
预检源码b6145a9e，d55eaf17只在其上增加阶段结束绘图及其回归检查。
首个预检发现DataLoader构造后不能改persistent_workers，已修正为构造前设置num_workers=0；
该失败未进入正式训练、未修改已有权重，保留诊断日志。

UTC 2026-09-13 17:41:30（北京时间2026-09-14 01:41:30）启动PID/PGID2330417，
GPU0 UUID `GPU-afc19890-6209-ee4d-622d-e619da5bd5b2`，只用1张卡。
启动前GPU0为12MiB、无计算进程；未停止GPU1/3的其他作业。
运行worktree `.worktrees/t03_baseline50_20260912`固定d55eaf17，不在运行时checkout。
配置SHA256 `20e33f2127606c00aeb5fb1b7caf93e843eac6e31869237066d4e089aa000bf5d`。
当前为新启动30轮预算，尚无新完整测试成绩；不得宣称已有.87或使用旧去光指标代表本组。
命令/PID/源码记录在本run的`launch_record.json`，过程日志`console.log`。
