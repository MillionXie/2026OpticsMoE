...
因此没有再占GPU3、也没有停止它。启动当时仅GPU0新组与GPU1 MGD25两张卡；
MGD25现已停止，正式训练仅GPU0，详见下节。

2026-09-10后续检查：该进程正常完成第7/40轮；第1/5轮完整5000张测试CC分别
`.8620417608261108`/`.8618063554763794`，尚未超过原初始化约`.86204960`。
不宣称已经改善；该状态不是40轮完成结果。保持当前运行源码不变，未重复启动训练。
...
## 遮挡比例单变量对照（已提前停止，未取得收益）

25%组第10轮完整测试CC=.8615615395545959，与50%同轮.8615770603179932几乎一致，
两组均未超过初始化.86204969。因此不再延长这条分支，停止25%组并保留第10轮last；
不是完成40轮，也不证明所有掩蔽蒸馏均无效。停止前后读取确认权重可加载且SHA一致：

- best epoch0 SHA `bf93ba1f31b00b5dc3fe33b70f81cfd6459ddd583eedfa76f7d016c5ac1cf529`。
- last epoch10 SHA `97c23c0d97633ad775f4c700e5a77a8ad0ee21fb1c25aa5ee24b596741c4c5ab`。

核对UID1011、完整命令、cwd和PGID后，仅停止自有
1931819/1937956/1937960/1938219/1940637/1940959；ps确认全部退出，GPU1没有本任务残留。
run、教师缓存与best/last均未删除。随后GPU1仅用于有界的增强教师一致性诊断，不再运行MGD。
...
25%对照第5/10轮也未提高，现已提前停止；结果与清理记录见上一节。

联合阶段检查：50%组第10轮完整5000张测试CC=.8615770603179932，继续低于初始化；
25%组首轮CC=.8620111406326294，当时在辅助detach预热，不能据首轮判断联合训练优劣。
...
完整训练集或最终路由审计。之后25%配对试验也未取得完整测试收益，已按上节记录停止。
