...
启动状态见[特征预训练说明](reports/reproduction/FEATURE_HINTS.md)。没有新性能结论。
...
GPU1现启动不增加参数的router直接弧度坐标对照，来源87ad，保持初始物理相位、光路和融合不变；
186项测试及真实图像SAM检查通过，训练源码`dabb9983`，当前没有新完成成绩。
