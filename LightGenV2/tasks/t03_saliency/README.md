...
GPU1/PID1483592继续60轮稳定路由预训练（源码`b5b4dd26`）；第25轮完整测试CC=.84001736，
尚未超过原正式best。GPU0/PID1691668现运行40轮`moe_alpha40_relational_kd_seed42`（源码`aa0dc202`）。
新方案保留87ad原core和读出头，仅蒸馏14×14位置间的特征关系，不添加投影/attention/推理参数，
不换头；启动时完整5000张CC=.86204969，缓存SHA已验证。这只是起点复核，不是新训练的提升。
