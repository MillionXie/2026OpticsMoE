...
当前完成并独立核验的较高CC光电候选：`moe_alpha40_sam_spatialcc_seed42`，50轮完成，
best为epoch5 EMA；重载best完整5000张测试CC=0.86172946，独立float64 CC=0.86172948。
alpha=0.430728/0.441080，四专家选择23.29%/26.47%/23.14%/27.10%，无明显坍缩。
仅在原电子残差内部增加6912个空间DW参数，读出头仍85412参数；SAM及空间CC蒸馏不增加推理参数。
相对已完成SAM.05的0.86133204，CC略高但KLD/SIM略差，不宣称所有指标全面提高。
...
KLD/SIM略差于SAM.05，作为当前已完成的较高CC候选记录。SHA、逐图改善和边界见同一SAM文档。
进一步只将空间CC教师系数.6改为2的epoch5候选独立复评达0.86204960，仍在训练；
