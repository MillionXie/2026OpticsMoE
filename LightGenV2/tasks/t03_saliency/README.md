...
其结构、初始化及参数预算也集中在同一文档，未替换当前已验证best。
不改推理网络的[电子子空间SAM对照](reports/reproduction/SAM_TRAINING.md)从上述最佳权重出发，
与相同预算的普通续训比较，只改变训练更新方式；目标未达成前不替换正式结果。
其中rho=.05的epoch5 EMA候选已独立复查全部5000张，float64 CC=0.86133204，
相对来源2947/5000张改善；训练尚未结束，不把该候选标为最终交付。
三档半径.01/.05/.10和普通续训继续对照，未达到0.87；完整SHA和逐图证据见上述SAM文档。
