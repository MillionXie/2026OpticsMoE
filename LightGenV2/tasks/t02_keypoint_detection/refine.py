...
def stage_spec(profile: str, epoch: int) -> tuple[str, dict[str, float]]:
    ...
    if profile in ('alpha40_polish','alpha40_distill'):
        ...
        if epoch <= boundary:
            ...
            scale = floor + (1-floor)*.5*(1+math.cos(math.pi*(epoch-1)/(boundary-1)))
            return 'alpha40_small_step_joint', {k:float(v)*scale for k,v in spec['joint_rates'].items()}
        return 'alpha40_readout_polish', {k:float(v) for k,v in spec['readout_rates'].items()}
    if profile == 'alpha40':
        if epoch <= 3:
...
def run(args):
    ...
    settings.student_epochs=profile_epochs(args.profile)
    ...
    if polish:
        for key,value in model.core.state_dict().items():
            if not torch.equal(value.detach().cpu(),payload['core'][key].cpu()):
                raise RuntimeError(f'Polish initialization changed source tensor {key}')
    ...
    try:
        ...
        write('final_report.json',{'best_epoch':best_epoch,'source_test':anchor,'test':normal,'optical_off':off,
                                  ...
                                  'target_met':(normal['pck_at_0.2_torso'] >= .73 and not args.smoke) if args.profile.startswith('alpha40') else None})
    finally:
