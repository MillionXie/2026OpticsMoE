"""Render every trainable T02 phase plane from a canonical checkpoint."""
...
def render(checkpoint: Path, output_dir: Path) -> dict[str, Any]:
    ...
    if not values:
        raise RuntimeError("No phase parameter was found in the T02 checkpoint")
    columns = min(3, len(values))
    rows = math.ceil(len(values) / columns)
    figure, axes = plt.subplots(rows, columns, figsize=(4.0 * columns, 3.5 * rows), squeeze=False, constrained_layout=True)
    statistics: list[dict[str, Any]] = []
    image = None
    for axis, (name, phase) in zip(axes.ravel(), values):
        circular_mean = torch.angle(torch.exp(1j * phase).mean())
        residual = torch.remainder(phase - circular_mean + math.pi, 2.0 * math.pi) - math.pi
    ...
    figure.suptitle(f"LSP selected phase masks (epoch {payload.get('epoch')})")
    ...
    report = {
        ...
        "weight_variant": payload.get("weight_variant"),
