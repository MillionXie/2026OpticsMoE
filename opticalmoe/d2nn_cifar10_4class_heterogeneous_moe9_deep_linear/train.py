...
def parse_args():
    parser = argparse.ArgumentParser(description="Train CIFAR-10 deep heterogeneous nine-expert linear optical MoE")
...
def architecture_report(model, config, class_names, train_loader, test_loader):
    ...
    return {
        "model": "DeepHeterogeneousOpticalMoEClassifier",
        "task": "CIFAR-10 four-class deep heterogeneous linear optical MoE",
...
def main():
    ...
    model = DeepHeterogeneousOpticalMoEClassifier(config, len(class_names)).to(device)
