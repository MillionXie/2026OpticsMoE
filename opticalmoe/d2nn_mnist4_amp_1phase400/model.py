...
class D2NNClassifier(nn.Module):
    def __init__(self, config: dict, num_classes: int = 10):
        ...
        self.detector = DetectorArray(
            ...
            start_pos_x=int(det_cfg.get("start_pos_x", 75)),
            start_pos_y=int(det_cfg.get("start_pos_y", 75)),
            n_det_sets=det_cfg.get("N_det_sets", det_cfg.get("n_det_sets", [2, 2])),
            det_steps_x=det_cfg.get("det_steps_x", [150, 150]),
            det_steps_y=int(det_cfg.get("det_steps_y", 150)),
        )
