...
def load_yaml(path):
    ...
    if base is None:
        return value
    return _deep_merge(load_yaml(path.parent / base), value)
...
def choose_device(name):
    return torch.device("cuda" if name in {None, "auto"} and torch.cuda.is_available() else "cpu" if name in {None, "auto"} else name)
