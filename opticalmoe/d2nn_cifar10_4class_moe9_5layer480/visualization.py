...
def _save_map(value,path,title,cmap="inferno",label="normalized intensity",vmin=None,vmax=None,normalize=True):
    fig,ax=plt.subplots(figsize=(5.3,4.6),constrained_layout=True)
    im=ax.imshow(_image(value,normalize=normalize),cmap=cmap,vmin=vmin,vmax=vmax,origin="upper")
...
def save_prompt_routing(amplitude,weights,selected,path):
    ...
    axes[0].set_xlabel("x pixel");axes[0].set_ylabel("y pixel");axes[0].set_title("Input-dependent prompt amplitude")
    cb=fig.colorbar(im,ax=axes[0],fraction=.046,pad=.04);cb.set_label("complex prompt amplitude")
...
def save_epoch_artifacts(model,batch,run_dir,epoch_name,class_names,enabled=True):
    ...
    _save_map(items["at_prompt"][0],sample/"01_at_prompt.png","After 5 cm propagation to prompt")
    save_prompt_routing(items["prompt_amplitude"][0],items["routing_weights"][0],items["routing_selected_mask"][0],sample/"02_prompt_amplitude_and_routing_weights.png")
    _save_map(items["prompt_phase"][0],sample/"03_prompt_phase.png","Prompt phase: no random phase, active 450, outer zero pad to 480","twilight","phase (rad)",normalize=False,vmin=0,vmax=2*math.pi)
    _save_map(items["expert_entrance"][0],sample/"04_expert_entrance.png","Expert entrance after prompt routing")
    ...
    _save_map(items["at_global_fc"][0],sample/"10_at_global_fc.png","After 5 cm propagation to global FC")
