...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a standalone MNIST 256-to-400 D2NN baseline.")
...
def architecture_report(model, config):
...
def main():
    ...
    phase_dropout = phase_dropout_settings(config)
    save_json(architecture_report(model, config), run_dir / "architecture_report.json")
    ...
    matrix = confusion_matrix(final_eval["preds"], final_eval["targets"], num_classes=10)
    ...
    save_json({**final_metrics, "architecture": architecture_report(model, config)}, run_dir / "summary.json")
