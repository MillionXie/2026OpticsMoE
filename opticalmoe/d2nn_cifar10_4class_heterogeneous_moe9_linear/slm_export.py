...
def export_best_slm_package(model, loader, checkpoint_path, output_dir, config, device, class_names):
    ...
    for expert_index, expert in enumerate(model.expert_bank.experts):
        ...
        if expert.expert_type == "d2nn":
            ...
            filename = output / f"fourier_expert_{expert_index:02d}_frequency_domain_phase.bmp"
            exported = export_plane_bmp(expert.fourier_phase().cpu(), filename, "phase", scale, width, height)
            files["fourier_frequency_domain_masks"].append(exported)
            entry.update(
                {
                    "deployment": "phase-only mask located in the expert Fourier plane; not a D2NN spatial mask",
                    "parameter_files": [exported],
                    "fft_normalization": "ortho",
                    "shift_convention": "ifftshift -> fft2 -> fftshift; inverse uses ifftshift -> ifft2 -> fftshift",
                }
            )
            raw_parameters[f"expert_{expert_index:02d}_fourier_phase"] = expert.fourier_phase().cpu()
        else:
            ...
            entry.update(
                {
                    "deployment": "coherent Gaussian fiber-mode array parameters; no D2NN phase-mask representation",
                    "parameter_files": [str(parameter_file)],
                }
            )
    ...
    metadata = {
        "format": "heterogeneous_optical_moe9_type_aware_export_v1",
        ...
        "warning": "Fourier and Fiber experts are not encoded as ordinary D2NN phase masks.",
