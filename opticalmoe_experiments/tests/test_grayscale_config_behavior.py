...
def test_grayscale_flag_false_still_outputs_single_channel_for_mnist_like_input():
    ...
    out_true = build_image_transform(134, grayscale=True)(image)
    out_false = build_image_transform(134, grayscale=False)(image)
    assert tuple(out_true.shape) == (1, 134, 134)
    assert tuple(out_false.shape) == (1, 134, 134)


def test_cifar_rgb_with_grayscale_false_currently_outputs_single_channel():
    image = Image.new("RGB", (32, 32), color=(255, 0, 0))
    out = build_image_transform(134, grayscale=False)(image)
    assert tuple(out.shape) == (1, 134, 134)
