...
本文件对应 `control_v3` 协议。最终结果表在训练、回传及一致性审计完成后填写；当前内容说明已经实现的结构与比较方式。
...
直接专家 LR=0.02，Router LR=0.01，global LR=0.006，decoder LR=0.0003；电子/读出与编码器 LoRA 的峰值 LR 由下述开发实验决定。AdamW 按参数组优化，电子/读出 weight decay=0.001，其他组为零。每阶段重启 20 步 warmup 和余弦调度，最低为峰值的 0.2；每组梯度范数裁剪阈值为 1。电子图像增强在 optical/joint 阶段启用，experts 阶段禁用。
...
Imagenette 最终 train / validation / gallery / test 为 9,119 / 300 / 50 / 3,925；CIFAR 十类为 4,750 / 200 / 50 / 1,000。
...
待正式训练、SHA256 回传和审计完成后，在此补充结果、光学干预及开发结论。
