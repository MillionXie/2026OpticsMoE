...
24组正式适配及两组smoke已完成，正式训练共120个epoch；训练源码固定 `0d854376cca29e16143ad1eb7762860770e59459`。实际全部在GPU5 RTX3090（`GPU-d53ce4c8-272d-c2fb-dc09-f182d586c4eb`）完成，未使用A100。任务A原等待GPU2，因持续被其他任务占用，在B全部结束后改用其释放的GPU5；改配前A尚未启动任何训练，保留独立分配审计。

下表是三次支持抽样的平均 **Top-1 / Top-3（%）**；完整标准差、未适配基线、Qwen只换参考图诊断另列报告。

| 新十类任务 | 每类支持图 | 直接相位适配 | Qwen视觉空间适配 |
|---|---:|---:|---:|
| A | 5 | 33.10 / 61.37 | 31.20 / 62.70 |
| A | 20 | 42.00 / 71.50 | 35.00 / 68.03 |
| B | 5 | 31.57 / 58.17 | 30.53 / 58.70 |
| B | 20 | 38.43 / 66.43 | 34.83 / 64.90 |

对应自身源mask不更新基线，直接组的平均Top-1增量为 **+2.73/+6.30/+3.03/+6.20 pp**，Qwen为 **+1.03/−0.30/+0.23/+1.27 pp**。直接组在这四项设置的平均Top-1和适配增量更高；Qwen在5-shot的Top-3略高，不能说所有指标都落后。Qwen只换参考图、不反传的平均Top-1变化为−0.07/0.00/+0.07/−0.20 pp，未显示稳定的免反传适配收益。该生成器源训练只见过一个固定参考任务，尚未进行多任务条件训练。

每方法全部60轮的平均训练循环/含保存审计epoch时间为direct **7.70/7.81秒**、Qwen **16.99/17.45秒**；每轮均20个batch。Qwen的B/5-shot/seed101运行遇到外部PID，其整组耗时保留标记，不能当严格独占GPU速度比。所有任务梯度、冻结参数和导出检查通过，原始逐查询预测与权重哈希已回传校验。

阅读 [实验设计与解释](reports/unseen_v1_20260909/实验设计与解释.md)、[完整成绩与标准差](reports/unseen_v1_20260909/完整结果.md)、[配对增量](reports/unseen_v1_20260909/paired_adaptation_gains.csv)、[120轮时间](reports/unseen_v1_20260909/epoch_times.csv) 和 [对照图](reports/unseen_v1_20260909/unseen_accuracy.png)。复现操作集中在 [唯一入口](reports/reproduction/README.md)。

仅比较直接优化expert和Qwen视觉空间生成expert。源模型复用第五轮CIFAR原十类的validation所选best（训练SHA `c024b928`），不使用训练过CIFAR全百类的模型。源十类为 `[3,13,14,17,28,31,35,81,86,94]`；从其余90类按固定seed20260909打乱后，锁定新任务A `[22,25,50,52,60,66,68,72,77,80]` 和B `[5,18,20,21,26,32,34,55,85,91]`，两任务也互不重叠。

每类5/20张支持样本，只取官方train；支持集同时用于梯度训练和类别原型gallery，不额外提供gallery/validation标签。抽样seed为101/202/303，同seed的5-shot包含于20-shot。每任务全部1,000张官方test图作为查询，不参与梯度、参考图选择或选模。两种方法分别测试原mask直接迁移和仅expert适配；Qwen另测只换为每类一张新支持参考图、保持源offset且不反传的生成诊断。后者使用有标签支持集建立任务条件，不称为无样本零样本学习。

适配固定5轮×20个PK batch（10类×3图），无数据增强；仅直接expert或视觉LoRA+expert decoder更新，router/global/电子/readout/0.6融合全部冻结。学习率沿用第五轮expert0.02、decoder3e-4、LoRA1e-4，10步warmup及最低0.15的余弦系数。只报告预先固定末轮，不按新类别test挑轮次。共24个正式适配run，另先做两组1轮×2步、仅官方train中不重叠查询的smoke。

源训练仍为raw0→物理π初始化；这轮是保留已学相位的迁移续训，不将续训起点宣称为零。Qwen改用固定新支持参考图后，以 `G当前(新参考)-G适配开始(新参考)+源expert` 继续优化，初始mask与本方法的源mask一致，避免换条件引起相位跳变。不同方法各自继承其源模型，源电子权重和相位并非跨方法相同；适配增量需与各自不更新基线配对，不能把全部差异归因于生成器。

“未见”限定为未进入这两个源模型的CIFAR任务训练；预训练Qwen及Caltech warmstart可能已有相关概念，历史其他实验也曾评估CIFAR全百类，因此不是未知预训练类别或全新盲测。源码入口 `run_unseen.py`，协议 `configs/unseen_v1/cifar.yaml`；每个新任务内两方法用同一张RTX顺序运行，禁用A100。逐样本预测、支持/查询清单、梯度、相位变化、冻结审计、CUDA同步每轮时间及带时间戳GPU PID审计均保存在独立run。
...
各卷积级使用GroupNorm/GELU，避免深层空间解码器的梯度衰减；这一结构在所有视觉生成组中保持一致。
...
固定训练预算仍为4/8/18轮三阶段，每轮120个PK batch（30张图），单seed42；同一数据集的五组绑定同一张空闲RTX顺序运行，
数据集可在不同空闲RTX上并行。服务器部分4090已被其他任务占用，因此调度允许空闲3090，数据集内的五组仍保持相同型号和物理卡，不混算跨数据集速度。
禁止A100，不与正在占卡的任务抢占同一GPU。每轮CUDA同步后记录训练/验证/保存的分项时间。
