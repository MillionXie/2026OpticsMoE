...
第四轮当前状态：六组确定性开发实验已完成，三数据集的 14 个正式 run 正在执行。
正式训练统一使用 RTX 4090、固定四路光学系数 0.6，比较直接相位优化、Qwen 和 CLIP；新增 Imagenette。
结构、训练阶段和组别说明集中在 [训练与结构说明](reports/control_v3_20260907/训练与结构说明.md)，
开发选择和数值复现证据见该目录。当前正式源码固定 `dee34f07`，本地后续报告编辑不会改动运行中的 worktree。
下述已完成指标及 0.5 起点描述属于第三轮历史；第四轮协议和开发结论见文末，最终结果完成后单独归档。
