...
## 2026-09-13 新鲜配对实测已完成

两组都重新采集同一组预先固定的40张held-out数字（0/1/2/3各10张），没有按结果删图、
挑样本或调整探测框；每组另有1张重复帧，仅用于重复性检查。数据在
`results/mnist_pair_wait400_20260913/`，逐张预测与指标见`comparison.json`。

|口径|A 旧mask重采样|B 原生8μm训练|
|---|---:|---:|
|完整4157张test仿真|86.5528%|88.7659%|
|本次固定40张仿真|95.0%（38/40）|97.5%（39/40）|
|本次固定40张实拍|60.0%（24/40）|92.5%（37/40）|
|同输入首末重复实拍PCC|0.995940|0.995541|
|40张实拍与对应仿真平均PCC|0.349509|0.447160|

共同条件：150μs、Gain_X4、相机100fps、振幅Visible后持续取帧丢弃400ms并额外丢6帧；
同一ROI/homography、同一振幅BMP、相位RGBA发送路径。每张CCD仅一次几何变换，
四探测区原始强度积分后argmax；无电子分类头、逐图归一化或背景扣除。
配置独立保存为`results/smoke_configs/smoke_mnist_pair_wait400_20260913.json`，未覆盖旧会话。
运行代码：`mnist_pair_hardware.py`（commit `d00836b8`包含本次配置接口）。

**本次原生8μm方案实测明显更好，但92.5%仅是40张小样本结果，不是完整测试集准确率。**
不同训练历史/参数量、固定A后B的采集顺序均限制因果解释；重复一张不能证明所有帧都稳定。
实拍与仿真平均PCC仍不高，分类改善不等于完整光场、相位LUT和精细配准已验收。
`paired_hardware_CCD.png`固定展示每类第一张，不按效果挑选；实拍固定0～255显示，
仿真共用线性尺度，不能直接把两种单位的亮度作绝对比较。
`paired_accuracy.png`只比较同一40张；不要与完整4157张仿真混作同一指标。

绘图（不控制硬件）：

```powershell
python mnist_native8_figures.py --run assets/mnist_native8_completed_20260913 --hardware-run results/mnist_pair_wait400_20260913
```
