...
固定输入两张快速相位检查、200ms数字等待复测。2026-09-13首轮因棋盘格探针近乎全黑停止，
未获得可批准的曝光或新等待值；不应使用这轮暗背景曲线选择曝光。

2026-09-13 新ROI下的 **ABO六层真实流程已完成**：4查询、100候选，324张网络实采，
R@1=3/4，R@5=4/4；不是完整数据集准确率，也不是四样本微调。
六层采前/采后PCC均>0.9995；相位LUT和精细配准仍未正式验收。
结果、CCD图和复现入口见 [ABO_SIX_SMOKE_20260913.md](ABO_SIX_SMOKE_20260913.md)。
MNIST旧mask重采样与原生8μm训练的对照见
[MNIST_NATIVE8_COMPARISON.md](MNIST_NATIVE8_COMPARISON.md)，训练状态以服务器run为准。
...
**尚未验收通过**。MNIST单层结果与上面的ABO六层流程检查是两项不同实验；旧配置和会话未覆盖。
