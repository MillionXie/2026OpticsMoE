...
2026-09-13 晚：按用户要求暂停自动六层换相位，改用 `manual_stage.py`。
每次只显示命令指定的一个相位并持续保持，师弟电脑采当前层后用CUDA计算并生成下一层输入，
**不会切下一层相位**。报告状态 `next_inputs_ready_waiting_for_user_layer_change` 后等待用户指令。
相位SDK在主线程运行；不插入平相位挑战、不自动重连、不宣称已通过相位光学认证。
操作员要求释放时，在该次输出目录建立 `RELEASE` 文件，先取消/结束远端任务再释放相位。
后续层沿用已有会话的真实CCD，不覆盖或删除已有采集。查看本次日志位置以 `reports/00_current/00_READ_ME.md` 为准。
...
新增全量诊断入口 `smoke_six.py --full-dataset --limit 0`：仅用于明确声明
`diagnostic_full_dataset=true` 且没有 `diagnostic_query_indices` 的新配置，固定2400查询、100标题，
六层14700张真实网络采集，无按结果选样、无仿真CCD替换。保留每层平相位挑战、重复和采后验证，
不是逐帧硬件确认，也不冒充相位LUT/精细配准已经正式验收。失败即停，不跳过失败阶段计算结果。
普通4查询入口及其1..8限制不变。每个远端作业的日志绝对路径由协调日志的 `REMOTE_LOG` 行给出。
正式已批准参考库路径仍使用 `dual_run.py`；本入口不会伪造参考库批准。

以下是此前300μs记录：**300μs+240ms时序短测通过，但统一300μs的保守六层曝光筛查未通过**。
240ms为80/80，最低PCC0.997280，单张原始帧就绪平均322.85ms；视觉全局相位下另20/20通过。
三个视觉阶段的抽样曝光/重复性通过，三个语言阶段p99最低11～12、重复PCC最低0.929～0.941，
因此没有修改正式配置、没有启动六层全量。详细结果与周期图仍在`reports/00_current`。

此前300μs+220ms未通过：80次数字切换79次正确，`test65_d1`拍到上一张0
...
已有整批完成后，用同一`--out`和`--append-trial 300 240 --switch-count 80`追加一组，
不同时传`--trial`或`--resume`；已完成组绝不重采。未完成整批不能追加，避免争用设备或覆盖部分采集。
