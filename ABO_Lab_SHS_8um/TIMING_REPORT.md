...
**补测决定（2026-09-12）：正式配置继续200 ms。** 100/120/140/160/180/200 ms各10次左右交替，成功数分别9/10、10/10、10/10、9/10、10/10、10/10。随后120与180 ms分别20次数字切换均通过，但160 ms的失败说明不能由少量通过推断更短等待可靠。按实验人员要求，不采用120或180 ms为正式值，也不将200 ms有限次通过说成绝对零错帧保证。原始数据在 `results/dense_timing_20260912`、`results/digits120_20260912`、`results/digits180_20260912`；未更改LAB.local.json。
...
SHS硬件手册第25页列出模拟增益，FastStream手册第59页明确1X/2X/4X/8X为**相机模拟增益**。Gain_X4是4倍档位，不是激光强度增加4倍，也不是脚本对PNG乘4。实验允许固定使用并记录，前提是没有饱和、标定和推理使用相同增益及明确强度尺度；不能把它当光学网络本身的增益或效率。

粗略在线性、已扣黑电平的范围内，输出数字量与 `增益×曝光×入射强度` 有关；因此X4短曝光可得到接近X1较长曝光的亮度，但不能认为信噪比相同。短曝光收集的光子更少，模拟增益不能补回光子统计；也会压缩可容纳的输入亮度范围，放大已有噪声。高增益可能降低后级量化/读出噪声的相对影响，是否有净收益需要实测，不笼统断言一定更好或更坏。

当前0.150 ms曝光远小于200 ms软件等待，缩短曝光对当前周期帮助很小。优先在不饱和和信噪比足够的条件下固定增益。更换增益后应新建会话并复核强度响应，不在同一层自动调增益。当前诊断回读是Gain_X4；配置gain=null会继承设备当前值，正式开始前应显式固定，不假定它为X1。
