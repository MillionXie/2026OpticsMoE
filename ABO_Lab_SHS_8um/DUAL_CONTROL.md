...
**后续构建更新：自动操作现在以 [START_HERE.md](START_HERE.md) 为唯一执行顺序。** `dual_run.py` 已改为调用带相机指纹参考验证的协调器：有界预热/通道恢复、诊断相位挑战、目标参考比对、批后复查、失败隔离与续跑。下文保留SDK不稳定的原始证据；这次增加容错不代表底层SDK已修复。没有标定/批准的参考库不能启动正式采集。
...
上述诊断轮未将预热宣称为修复。随后按用户要求增加 `phase_owner.py` 的显式有界恢复候选，并由相机参考验证决定是否允许采集；不得仅凭恢复API返回成功放行。相位方向、反灰度、SHS四角几何仍待独立验收。
