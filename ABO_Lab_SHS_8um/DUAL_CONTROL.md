...
**2026-09-13更新：SDK已观察到真实相位响应，但仍未通过可靠性验收。**用户再次授权后已恢复有界SDK诊断；纯黑、光栅和透镜能够产生不同实测光场，但存在首次换图无效、重新连接后无效，以及运行中回退到接近初始异常画面的情况。不能将成功的几张照片概括为“SDK已经修好”，不开放正式六层采集验收标记。

只读二进制审计发现，厂商 `HdmiDisplay.dll` 的1920×1200初始化路径会调用颜色通道与VCom设置实现。我们没有显式调用 `Set_SLMVCom`，但不能因此声称 `Create_SDK` 不会重新下发控制器状态。此前“不改VCom”只能解释为“包装代码不显式修改”，不是厂商初始化无副作用的保证。后续诊断**显式调用了文档中的 `Set_channel(1)` 和 `Set_channel(0)`**，结束恢复配置中的红通道0，回执保留；未显式改VCom、ramp、固件或使用控制器复位。未改原厂 `mlo.bmp`，不把替换默认图片作为修复。

### 2026-09-13重试证据（全部原始帧保留）

- `results/sdk_phase_channel_init_20260913_034020`：3次SDK连接共12张。初始化绿→红通道后，首连接透镜可见，但后两次连接的不同相位仍得到近似相同图像。不能将通道切换视为确定修复。
- `results/sdk_phase_channel_each_20260913_034619`：每次写图后绿→红，共8张。第一次透镜失败（与初始黑相位PCC=0.997934）；后续两次光栅PCC=0.983197，透镜与正常黑相位PCC约0.585。首张失败不删除。
- `results/sdk_phase_channel_warmup_20260913_034924`：先30次黑/透镜交替写入及通道往返，约60秒，再采8张。两次透镜PCC=0.983948、两次光栅PCC=0.984266，但最后黑相位与前面正常黑相位PCC降到约0.965、形态接近启动异常图。**预热不是已经验证的修复。**
- `results/sdk_phase_channel_warmup_20260913_035348`：新进程重复相同预热方案，8张均观察到对应的黑/透镜/光栅形态，重复黑相位最低PCC=0.998429，透镜PCC=0.983585，光栅PCC=0.979351，无饱和。此轮成功不抵消上一轮失败；尚无足够证据保证重连和长时运行可靠。结束恢复红通道0、下发纯黑并关闭SDK，相机作业全部结束；关闭SDK后的面板内容未经光学确认。
- 上述PCC均为固定相机ROI `[760,470,1000,710]`，不配准、不逐图增强、不挑选ROI；统计见各目录 `metrics.json`，统一0～255灰度预览见 `comparison_center.png`。相机150 μs、Gain_X4、100 fps一致，图像无255饱和像素。PCC不是控制器应答，也不是相位LUT线性度证明。
- 主线程/独立SDK线程消息泵、双写、持续8-bit重绘均曾测试，未获得稳定恢复。用户曾确认SDK期间Windows扩展屏显示透镜；这不等于已确认控制器实际连续接收正确HDMI图像。SDK/显示链路/控制器同步的根因仍未定位，不据此宣布硬件损坏。

本轮只发布诊断结论，不将尚不可靠的通道往返/预热方案加入生产驱动。相位方向、反灰度、SHS四角几何仍待独立验收。
