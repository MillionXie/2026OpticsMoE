...
def parse_option():
    ...
    parser.add_argument('--smoke_test', default=False,
                        action="store_true",
                        help='run a tiny 1-epoch reproducibility check')
...
def build_datasets(args, preprocess):
    ...
    if dataset_name == 'cifar10':
        ...
        raise ValueError(
            ...
            "Use one of: cifar10, cifar100, eurosat.")
...
def verify_trainable_parameters(model, prompter):
    clip_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    prompt_trainable = sum(p.numel() for p in prompter.parameters() if p.requires_grad)

    print(f'CLIP trainable parameters: {clip_trainable}')
    print(f'Visual prompt trainable parameters: {prompt_trainable}')

    if clip_trainable != 0:
        raise RuntimeError('Frozen CLIP model has trainable parameters.')
    if prompt_trainable == 0:
...
def main():
    ...
    verify_trainable_parameters(model, prompter)
    ...
    train_dataset, val_dataset, class_names = build_datasets(args, preprocess)
    ...
    for epoch in range(args.epochs):
        ...
        if args.smoke_test:
            ...
            if not os.path.isfile(checkpoint_path):
                raise RuntimeError(f'Smoke test checkpoint was not saved: {checkpoint_path}')
            print(f'Smoke test checkpoint saved: {checkpoint_path}')
            print(f'Smoke test train/eval Acc@1: train={train_acc1:.3f}, eval={acc1:.3f}')
