...
| A13 | A07 + 低分辨率电子残差 | test 72.34% ± 0.14 pp，光学依赖 94.07% | 已复验并冻结的性能骨干 |
...
A13 = BP 性能骨干四 seed 达到 72.34% ± 0.14 pp、光学依赖 94.07%，已冻结；
P02 = 同一 A13 骨干上，BP 72.30%、FA-pretrained 71.44%、FA-random 63.29%、
      NoFT 51.18%，每组均为三个配对 seeds。
```

P02 的逐 seed 配对差值为 `BP - FA-pretrained = 0.86 ± 0.26 pp`、
`FA-pretrained - FA-random = 8.15 ± 1.32 pp`。因此 P01 的方法关系已经延伸到 A13 强骨干，
不再是把两个不同实验拼接成结论。FA-pretrained 的 epoch-0 梯度余弦约为 1；FA-random
均值为 0.224，并产生更大的相位漂移但更低的性能。

### 6.5 P03：理想仿真到部署非理想因素

P03 冻结 P02 checkpoint，只在推理光路注入静态相位标定误差、亚像素横向失配和相对 CCD
读出噪声。三个 training seeds × 三个 deployment seeds 的 test 确认表明，在 phase 0.15 rad、
shift 0.125 pixel、detector 5% RMS 和联合工作点下，FA-pretrained 相对 FA-random 仍分别领先
4.69/3.35/7.89/4.20 pp；相对 BP 的差距为 2.03/2.56/1.08/2.00 pp。

边界同样明确：0.25 pixel 横向失配使 BP/FA-pretrained 降到 30.91%/28.45%，并使
FA-pretrained - FA-random 反转为 -7.36 pp。这是高性能光学表征的装调失效区，不能声称固定
反馈关系对任意硬件误差成立。当前 detector noise 仍是经过每层标准化的相对高斯模型，尚未
覆盖绝对光功率、shot noise、量化、饱和和动态范围。

### 6.6 2026-08-19 性能优化闭环：A08--A13
...
| A13 low-resolution residual | **72.34% ± 0.14 pp** | **13.70%** | **94.07%** | 四 seed 复验通过并冻结 |
...
时延和能耗分析。A13 的结构筛选发生在 CIFAR-10；四 seed 复现解决了随机性问题，但没有解决
同数据集选择偏差或外部有效性，不能把 72.34% 当成顶刊级跨任务证据。
...
- P02 在 72% 强骨干上以三个配对 seeds 得到 FA-pretrained 71.44%、BP 72.30%、
  FA-random 63.29%；
- P03 表明上述关系在预注册的相位误差、亚像素失配、读出噪声和联合工作区内仍成立，并找到
  约 0.25 pixel 的装调失效边界；
- A13 说明四 seed 72.34% 准确率、每层至少 50% optical mixing、几十万 residual electronic
...
- 不能把 P03 的数字扰动仿真称为真实硬件验证，也不能外推到未建模的散射、像差、绝对光功率、
  shot noise、量化、饱和和动态范围；
...
- A13 已完成额外 seeds 复验；冻结其结构、电子预算、`main_min=0.50`、50-epoch schedule
  和 source/common start；
