...
A13 = BP 性能骨干四 seed 达到 72.34% ± 0.14 pp、光学依赖 94.07%，已冻结；
      P02 四组已使用共同起点启动，但尚未形成四组最终结果。
```

因此不能把 72.34% 和 P01 的固定反馈关系拼成一个未经运行的结论。A13 已经复验并冻结，
当前 P02 正在相同共同起点上运行 NoFT、BP、FA-pretrained、FA-random。它将回答 P01 结论
能否延伸到更强骨干，而不是继续增加方法组。
...
- 不能声称 A13 的强性能设置已经验证四组结论；P02 尚在运行；
