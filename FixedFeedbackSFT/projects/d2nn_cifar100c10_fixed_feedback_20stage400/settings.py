...
from FixedFeedbackSFT.paths import REPOSITORY_ROOT, RUNS_ROOT, resolve_repository_path
...
def _resolve(value: str | Path, base: Path = REPOSITORY_ROOT) -> Path:
    return resolve_repository_path(value, base=base)
...
def load_settings(path: str | Path) -> Settings:
    ...
    optical = OpticalConfig(
        ...
        residual_main_init=float(_get(raw, "optical.residual.main_init", 0.1)),
        residual_skip_init=float(_get(raw, "optical.residual.skip_init", 0.9)),
    ...
    settings = Settings(
        ...
        output_dir=_resolve(
            _get(
                raw,
                "output_dir",
                str(RUNS_ROOT / "d2nn_cifar100c10_fixed_feedback_20stage400/main"),
            )
        ),
        optical=optical,
        data=data,
