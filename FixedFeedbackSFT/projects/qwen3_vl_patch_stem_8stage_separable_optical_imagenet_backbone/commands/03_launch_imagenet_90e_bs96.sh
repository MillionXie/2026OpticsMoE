...
RUN_DIR="${RUNS_DIR}/p11_imagenet1k_pretrain_bs96_90e"
LOG="${RUNS_DIR}/logs/p11_imagenet1k_pretrain_bs96_90e.log"
...
if [[ -f "${PID_FILE}" ]]; then
  ...
  if [[ -n "${previous_pid}" ]] && kill -0 "${previous_pid}" 2>/dev/null; then
    echo "P11 is already running as PID ${previous_pid}."
    exit 1
  fi
fi
...
sleep 8
kill -0 "${launch_pid}"
echo "Started P11 PID=${launch_pid}, GPUs=${PHYSICAL_GPU_INDICES}, global_batch=192"
echo "LOG=${LOG}"
