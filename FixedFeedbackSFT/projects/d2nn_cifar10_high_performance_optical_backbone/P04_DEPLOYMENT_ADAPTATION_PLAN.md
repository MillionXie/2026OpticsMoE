...
P04-S2 启动后由 command 44 在 CPU 后台检查固定的 24 个结果；若 GPU launcher 提前退出则明确报错，齐全后自动执行 command 41 生成 `comparison.json` 和 `runs.csv`。
...
数 pixel 偏移下的零样本性能严重且非单调退化，但 BP-current 在 10 epoch 内仍恢复到 66.04%--73.28%；FA-pretrained 恢复到 65.44%--71.42%，六个条件全部优于 FA-random，优势为 2.22--5.88 pp。偏移越强，FA-pretrained 与 BP 的差距总体扩大，说明旧算子仍有价值但不是任意偏移下的精确替代。

P04 已回答“部署后能否恢复”，继续枚举更大固定偏移只会形成更多消融。下一阶段转入 P05：训练阶段连续随机错位，先扩大部署前工作区，再在未见固定部署上用同样四组检验残余校准能力。协议见 `P05_MISALIGNMENT_VACCINATION_PLAN.md`。
