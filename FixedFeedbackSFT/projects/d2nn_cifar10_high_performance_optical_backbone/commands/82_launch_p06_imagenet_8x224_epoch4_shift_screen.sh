...
RUN_DIR="FixedFeedbackSFT/runs/d2nn_cifar10_high_performance_optical_backbone/p06_imagenet_8x224_epoch4_shift_screen"
mkdir -p "${RUN_DIR}"
...
nohup env PHYSICAL_GPU_INDEX="${PHYSICAL_GPU_INDEX:-4}" \
  bash FixedFeedbackSFT/projects/d2nn_cifar10_high_performance_optical_backbone/commands/81_run_p06_imagenet_8x224_epoch4_shift_screen.sh \
  > "${RUN_DIR}/run.log" 2>&1 &
pid=$!
