...
BASE_LOG="${RUNS_DIR}/logs/p13_growth16_fa_source_20e_gb192.log"
LATEST_LOG="${RUNS_DIR}/logs/p13_growth16_fa_source_20e_gb192.latest.log"
...
nohup "${TORCHRUN_BIN}" --standalone --nproc_per_node=4 \
  -m "${MODULE}.train" --config "${CONFIG}" "${MODE}" \
