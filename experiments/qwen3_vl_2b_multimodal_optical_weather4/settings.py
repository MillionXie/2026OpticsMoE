...
class Settings:
    ...
    output_dir: Path = PROJECT_DIR / "runs" / "qwen3_vl_2b_multimodal_optical_bdd100k_weather4"
    ...
    teacher_cache_shard_size: int = 32
    hidden_dim: int = 1024
    dropout: float = 0.1
    epochs: int = 30
    ...
    replace_last_n_vision_blocks: int = 20
    ...
    def validate(self) -> None:
        ...
        if self.replace_last_n_vision_blocks != expected_blocks:
            raise ValueError(
                "replace_last_n_vision_blocks must equal optical_conversions * "
                "teacher_blocks_per_conversion "
                f"({self.replace_last_n_vision_blocks} != {expected_blocks})"
            )
