...
def run_and_save_inference(
    *,
    ...
    report = {
        "model": (
            f"qwen3_vl_2b_with_{len(replacement.block_groups)}_single_mask_optical_"
            f"conversions_for_vision_blocks_{replacement.block_groups[0][0]}_"
            f"{replacement.block_groups[-1][1]}"
            if student
            else "full_electronic_qwen3_vl_2b_multimodal_mlp"
        ),
...
def write_comparison(
    ...
    comparison = {
        ...
        "teacher": {
            ...
            "per_class_accuracy": teacher["metrics"].get("per_class_accuracy", {}),
        },
        "student": {
            "model": student["model"],
            "top1_accuracy": student_top1,
            "top5_accuracy": student_top5,
            "per_class_accuracy": student["metrics"].get("per_class_accuracy", {}),
        },
        "accuracy_drop": {
            "definition": "teacher_minus_student",
            "top1": teacher_top1 - student_top1,
            "top5": teacher_top5 - student_top5,
        },
        "loss_weights": loss_weights,
        "class_imbalance_counts": class_imbalance,
    }
