...
def train_optical_student(
    ...
    """Train only optical conversions and the student MLP from cached teacher targets."""
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in iterator:
            ...
            teacher_group_outputs = [
                ...
                for value in batch["teacher_group_outputs"]
            ]
            teacher_token_counts = [int(value) for value in batch["teacher_token_counts"]]
            ...
            _validate_student_boundaries(replacement, teacher_token_counts)
            ...
            if first_shapes is None:
                first_shapes = {
                    ...
                    "per_image_token_counts": teacher_token_counts,
                    "packed_hidden_shape": list(student_group_outputs[0].shape),
                    "distillation_groups": [
                        {
                            "teacher_blocks": [start, end],
                            "group_input_shape": list(surrogate.last_input.shape)
                            if surrogate.last_input is not None
                            else [],
                            "group_output_shape": list(student_output.shape),
                            "teacher_output_shape": list(teacher_output.shape),
                            "optical_field_shapes": surrogate.last_field_shapes or [],
                        }
                        for (start, end), surrogate, student_output, teacher_output in zip(
                            replacement.block_groups,
    ...
    report = {
        ...
        "teacher_forward_during_student_training": False,
        "teacher_targets_source": "teacher_cache/train.pt shards",
        "electronic_residual_bypass": False,
...
def _validate_student_boundaries(
    replacement: VisionBlockReplacement, teacher_token_counts: list[int]
) -> None:
    for group, surrogate in zip(replacement.block_groups, replacement.surrogates):
        if surrogate.last_token_counts != teacher_token_counts:
            raise RuntimeError(
                f"Student group {group} per-image token boundaries "
                f"{surrogate.last_token_counts} do not match cached teacher boundaries "
                f"{teacher_token_counts}"
            )
