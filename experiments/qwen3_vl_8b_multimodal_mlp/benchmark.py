...
def benchmark_inference(
    ...
    count = available if benchmark_batches is None else min(
        available or benchmark_batches, benchmark_batches
    )
    ...
    if progress:
        try:
            ...
            indices = tqdm(indices, desc="Full multimodal inference")
        except ImportError:
            pass
    ...
    with torch.inference_mode():
        for batch_index in indices:
            ...
            cpu_inputs = preprocess_image_text(
                processor, images, classification_prompt
            )
            ...
            if shape_record is None:
                shape_record = {
                    "input_ids": list(cpu_inputs["input_ids"].shape),
                    "attention_mask": list(cpu_inputs["attention_mask"].shape),
                    "pixel_values": list(cpu_inputs["pixel_values"].shape),
                    "image_grid_thw": list(cpu_inputs["image_grid_thw"].shape),
                    "image_grid_thw_values": cpu_inputs["image_grid_thw"].tolist(),
                    "last_hidden_state": list(hidden.shape),
                    "answer_positions": answer_positions.cpu().tolist(),
    ...
    metrics = classification_metrics(
        labels_all, predictions_all, top5_all, class_names
    )
    ...
    report = {
        ...
        "peak_cuda_memory": _peak_memory(device),
    }
    write_csv(
        output_dir / "metrics" / "inference_batches.csv",
        rows,
        list(rows[0]) if rows else [],
    )
    write_json(output_dir / "metrics" / "inference.json", report)
    _write_predictions(
        output_dir / "metrics" / "predictions.csv",
        labels_all,
        predictions_all,
        class_names,
    )
    _write_confusion(
        output_dir / "metrics" / "confusion_matrix.csv",
