...
def save_optical_diagnostics(model:Any,images:torch.Tensor,root:Path,epoch:int)->None:
    ...
    values=diagnostics["detector_input"].detach().cpu();distributions=diagnostics["region_distribution"].detach().cpu();detector_fractions=diagnostics["detector_fraction"].detach().cpu();sample_count=min(4,len(values));colors=["cyan","lime","magenta"];fig,axes=plt.subplots(sample_count,2,figsize=(9,3.2*sample_count),squeeze=False)
    for i in range(sample_count):
        ax=axes[i,0];ax.imshow(_log(values[i]),cmap="inferno")
        for color,box in zip(colors,model.class_detector.boxes):ax.add_patch(Rectangle((box["x0"],box["y0"]),box["width"],box["height"],fill=False,edgecolor=color,linewidth=1.5));ax.text(box["x0"],max(0,box["y0"]-3),box["class_name"],color=color,fontsize=7)
        ax.set_title(f"Sample {i}: detector energy={detector_fractions[i]:.3f}");ax.axis("off");bar=axes[i,1];bar.bar(model.class_detector.class_names,distributions[i].numpy(),color=colors);bar.set_ylim(0,1);bar.set_ylabel("Energy share inside regions");bar.set_title(f"Region prediction: {model.class_detector.class_names[int(distributions[i].argmax())]}")
    path=root/"detector_outputs"/f"{epoch_name}.png";path.parent.mkdir(parents=True,exist_ok=True);fig.tight_layout();fig.savefig(path,dpi=150);plt.close(fig);_save_region_layout(model,root/"detector_regions"/"layout.png",colors);model.train(was)
...
def _save_region_layout(model:Any,path:Path,colors:list[str])->None:
    ...
    for color,box in zip(colors,model.class_detector.boxes):ax.add_patch(Rectangle((box["x0"],box["y0"]),box["width"],box["height"],facecolor=color,edgecolor="white",alpha=.55));ax.text((box["x0"]+box["x1"])/2,(box["y0"]+box["y1"])/2,box["class_name"],ha="center",va="center",fontsize=9,weight="bold")
