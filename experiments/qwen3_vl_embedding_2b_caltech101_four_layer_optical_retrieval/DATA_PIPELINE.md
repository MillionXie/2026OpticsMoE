...
服务器 compact_amplitude/phase 478 PNG
    ↓ 仅传紧凑文件
实验室按各自 SLM 的物理像素规则重建，并按已标定中心补零
    ↓
amplitude：478×478 @ 17 µm，1:1 放入 1024×1024
phase：478×478 @ 17 µm，按物理坐标栅格化到约 1016×1016 @ 8 µm，
       再放入 1920×1200
    ↓ 光路
相机 ROI（仅内存 uint16）
...
紧凑 SLM 重建是确定性的。振幅不缩放，一个逻辑像素对应一个输入 SLM 像素，
`478×478` 有效区居中放入 `1024×1024`，位于约 `518×518` 可用孔径内。相位不使用
普通图像 resize，而是按像素中心的物理坐标执行 nearest 栅格化；`17/8=2.125`，所以
有效区约为 `1016×1016`，不会累积非整数倍率的单向偏差。相位可通过
`--center-x/--center-y` 使用标定中心，当前为 `(980,590)`；纵向翻转仍沿用旧光路规则。
