...
def _plot_retrieval_rows(
    ...
    *,
    ...
    available_ranks = [
        ...
        if f"top{rank}_gallery_path" in rows[0] and rows[0][f"top{rank}_gallery_path"]
    ]
    panel_count = 1 + len(available_ranks)
    figure, axes = plt.subplots(
        len(rows),
        panel_count,
        figsize=(3.5 * panel_count, 3.5 * len(rows)),
        squeeze=False,
        ...
        for rank in available_ranks:
            panels.append(
                (
                    ...
                    f"Top-{rank}: {row[f'top{rank}_class']}\n"
