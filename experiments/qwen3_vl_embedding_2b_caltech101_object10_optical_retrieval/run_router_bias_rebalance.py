"""Rebalance deterministic Top-2 choice using a training-only gate-bias update."""
...
def main() -> int:
    ...
    config = experiment / "configs" / "caltech101_10class_router_bias_rebalance.yaml"
    checkpoint = (
        Path(args.checkpoint).expanduser().resolve()
        if args.checkpoint
        else experiment / "runs" / "caltech101_10class_finetune_epoch50" / "ema_last_checkpoint.pt"
    )
    output = experiment / "runs" / "caltech101_10class_router_bias_rebalance_epoch53"
    commands = [
