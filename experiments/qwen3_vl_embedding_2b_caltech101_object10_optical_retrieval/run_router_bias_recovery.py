"""Freeze calibrated routers and recover task accuracy through optical training."""
...
def main() -> int:
    ...
    config = experiment / "configs" / "caltech101_10class_router_bias_rebalance_strong_recovery.yaml"
    source = experiment / "runs" / "caltech101_10class_router_bias_rebalance_strong_epoch53"
    checkpoint = source / "last_checkpoint.pt"
    output = experiment / "runs" / "caltech101_10class_router_bias_rebalance_strong_recovery_epoch58"
    if not checkpoint.is_file():
        raise FileNotFoundError(f"Strong calibrated checkpoint is missing: {checkpoint}")
    commands = [
