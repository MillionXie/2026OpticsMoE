...
class TeacherEmbeddingStore:
    ...
    def lookup(self, samples: Sequence[Caltech101Sample]) -> torch.Tensor:
        try:
    ...
    def split(
        self, samples: Sequence[Caltech101Sample]
...
def cache_identity(
    bundle: Caltech101RetrievalBundle, settings: Settings
) -> dict[str, Any]:
    return {
        "cache_version": CACHE_VERSION,
        "dataset": f"Caltech101-{settings.dataset_variant}-class-retrieval",
...
def _sample_record(sample: Caltech101Sample) -> dict[str, Any]:
    return {
        "sample_id": sample.sample_id,
        "image_path": str(sample.image_path.resolve()),
        "class_id": sample.class_id,
