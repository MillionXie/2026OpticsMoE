...
def evaluate_all_systems(
    ...
    for name, result in systems.items():
        for class_name, values in result.metrics["per_class"].items():
            per_class_rows.append({"system": name, "class": class_name, **values})
    write_csv(
        settings.output_dir / "per_class_metrics.csv",
        per_class_rows,
        ["system", "class", "query_count", "top1_accuracy", "top3_accuracy"],
    )
    _plot_confusion(
...
def _plot_confusion(
    ...
    axis.set_title(
        f"Optical Student Caltech101-{len(class_names)} Retrieval Confusion Matrix"
    )
