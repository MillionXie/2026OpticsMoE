...
class DeepStackMultimodalReplacement:
    ...
    def _capture_language(
        ...
        setattr(self.model, "_caltech101_retrieval_optical_last_hidden", value)
    ...
    def router_hard_load_balance_loss(self) -> dict[str, torch.Tensor]:
        vision = self.vision_surrogate.router_hard_load_balance_loss()
        if self.language_mode == "optical_moe":
            language = self.language_surrogate.router_hard_load_balance_loss()
        else:
            language = vision.new_zeros(())
        return {"vision": vision, "language": language}

    def update_router_load_bias(self, update_rate: float) -> None:
        self.vision_surrogate.update_router_load_bias(update_rate)
        if self.language_mode == "optical_moe":
            self.language_surrogate.update_router_load_bias(update_rate)
    ...
    def close(self) -> None:
        ...
        for handle in self._handles:
            handle.remove()
        if hasattr(self.model, "_caltech101_retrieval_optical_last_hidden"):
            delattr(self.model, "_caltech101_retrieval_optical_last_hidden")
