...
class PKBatchSampler(Sampler[list[int]]):
    """Deterministic epoch-aware P-class x K-image batch sampler."""
    ...
    def __init__(
        ...
        for index, sample in enumerate(samples):
            grouped[sample.class_index].append(index)
        self.grouped = {key: tuple(values) for key, values in sorted(grouped.items())}
        if len(self.grouped) < self.p:
            raise ValueError(f"PK sampler needs P={self.p} classes, found {len(self.grouped)}")
        if any(not values for values in self.grouped.values()):
            raise ValueError("PK sampler encountered an empty class")
    ...
    def __iter__(self) -> Iterator[list[int]]:
        ...
        class_ids = list(self.grouped)
        pools: dict[int, list[int]] = {}
        positions: dict[int, int] = {}
        for class_id in class_ids:
            order = torch.randperm(len(self.grouped[class_id]), generator=generator).tolist()
            pools[class_id] = [self.grouped[class_id][index] for index in order]
            positions[class_id] = 0
        class_order = torch.randperm(len(class_ids), generator=generator).tolist()
        class_cursor = 0
        for _ in range(self.batch_count):
            if self.p == len(class_ids):
                selected_classes = class_ids
            else:
                if class_cursor + self.p > len(class_order):
                    class_order = torch.randperm(len(class_ids), generator=generator).tolist()
                    class_cursor = 0
                selected_classes = [class_ids[index] for index in class_order[class_cursor:class_cursor + self.p]]
                class_cursor += self.p
            batch: list[int] = []
            for class_id in selected_classes:
                for _ in range(self.k):
                    if positions[class_id] >= len(pools[class_id]):
                        order = torch.randperm(
                            len(self.grouped[class_id]), generator=generator
                        ).tolist()
                        pools[class_id] = [self.grouped[class_id][index] for index in order]
                        positions[class_id] = 0
                    batch.append(pools[class_id][positions[class_id]])
...
def supervised_contrastive_loss(
    ...
    if not torch.all(valid):
        ...
        raise RuntimeError(
            f"Every contrastive anchor needs a same-class positive; missing anchors={missing}"
        )
...
def gallery_retrieval_loss(
    ...
    *,
    ...
    """Cross-entropy retrieval against one differentiable prototype per class.

    Unlike the in-batch supervised contrastive objective, this loss exactly
    matches deployment: every natural query must rank its standard gallery class
    above all other gallery classes. Multiple gallery images per class are supported
...
def gallery_retrieval_logits(
    ...
    *,
    ...
    class_ids = torch.unique(gallery_labels, sorted=True)
    if class_ids.numel() < 2:
        raise RuntimeError("Gallery retrieval loss needs at least two distinct classes")
    prototypes = []
    for class_id in class_ids:
        prototype = gallery[gallery_labels == class_id].mean(dim=0)
        prototypes.append(F.normalize(prototype, dim=0))
    prototype_tensor = torch.stack(prototypes)
    targets = torch.searchsorted(class_ids, query_labels)
    valid = (targets < len(class_ids)) & class_ids[
        targets.clamp_max(len(class_ids) - 1)
    ].eq(query_labels)
    if not torch.all(valid):
        missing = torch.unique(query_labels[~valid]).detach().cpu().tolist()
        raise RuntimeError(f"Gallery is missing query class labels: {missing}")
...
def retrieval_ranking_sums(
    ...
    if logits.shape[1] < 2:
        raise ValueError("Retrieval metrics need at least two candidate classes")
...
def select_gallery_items_for_queries(
    ...
    """Select all standard gallery images for only the classes in this query batch."""
    requested = sorted({int(sample.class_index) for sample in query_samples})
    grouped: dict[int, list[dict[str, Any]]] = defaultdict(list)
    for item in gallery_items:
        grouped[int(item["sample"].class_index)].append(item)
    missing = [class_index for class_index in requested if class_index not in grouped]
    if missing:
        raise RuntimeError(f"Gallery is missing query class labels: {missing}")
    return [
        item
        for class_index in requested
        for item in grouped[class_index]
    ]
    ...
    payload = {
        ...
        "metadata": {
            ...
            "training_objective": {
                ...
                "router_load_bias_update_rate": (
                    settings.router_load_bias_update_rate
                ),
...
def train_optical_retrieval(
    ...
    *,
    ...
    train_dataset = Caltech101RetrievalDataset(
        ...
        horizontal_flip_probability=settings.horizontal_flip_probability,
    )
    sampler = PKBatchSampler(
        bundle.train_samples,
        settings.pk_classes_per_batch,
        settings.pk_images_per_class,
    ...
    gallery_dataset = Caltech101RetrievalDataset(
    ...
    gallery_class_ids = {
        int(item["sample"].class_index) for item in gallery_items
    }
    if settings.lambda_gallery > 0 and len(gallery_class_ids) != len(bundle.class_names):
        raise RuntimeError(
            "Gallery-aligned loss requires at least one standard image for every class"
        )
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        for batch_index, batch in enumerate(loader, 1):
            ...
            query_labels = torch.tensor(
                [sample.class_index for sample in batch["samples"]],
            ...
            with torch.autocast(
                ...
                hard_load_losses = replacement.router_hard_load_balance_loss()
                hard_load_balance = 0.5 * (
                    hard_load_losses["vision"] + hard_load_losses["language"]
                )
                router_response = (
            ...
            optimizer.step()
            if (
                settings.router_load_bias_update_rate > 0.0
                and relative_epoch >= settings.router_load_bias_update_start_epoch
            ):
                replacement.update_router_load_bias(
                    settings.router_load_bias_update_rate
                )
        ...
        if (
            ...
            save_phase_preview(
                ...
                title=(
                    f"Caltech101 optical phase epoch {epoch} "
