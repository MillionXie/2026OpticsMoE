...
1. `Optical5EnhancedReadout`：五次可微光学传播与平方律探测，最终探测面设置三个固定类别方格，再接两层卷积、普通平均池化和轻量 MLP detector readout。
...
最终 256×256 探测面中央水平排列三个固定 48×48 方格，顺序为 `daytime`、`night`、`dawn_dusk`。训练目标为：

```text
L = electronic_classification_CE
  + 1.0 * detector_region_CE
  + 0.1 * detector_concentration_loss
```

`detector_region_CE` 要求真实类别对应方格的能量高于另外两个方格；`detector_concentration_loss` 防止绝大多数能量留在三个方格以外。电子 readout 仍处理完整末端强度图，并额外接收三个方格内部的归一化能量分布。详细设计见 `DETECTOR_REGIONS.md`。
...
区域监督改变了 readout 输入维度和训练目标，旧版 `best.pt`/`last.pt` 与新版结构不兼容，需要使用新的输出目录重新训练。
