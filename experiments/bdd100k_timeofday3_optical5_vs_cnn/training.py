...
def train_model(model:nn.Module,data:DataBundle,settings:Any,device:torch.device)->list[dict[str,Any]]:
    ...
    for epoch in range(1,settings.epochs+1):
        ...
        if isinstance(model,OpticalTimeOfDayClassifierBase) and (epoch==1 or epoch%settings.save_interval_epochs==0 or epoch==settings.epochs):save_optical_diagnostics(model,diagnostic_images,settings.output_dir/"figures",epoch)
...
def _write_predictions(path:Path,result:dict[str,Any],names:Sequence[str])->None:
    ...
    has_detector="detector_region_logits" in result;detector_fields=["detector_pred_label","detector_pred_name","detector_energy_fraction",*[f"detector_region_fraction_{name}" for name in names]] if has_detector else [];fieldnames=["sample_index","image_path","true_label","true_name","pred_label","pred_name","correct",*detector_fields,*[f"logit_{name}" for name in names]]
