...
def train_model(model:nn.Module,data:DataBundle,settings:Any,device:torch.device)->list[dict[str,Any]]:
    ...
    for epoch in range(1,settings.epochs+1):
        ...
        for batch_index,(images,labels,indices,paths) in enumerate(train_loader,1):
            ...
            logits,loss,components,aux=_forward_objective(model,images,labels,criterion);loss.backward();optimizer.step()
            ...
            if batch_index%settings.log_interval_batches==0 or batch_index==len(train_loader):
                ...
                detector_running=(sum(int(a==b) for a,b in zip(targets,detector_predictions))/seen) if detector_predictions else None
                detector_text=f" detector_region_top1={detector_running:.4f}" if detector_running is not None else ""
...
def evaluate(model:nn.Module,loader:Any,device:torch.device,class_names:Sequence[str])->dict[str,Any]:
    model.eval();criterion=nn.CrossEntropyLoss();loss_totals={"total":0.0,"classification":0.0,"detector_region":0.0,"detector_concentration":0.0};logits_all=[];labels_all=[];indices_all=[];paths_all=[];region_logits_all=[];region_fractions_all=[];detector_fractions_all=[]
    for images,labels,indices,paths in loader:
        ...
        logits_all.append(logits.cpu());labels_all.append(labels);indices_all.extend(indices.tolist());paths_all.extend(paths)
        if aux is not None:
            region_logits_all.append(aux["region_logits"].cpu());region_fractions_all.append(aux["region_fractions"].cpu());detector_fractions_all.append(aux["detector_fraction"].cpu())
    logits=torch.cat(logits_all);labels=torch.cat(labels_all);predictions=logits.argmax(1);metrics=classification_metrics(labels.tolist(),predictions.tolist(),list(class_names));samples=max(1,len(labels))
    metrics.update({"loss":loss_totals["total"]/samples,"classification_loss":loss_totals["classification"]/samples,"detector_region_loss":loss_totals["detector_region"]/samples,"detector_concentration_loss":loss_totals["detector_concentration"]/samples})
    result={"metrics":metrics,"logits":logits,"labels":labels,"indices":indices_all,"paths":paths_all}
    if region_logits_all:
        region_logits=torch.cat(region_logits_all);region_fractions=torch.cat(region_fractions_all);detector_fractions=torch.cat(detector_fractions_all);region_predictions=region_logits.argmax(1)
        target_fractions=region_fractions.gather(1,labels[:,None]).squeeze(1)
        metrics.update({"detector_region_accuracy":float(region_predictions.eq(labels).float().mean()),"mean_detector_energy_fraction":float(detector_fractions.mean()),"mean_target_region_energy_fraction":float(target_fractions.mean()),"per_class_mean_target_region_energy_fraction":{name:float(target_fractions[labels.eq(index)].mean()) if labels.eq(index).any() else 0.0 for index,name in enumerate(class_names)}})
        result.update({"detector_region_logits":region_logits,"detector_region_fractions":region_fractions,"detector_fractions":detector_fractions})
    return result
...
def _forward_objective(model:nn.Module,images:torch.Tensor,labels:torch.Tensor,criterion:nn.Module)->tuple[torch.Tensor,torch.Tensor,dict[str,torch.Tensor],dict[str,torch.Tensor]|None]:
    if isinstance(model,Optical5EnhancedTimeOfDayClassifier):
        logits,aux=model(images,return_aux=True);classification=criterion(logits,labels);region=criterion(aux["region_logits"],labels);concentration=-torch.log(aux["detector_fraction"].clamp_min(1e-8)).mean()
        total=classification+model.detector_region_loss_weight*region+model.detector_concentration_loss_weight*concentration
...
def _write_predictions(path:Path,result:dict[str,Any],names:Sequence[str])->None:
    ...
    has_detector="detector_region_logits" in result
    detector_fields=["detector_pred_label","detector_pred_name","detector_energy_fraction",*[f"detector_region_fraction_{name}" for name in names]] if has_detector else []
    fieldnames=["sample_index","image_path","true_label","true_name","pred_label","pred_name","correct",*detector_fields,*[f"logit_{name}" for name in names]]
    with path.open("w",newline="",encoding="utf-8") as handle:
        writer=csv.DictWriter(handle,fieldnames=fieldnames);writer.writeheader()
        detector_predictions=result["detector_region_logits"].argmax(1).tolist() if has_detector else [None]*len(predictions)
        ...
        for index,image_path,truth,pred,values,detector_pred,detector_fraction,region_values in zip(result["indices"],result["paths"],result["labels"].tolist(),predictions,result["logits"].tolist(),detector_predictions,detector_fractions,region_fractions):
            row={"sample_index":index,"image_path":image_path,"true_label":truth,"true_name":names[truth],"pred_label":pred,"pred_name":names[pred],"correct":truth==pred};row.update({f"logit_{name}":value for name,value in zip(names,values)})
            if has_detector:
                row.update({"detector_pred_label":detector_pred,"detector_pred_name":names[detector_pred],"detector_energy_fraction":detector_fraction})
                row.update({f"detector_region_fraction_{name}":value for name,value in zip(names,region_values)})
            writer.writerow(row)
