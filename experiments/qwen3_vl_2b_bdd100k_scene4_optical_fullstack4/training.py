...
def train_teacher_head(train_store: TeacherCacheStore, test_store: TeacherCacheStore, settings: Any,
    ...
    sampler=EpochClassMixedSampler(
        range(len(train_idx)),train_labels.tolist(),len(class_names),settings.head_batch_size,
        settings.seed,settings.train_samples_per_class_per_epoch,oversample_minority=settings.oversample_minority_classes,
    )
    history=[]; best=-1.0
    for epoch in range(1,settings.epochs+1):
        head.train(); total=0.0;sampler.set_epoch(epoch)
        loader=DataLoader(TensorDataset(train_features,train_labels),batch_size=settings.head_batch_size,sampler=sampler)
        for x,y in loader:
        ...
        row={"epoch":epoch,"loss":total/len(sampler),"epoch_samples":len(sampler),**{k:metrics[k] for k in ("top1_accuracy","top2_accuracy","top5_accuracy","macro_f1","balanced_accuracy")}}
...
def train_student(model: nn.Module, processor: Any, replacement: Any, head: MLPHead, train_dataset: Dataset[Any],
    ...
    sampler=EpochClassMixedSampler(train_indices,labels_of(train_dataset),len(class_names),settings.student_batch_size,settings.seed,settings.train_samples_per_class_per_epoch,settings.teacher_cache_shard_size,settings.oversample_minority_classes)
    ...
    for epoch in range(1,settings.epochs+1):
        ...
        for batch_index,(images,labels,indices,cached_grids,cached_counts,teacher_vision,teacher_answer,teacher_batch_logits) in enumerate(loader,1):
            ...
            if batch_index%settings.log_interval_batches==0 or batch_index==len(loader):
                ...
                print(f"epoch {epoch}/{settings.epochs} batch {batch_index}/{len(loader)}\nloss_total={totals['total']/seen:.5f} loss_vision={totals['vision']/seen:.5f} loss_answer={totals['answer']/seen:.5f} loss_kd={totals['kd']/seen:.5f} loss_ce={totals['ce']/seen:.5f} running_top1={running['top1_accuracy']:.4f} lr={optimizer.param_groups[0]['lr']:.3e}",flush=True)
    ...
    write_json(settings.output_dir/"metrics"/"student_training.json",{"epochs":settings.epochs,"best_validation_top1":best_top1,"best_validation_macro_f1":best_macro,"history_rows":len(history)})
...
def save_student_inference(result: dict[str,Any], settings: Any, names: Sequence[str]) -> list[dict]:
    write_json(settings.output_dir/"metrics"/"student_inference.json",result["metrics"])
...
def _write_first_shapes(settings:Any,inputs:dict,teacher_vision:list,teacher_answer:torch.Tensor,student_answer:torch.Tensor,replacement:Any,logits:torch.Tensor)->None:
    write_json(settings.output_dir/"metrics"/"first_batch_shapes.json",{
        "input_ids":list(inputs["input_ids"].shape),"attention_mask":list(inputs["attention_mask"].shape),"pixel_values":list(inputs["pixel_values"].shape),"image_grid_thw":inputs["image_grid_thw"].tolist(),"visual_token_counts":replacement.vision_surrogate.last_token_counts,"teacher_vision_stack_output":[list(x.shape) for x in teacher_vision],"student_vision_stack_output":list(replacement.vision_surrogate.last_output.shape),"teacher_answer_hidden":list(teacher_answer.shape),"student_answer_hidden":list(student_answer.shape),"vision_optical_field_shapes":[list(x.shape) for x in replacement.vision_surrogate.last_fields],"language_optical_field_shapes":[list(x.shape) for x in replacement.language_surrogate.last_fields],"logits":list(logits.shape)})
