...
from .datasets import DatasetBundle, load_scene4, make_indexed_loader, stratified_split_indices
...
def build_parser() -> argparse.ArgumentParser:
    parser=argparse.ArgumentParser(description="Qwen3-VL-2B BDD100K Scene-4 full vision+language optical4 distillation")
...
def main(argv:list[str]|None=None)->int:
    ...
    data=load_scene4(settings); write_json(settings.output_dir/"dataset.json",data.metadata); _write_dataset_report(settings,data)
    if args.phase=="prepare_data": print(f"BDD100K Scene-4 ready: train={len(data.train)} test={len(data.test)}"); return 0
...
def _compare(settings:Settings,names:list[str])->None:
    ...
    comparison={"dataset":"bdd100k_scene4","classes":names,"replacement":"both_full_stacks_optical4","teacher":teacher,"student":student,"accuracy_drop":{"top1":teacher["top1_accuracy"]-student["top1_accuracy"],"top2":teacher["top2_accuracy"]-student["top2_accuracy"],"top5":teacher["top5_accuracy"]-student["top5_accuracy"]}}
...
def _write_dataset_report(settings:Settings,data:DatasetBundle)->None:
    metadata=data.metadata
    lines=[
        "# BDD100K Scene-4 Run Dataset Report","",
        f"- Data root: `{settings.data_root}`",
        f"- Classes: `{', '.join(data.class_names)}`",
        f"- Validation fraction: `{settings.validation_fraction}`",
        f"- Minority oversampling: `{settings.oversample_minority_classes}`",
        f"- Samples per class per training epoch: `{settings.train_samples_per_class_per_epoch}`","",
        "## Effective splits","",
        "| Class | Full BDD train | Student train | Validation | Test (BDD val) | Epoch samples |",
        "|---|---:|---:|---:|---:|---:|",
    ]
    full=metadata["per_class_full_train_counts"];train=metadata["per_class_train_counts"]
    validation=metadata["per_class_validation_counts"];test=metadata["per_class_test_counts"];epoch=metadata["per_class_epoch_sample_counts"]
    for name in data.class_names:
        lines.append(f"| `{name}` | {full[name]} | {train[name]} | {validation[name]} | {test[name]} | {epoch[name]} |")
    imbalance=metadata["class_imbalance"]
    lines.extend(["", "## Imbalance", "",
                  f"- Majority class: `{imbalance['majority_class']}`",
                  f"- Minority class: `{imbalance['minority_class']}`",
                  f"- Train majority/minority ratio: `{imbalance['train_majority_to_minority_ratio']:.2f}:1`",
                  "- Report macro-F1, balanced accuracy, per-class recall, and confusion matrix together with top-1 accuracy.",""])
    (settings.output_dir/"dataset_report.md").write_text("\n".join(lines),encoding="utf-8")
