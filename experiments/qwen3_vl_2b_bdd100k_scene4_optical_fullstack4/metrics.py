...
def metrics_from_logits(logits: torch.Tensor, labels: torch.Tensor, class_names: Sequence[str]) -> dict[str, Any]:
    predictions = logits.argmax(dim=1); top2=logits.topk(min(2,logits.shape[1]),dim=1).indices;top5 = logits.topk(min(5, logits.shape[1]), dim=1).indices
    ...
    return {
        ...
        "top2_accuracy": float(sum(int(int(y) in row) for y,row in zip(labels.cpu().tolist(),top2.cpu().tolist())))/total,
