...
from .data import build_canonical_cache, cache_report, load_canonical_cache
...
PHASES = {"preflight", "cache", "train", "evaluate", "smoke"}
...
def synthetic_smoke(settings: ExperimentSettings) -> dict[str, Any]:
    """Small CPU test of E2 and O2, including all four fusion layers."""
    ...
    for backend in ("electronic", "optical"):
        variant = replace(
            settings,
            router_backend=backend,
            top_k=2,
            output_dir=settings.output_dir / f"smoke_{backend}",
            synthetic=True,
            device="cpu",
        )
        variant.validate()
        ...
        if bad or not bool(torch.isfinite(loss)):
            raise RuntimeError(f"Synthetic {backend} smoke has non-finite values: {bad}")
...
def main() -> int:
    ...
    if args.phase == "cache":
        required = {
            ...
            "source_feature_cache": settings.source_feature_cache,
            "cache": settings.cache_path,
        }
        missing = [name for name, value in required.items() if value is None]
        if missing:
            parser.error(f"cache phase needs config fields {missing}")
        assert settings.manifest_path is not None
        assert settings.source_feature_cache is not None
        assert settings.cache_path is not None
        report = build_canonical_cache(
            manifest_path=settings.manifest_path,
            source_feature_cache=settings.source_feature_cache,
            source_language_cache=settings.source_language_cache,
            output_path=settings.cache_path,
            frame_count=settings.frame_count,
            token_count=settings.token_count,
            prompt=settings.prompt,
            language_token_count=settings.language_token_count,
        )
        _json(settings.output_dir / "cache_report.json", report)
        print(json.dumps(report, indent=2, ensure_ascii=False))
        return 0
