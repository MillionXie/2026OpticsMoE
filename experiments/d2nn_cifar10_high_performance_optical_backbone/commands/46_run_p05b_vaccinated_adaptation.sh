...
: "${CONDITIONS_CSV:?Set CONDITIONS_CSV to a comma-separated subset of P05-B conditions}"
...
"${PYTHON_BIN}" -m experiments.d2nn_cifar10_high_performance_optical_backbone.deployment_adaptation \
  --config experiments/d2nn_cifar10_high_performance_optical_backbone/configs/p05b_vaccinated_deployment_adaptation.yaml \
  --phase run \
  --methods "${methods[@]}" \
