...
launch_one() {
  local name="$1"
  local gpu="$2"
  local config="experiments/d2nn_cifar10_high_performance_optical_backbone/configs/p07_teacher_free_screen_${name}.yaml"
  local run_dir="experiments/d2nn_cifar10_high_performance_optical_backbone/runs/p07_teacher_free_screen_${name}"
  ...
  nohup env PHYSICAL_GPU_INDEX="${gpu}" P07_CONFIG="${config}" \
    bash experiments/d2nn_cifar10_high_performance_optical_backbone/commands/87_run_p07_teacher_free_head_screen.sh \
