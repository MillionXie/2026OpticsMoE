...
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline import (
    TASK_PROMPTS,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.datasets import (
    load_spaq,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.features import (
    multimodal_forward_features,
    pool_answer_hidden_state,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.modeling import (
    build_head,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.optics.geometry import (
    Aperture,
    MoEGeometry,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.optics.moe import (
    ExpertPhasePlane,
    FullPlaneReadout,
...
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.optics.physical import (
    AngularSpectrumPropagator,
    PhaseLayer,
    SquareDetectionLayerNormReload,
    aperture_linear_indices,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.optics.replacement import (
    DeepStackMultimodalReplacement,
    VisionNativeAttentionPrelude,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.optics.router import (
    ElectronicAmplitudeRouter,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.processor_cache import (
    ProcessorCacheStore,
    collate_processor_samples,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.cache_paths import (
    cache_identity_digest,
    precompute_cache_root,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.settings import (
    load_settings,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.sam import (
    SharpnessAwareMinimizer,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.sampling import (
    EpochRotatingSampler,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.teacher_cache import (
    TeacherCacheStore,
    expected_metadata as expected_teacher_metadata,
)
from experiments.qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline.training import (
...
ROOT = Path(
    "experiments/qwen3_vl_2b_spaq_single_attribute_multimodal_electronic_router_moe16_224_1layer_baseline"
)
...
def test_all_four_single_attribute_configs(filename: str, task: str) -> None:
    ...
    assert settings.phase_dropout_mode == "none"
    assert settings.phase_dropout_p == 0.0
    assert settings.loss_ranking_weight == 0.0
    assert settings.loss_norm_in_norm_weight == 0.0
    assert (
...
def test_parameter_budget_is_within_requested_vision_language_range() -> None:
    ...
    assert total == 4_943_145
    assert 4_900_000 <= total <= 5_000_000
...
def test_teacher_cache_keeps_only_first_auxiliary_and_final_vision_targets(
    tmp_path: Path,
) -> None:
    settings = load_settings(CONFIGS / "spaq_mos_smoke.json")
    settings.precompute_cache_dir = tmp_path / "cache"
    ...
    metadata = expected_teacher_metadata("train", 8, settings, model=None)
    assert metadata["cache_schema_version"] == 4
    assert metadata["deepstack_visual_indexes"] == [5, 11, 17]
    assert metadata["student_deepstack_visual_indexes"] == [5]
    assert metadata["cached_teacher_vision_tap_indexes"] == [5, 23]
    assert (
        metadata["replacement_mode"]
        == "qwen3_vl_single_auxiliary_deepstack_teacher_targets"
    )
...
class _FakeOneStageVisionCore(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.expert_layers = nn.ModuleList([nn.Identity()])

    @staticmethod
    def encode_groups(groups: list[torch.Tensor]) -> torch.Tensor:
        return torch.cat(groups)

    @staticmethod
    def begin(fields: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        return fields, {}
    ...
    def run_stage(
        _index: int,
        field: torch.Tensor,
        _routing: dict[str, torch.Tensor],
    ) -> torch.Tensor:
        return field + 1.0
    ...
    def read_hidden(
        ...
        *,
        ...
        return (field + (10.0 if final else 0.0)).to(boundary_dtype)


def test_one_vision_expert_stage_produces_one_auxiliary_deepstack_tap() -> None:
    surrogate = VisionDeepStackHomogeneousMoE.__new__(VisionDeepStackHomogeneousMoE)
    nn.Module.__init__(surrogate)
    surrogate.core = _FakeOneStageVisionCore()
    surrogate.tap_stages = (1,)
    surrogate.last_token_counts = []
    surrogate.tap_outputs = []
    surrogate.last_output = None
    surrogate.last_residual_base = None
    ...
    assert surrogate.last_token_counts == [2, 3]
    assert len(surrogate.tap_outputs) == 1
    expected_tap = 2.0 * hidden + 1.0
    assert torch.equal(surrogate.tap_outputs[0], expected_tap)
    assert torch.equal(surrogate.last_output, 2.0 * hidden + 11.0)
...
def test_replacement_maps_native_deepstack_taps() -> None:
    ...
    assert model.model.visual.deepstack_visual_indexes == [5]
    assert replacement.vision_blocks[5].slot == 0
    assert replacement.vision_blocks[23].slot == 1
    assert not hasattr(replacement.vision_blocks[11], "slot")
    assert not hasattr(replacement.vision_blocks[17], "slot")
    assert not hasattr(replacement.language_layers[0], "stage")
    assert replacement.language_layers[1].stage == 0
    assert replacement.language_optical_layer_indexes == (1,)
    specification = replacement.alignment_specification()
    assert not specification["vision_single_expert_tap_reused_for_deepstack"]
    assert specification["native_deepstack_visual_indexes"] == [5, 11, 17]
    assert specification["student_deepstack_visual_indexes"] == [5]
    assert specification["student_deepstack_auxiliary_count"] == 1
    assert specification["language_deepstack_injections_before_optics"] == 1
    assert specification["language_optical_layer_indexes"] == [1]
    replacement.close()
    assert model.model.visual.deepstack_visual_indexes == [5, 11, 17]
...
def test_baseline_config_has_no_hidden_finetune_or_regularization_state() -> None:
    settings = load_settings(CONFIGS / "spaq_mos.json")
    assert settings.student_initialization_run_dir is None
    assert settings.student_initialization_tag is None
    assert settings.student_initialization_expected_epoch is None
    assert settings.teacher_artifact_source_run_dir is None
    assert not settings.sam_enabled
    assert settings.weight_decay == 0.0
    assert settings.phase_weight_decay == 0.0
    assert not settings.phase_dropout_enabled
...
def test_optimizer_separates_phase_weight_decay_from_electronics() -> None:
    ...
    assert groups["phase"]["lr"] == pytest.approx(8e-3)
    assert groups["phase"]["weight_decay"] == 0.0
    assert groups["optical_electronic"]["lr"] == pytest.approx(8e-3)
    assert groups["optical_electronic"]["weight_decay"] == 0.0
    assert groups["routers"]["lr"] == pytest.approx(1e-3)
    assert groups["routers"]["weight_decay"] == 0.0
    assert groups["head"]["lr"] == pytest.approx(1e-3)
    assert groups["head"]["weight_decay"] == 0.0
    assert sam is None
