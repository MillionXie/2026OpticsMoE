...
from .cache_paths import cache_identity_digest, teacher_cache_root
...
def expected_metadata(split: str, samples: int, settings: Any, model: nn.Module | None) -> dict[str, Any]:
    native_indexes = list(settings.deepstack_visual_indexes or [])
    selected_indexes = native_indexes[: len(settings.vision_tap_stages)]
    final_index = int(settings.vision_depth) - 1 if settings.vision_depth is not None else None
    return {
        "cache_schema_version": CACHE_SCHEMA_VERSION, "split": split, "sample_count": int(samples),
        "shared_cache_identity": cache_identity_digest(settings),
        ...
        "deepstack_visual_indexes": native_indexes,
...
def build_teacher_cache(split: str, model: nn.Module, processor: Any, replacement: Any, loader: Any,
    ...
    root = teacher_cache_root(settings); manifest_path = root / f"{split}.pt"
