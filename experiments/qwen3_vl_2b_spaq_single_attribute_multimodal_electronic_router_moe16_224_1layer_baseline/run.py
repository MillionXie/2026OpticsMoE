...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "electronic top-k amplitude routing and a one-expert-stage plus "
            "global-phase homogeneous optical MoE16-224 baseline"
        )
    )
    ...
    parser.add_argument("--train-epoch-partitions", type=int)
...
def main(argv: list[str] | None = None) -> int:
    ...
    shared_cache_root = precompute_cache_root(settings)
    shared_cache_root.mkdir(parents=True, exist_ok=True)
    write_json(
        shared_cache_root / "cache_identity.json",
        {
            **cache_identity(settings),
            "digest": cache_identity_digest(settings),
...
def _stores(settings: Settings, data: DatasetBundle):
    stores = {split: TeacherCacheStore(teacher_cache_root(settings) / f"{split}.pt",
                                       settings.teacher_cache_lru_shards) for split in ("train", "test")}
    for split, dataset in (("train", data.train), ("test", data.test)):
        selected_indexes = list(settings.deepstack_visual_indexes or [])[
            : len(settings.vision_tap_stages)
        ]
        expected = {"split": split, "sample_count": len(dataset), "task": settings.task_name,
                    ...
                    "student_deepstack_visual_indexes": selected_indexes,
                    "cached_teacher_vision_tap_indexes": [
                        *selected_indexes,
                        int(settings.vision_depth) - 1,
                    ],
                    "replacement_mode": "qwen3_vl_single_auxiliary_deepstack_teacher_targets"}
...
def _input_stores(settings: Settings, data: DatasetBundle):
    stores = {split: ProcessorCacheStore(processor_cache_root(settings) / f"{split}.pt",
                                         settings.teacher_cache_lru_shards) for split in ("train", "test")}
    for split, dataset in (("train", data.train), ("test", data.test)):
...
def _model_report(model: torch.nn.Module, replacement: Any, settings: Settings):
    ...
    write_json(settings.output_dir / "model.json", {"model_id": settings.model_id, "task": settings.task_name,
        ...
        "native_deepstack_visual_indexes": list(settings.deepstack_visual_indexes or []),
        "student_deepstack_visual_indexes": list(replacement.deepstack_indexes),
        "student_deepstack_auxiliary_count": len(replacement.deepstack_indexes),
        "native_deepstack_preserved": "single_auxiliary_only",
        "vision_single_expert_tap_reused_for_deepstack": False,
        "language_optical_layer_indexes": list(replacement.language_optical_layer_indexes),
        "phase_planes_per_optical_stack": 2,
        ...
        "optical_geometry": {
            ...
            "expert_gap": settings.expert_pitch - settings.expert_size,
            "expert_pitch": settings.expert_pitch,
            "expert_footprint": [settings.active_size, settings.active_size],
            "global_phase_active_size": [settings.active_size, settings.active_size],
            "propagation_canvas": [settings.canvas_size, settings.canvas_size],
            "propagation_outer_padding": (settings.canvas_size - settings.active_size) // 2,
            "ccd_effective_roi": [settings.active_size, settings.active_size],
            "ccd_electronic_readout": [settings.detector_output_size, settings.detector_output_size],
            "expert_stages": settings.expert_layers,
            "phase_planes_per_stack": settings.expert_layers + 1,
        },
        "shared_precompute_cache": {
        ...
        "baseline_regularization_state": {
            "native_attention_enabled": settings.native_pre_attention_enabled,
            "sam_enabled": settings.sam_enabled,
            "electronic_weight_decay": settings.weight_decay,
            "phase_weight_decay": settings.phase_weight_decay,
            "phase_dropout_enabled": settings.phase_dropout_enabled,
            "ranking_loss_weight": settings.loss_ranking_weight,
            "norm_in_norm_loss_weight": settings.loss_norm_in_norm_weight,
            "historical_student_initialization": settings.student_initialization_run_dir is not None,
        },
        "formal_batching": {
            "feature_batch_size": settings.feature_batch_size,
            "student_batch_size": settings.student_batch_size,
            "inference_batch_size": settings.inference_batch_size,
            "head_batch_size": settings.head_batch_size,
        },
...
def _dirs(root: Path):
    # teacher_cache under a run only stores teacher-head predictions. Frozen
    # Qwen teacher/processor shards live in settings.precompute_cache_dir.
    for name in ("teacher_cache", "metrics", "checkpoints", "figures"):
