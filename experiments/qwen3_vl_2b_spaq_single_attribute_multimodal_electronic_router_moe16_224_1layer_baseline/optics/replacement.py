...
class DeepStackMultimodalReplacement:
    ...
    def __init__(
        ...
        if auxiliary_count != 1:
            raise RuntimeError(
                "The one-layer baseline requires exactly one student "
                f"DeepStack auxiliary tap, got {auxiliary_count}"
            )
        # Preserve Qwen's first auxiliary path only. Its dedicated frozen
        # deepstack_merger_list[0] remains paired with the first native source
        # index; the other two native auxiliary paths are disabled for student
        # forward, not filled with repeated copies of the same optical hidden.
        ...
        # Student Qwen receives exactly one auxiliary tensor after language
        # slot 0. Place the sole optical language stage at slot 1 so both the
        # main image embedding and that auxiliary feature pass through optics.
        ...
        for stage in range(len(language.core.expert_layers)):
            layer_index = self.language_optical_layer_indexes[stage]
    ...
    def use_teacher(self) -> None:
        # The full electronic teacher keeps all three native auxiliary visual
        # features so its answer hidden remains the unmodified Qwen target.
    ...
    def use_student(self) -> None:
        # A list of length one makes Qwen inject only deepstack_merger_list[0]
        # after language slot 0. No repeated placeholder tensors are produced.
    ...
    def alignment_specification(self) -> dict[str, Any]:
        if not self.native_pre_attention_enabled:
            init = "disabled"
            equation = "Y = X + OpticalMoE(X)"
        else:
            init = (
                "teacher_copy"
