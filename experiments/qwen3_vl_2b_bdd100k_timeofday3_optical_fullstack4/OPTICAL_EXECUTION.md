...
每个 phase mask 为 `[256,256]`，有 65,536 个独立可训练相位参数。
...
```text
field_0 [B,256,256]
  -> OpticalConversion 1 -> intensity_1 [B,256,256]
  -> OpticalConversion 2 -> intensity_2 [B,256,256]
  -> OpticalConversion 3 -> intensity_3 [B,256,256]
  -> OpticalConversion 4 -> intensity_4 [B,256,256]
```
...
5. Bilinear interpolation 得到独立 `[256,256]` 光场。
...
|---|---:|
| field size | 256×256 |
| padded propagation size | 400×400 |
| wavelength | 532 nm |
| pixel pitch | 17 µm |
| propagation distance | 5 cm |
| phase initialization | 全 0 |
...
- 非线性：ReLU；
- 归一化：每个样本按整个 256×256 光场的空间均值归一化。
...
```text
phase_mask              [256,256], FP32, trainable
amplitude_mask_logits   [256,256], FP32, trainable
```
...
倏逝波区域在当前实现中置 0。传播后中心 crop 回 `[256,256]`。
...
每个 OpticalConversion 都包含一个 400×400 complex FFT2 和一个 IFFT2。
