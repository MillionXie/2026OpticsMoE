...
def pooled_teacher_features(
    store: TeacherCacheStore,
    targets_override: torch.Tensor | list[float] | None = None,
    ...
    cached_targets = torch.cat(targets)
    if targets_override is None:
        return torch.stack(features), cached_targets
    replacement_targets = torch.as_tensor(targets_override, dtype=torch.float32).reshape(-1)
    if len(replacement_targets) != len(cached_targets):
        raise RuntimeError(
            f"Target override has {len(replacement_targets)} samples but visual cache has {len(cached_targets)}"
        )
    return torch.stack(features), replacement_targets
