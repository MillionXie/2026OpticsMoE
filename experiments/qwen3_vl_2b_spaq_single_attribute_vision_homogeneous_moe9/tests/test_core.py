...
from experiments.qwen3_vl_2b_spaq_single_attribute_vision_homogeneous_moe9.datasets import load_spaq
from experiments.qwen3_vl_2b_spaq_single_attribute_vision_homogeneous_moe9.metrics import regression_metrics
from experiments.qwen3_vl_2b_spaq_single_attribute_vision_homogeneous_moe9.modeling import (NormalizedLinearRegressionHead,
                                                                              build_head,
                                                                              resolve_cached_model_source)
from experiments.qwen3_vl_2b_spaq_single_attribute_vision_homogeneous_moe9.optics.geometry import MoEGeometry
from experiments.qwen3_vl_2b_spaq_single_attribute_vision_homogeneous_moe9.optics.moe import VisionHomogeneousMoESurrogate
from experiments.qwen3_vl_2b_spaq_single_attribute_vision_homogeneous_moe9.processor_cache import (ProcessorCacheStore,
                                                                                     build_processor_cache,
                                                                                     validate_processor_cache)
from experiments.qwen3_vl_2b_spaq_single_attribute_vision_homogeneous_moe9.sampling import EpochRotatingSampler
from experiments.qwen3_vl_2b_spaq_single_attribute_vision_homogeneous_moe9.settings import load_settings
from experiments.qwen3_vl_2b_spaq_single_attribute_vision_homogeneous_moe9.teacher_cache import (expected_metadata,
                                                                                  load_teacher_predictions,
                                                                                  pooled_teacher_features,
                                                                                  write_teacher_predictions)


CONFIG = "experiments/qwen3_vl_2b_spaq_single_attribute_vision_homogeneous_moe9/configs/spaq_brightness_smoke.json"
MAIN_CONFIG = "experiments/qwen3_vl_2b_spaq_single_attribute_vision_homogeneous_moe9/configs/spaq_brightness.json"
...
def test_grouped_config_and_identical_small_regression_head() -> None:
    ...
    assert settings.dataset == "spaq_single_attribute" and settings.task_name == "Brightness"
...
def test_spaq_attribute_loader_uses_selected_label_rgb_and_persistent_split(tmp_path: Path) -> None:
    root = tmp_path / "SPAQ"; images = root / "images"; images.mkdir(parents=True)
    rows = ["Image name,Brightness"]
    ...
    config.write_text(json.dumps({"dataset": "spaq_single_attribute", "task_name": "Brightness", "data_root": str(root),
...
def test_teacher_cache_schema_contains_regression_targets_only() -> None:
    ...
    assert metadata["dataset"] == "spaq_single_attribute" and metadata["task"] == "Brightness"
    assert "targets" in metadata["cached_tensors"] and "labels" not in metadata["cached_tensors"]
    assert metadata["target_scale"] == [0.0, 1.0]


def test_task_target_override_ignores_legacy_mos_cache_targets() -> None:
    class FakeStore:
        def iter_shards(self):
            yield {
                "teacher_vision_stack_output": [torch.ones(2, 4), torch.full((3, 4), 2.0)],
                "targets": torch.tensor([0.91, 0.87]),
            }

    features, targets = pooled_teacher_features(FakeStore(), [0.12, 0.34])
    assert features.shape == (2, 4)
    assert torch.allclose(targets, torch.tensor([0.12, 0.34]))
...
def test_processor_cache_persists_qwen_arrays_and_validates_metadata(tmp_path: Path) -> None:
    ...
    settings = SimpleNamespace(
        ...
        processor_min_pixels=25600, processor_max_pixels=25600, task_name="Brightness",
    )
...
def test_each_attribute_has_an_independent_full_config(filename: str, task_name: str) -> None:
    root = Path("experiments/qwen3_vl_2b_spaq_single_attribute_vision_homogeneous_moe9/configs")
    settings = load_settings(root / filename)
    assert settings.task_name == task_name
    assert task_name.lower() in settings.experiment_name
    assert task_name.lower() in settings.output_dir.name
    assert settings.source_cache_run_dir is not None
