...
def expected_metadata(split: str, samples: int, settings: Any, model: nn.Module | None,
    ...
    return {
        ...
        "cached_tensors": ["labels", "image_grid_thw", "visual_token_counts", "teacher_vision_stack_output"],
...
def pooled_teacher_features(store: TeacherCacheStore) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    for shard in store.iter_shards():
        ...
        labels.append(shard["labels"].long())
    return torch.stack(features), torch.cat(labels)


def write_teacher_logits(output_dir: Path, split: str, logits: torch.Tensor, labels: torch.Tensor) -> None:
    torch.save({"sample_indices": torch.arange(len(labels)), "labels": labels.long(), "teacher_logits": logits.half()},
               output_dir / "teacher_cache" / f"{split}_teacher_logits.pt")


def load_teacher_logits(path: Path) -> torch.Tensor:
    if not path.is_file():
        raise FileNotFoundError(f"Teacher logits missing: {path}. Run --phase teacher_logits first.")
