...
def train_teacher_head(train_store: TeacherCacheStore, test_store: TeacherCacheStore, settings: Any,
    ...
    references=load_cached_tensor(train_store,"reference_ids").long()
    train_idx,val_idx=_split_tensor_labels(labels,settings.validation_fraction,settings.seed,references)
...
def _write_student_epoch_outputs(settings: Any, history: list[dict], row: dict, validation: dict, epoch: int, names: Sequence[str]) -> None:
    ...
    if epoch%settings.save_predictions_interval_epochs==0:
        rows=_prediction_rows(validation["indices"],validation["labels"],validation["logits"],names,validation.get("sample_metadata"))
...
def save_student_inference(result: dict[str,Any], settings: Any, names: Sequence[str], replacement: Any | None=None) -> list[dict]:
    ...
    rows=_prediction_rows(result["indices"],result["labels"],result["logits"],names,result.get("sample_metadata"))
...
def _prediction_rows(indices: Sequence[int], labels: torch.Tensor, logits: torch.Tensor, names: Sequence[str], sample_metadata: Sequence[dict[str,Any]]|None=None) -> list[dict]:
...
def load_head(path:Path,settings:Any,device:torch.device)->ClassificationHead:
    ...
    saved_type=str(p.get("head_type","mlp")); saved_hidden=p.get("head_hidden_dim",p.get("hidden_dim"))
    expected=head.specification(); mismatch=saved_type!=expected["type"]
    ...
    if "head_type" in p and saved_type=="normalized_linear": mismatch=mismatch or not expected["use_layernorm"]
    if mismatch:
        raise RuntimeError(
...
def _split_tensor_labels(labels:torch.Tensor,fraction:float,seed:int,references:torch.Tensor|None=None):
    attributes={"labels":labels.tolist(),"__len__":lambda self:len(self.labels)}
    if references is not None:attributes["references"]=[int(value) for value in references.tolist()]
    dataset=type("Labels",(),attributes)()
