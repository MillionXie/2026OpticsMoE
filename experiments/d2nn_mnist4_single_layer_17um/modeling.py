...
class SingleLayerMNIST4D2NN(nn.Module):
    """Co-planar amplitude/phase input followed by one 5 cm propagation."""
    ...
    def forward(self, images: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        canvas_field = F.pad(modulated, (guard, guard, guard, guard))
        detector_field = self.propagator(canvas_field)
        detector_intensity_canvas = detector_field.abs().square().float()
