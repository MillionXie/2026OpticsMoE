...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Train/export the 17 um single-layer MNIST-4 D2NN"
    )
    ...
    write_json(
        ...
        {
            "architecture": "co-planar amplitude + one phase mask + one ASM propagation + four detector regions",
    ...
    if args.phase in {"export", "all"}:
        ...
        print(
            f"hardware BMPs exported to {export_dir}; samples={contract['sample_count']}",
            flush=True,
        )
    return 0
