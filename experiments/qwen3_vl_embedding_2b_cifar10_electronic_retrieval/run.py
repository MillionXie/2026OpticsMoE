...
from .modeling import build_electronic_student, load_backbone
from .prepare_cifar10_retrieval import prepare_cifar10_subset
from .settings import load_settings, save_resolved_config


PHASES = {"prepare_data", "train", "evaluate", "visualize", "all"}


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Attention-free Qwen3-VL CIFAR-10 electronic retrieval"
    )
    ...
    bundle = prepare_cifar10_subset(settings, persist=True)
    if args.phase == "prepare_data":
        counts = bundle.metadata["counts"]
        print(
            f"Prepared CIFAR-10 retrieval: train={counts['train']} "
    ...
    try:
        ...
        if args.phase in {"evaluate", "all"}:
            ...
            evaluate_all_systems(
                loaded, replacement, readout, bundle, None, settings, checkpoint
            )
