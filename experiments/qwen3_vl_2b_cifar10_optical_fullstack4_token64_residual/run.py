...
def build_parser() -> argparse.ArgumentParser:
    parser=argparse.ArgumentParser(description="Qwen3-VL-2B CIFAR-10 token64 optical4 residual distillation")
...
def main(argv:list[str]|None=None)->int:
    ...
    data=load_cifar10(settings); write_json(settings.output_dir/"dataset.json",data.metadata)
    if args.phase=="prepare_data": print(f"CIFAR-10 ready: train={len(data.train)} test={len(data.test)}"); return 0
...
def _write_model_report(model:torch.nn.Module,replacement:FullStackReplacement,settings:Settings,num_classes:int)->None:
    ...
    vision_parameters=sum(p.numel() for p in vision.parameters() if p.requires_grad)
    language_parameters=sum(p.numel() for p in language.parameters() if p.requires_grad)
    phase_parameters=sum(c.phase_mask.numel() for stack in (vision,language) for c in stack.conversions)
    amplitude_parameters=sum(c.amplitude_mask_logits.numel() for stack in (vision,language) for c in stack.conversions if c.amplitude_mask_logits is not None)
    ...
    residual_parameters=sum(p.numel() for stack in (vision,language) for name,p in stack.named_parameters() if name in {"identity_scale","modulated_scale"} and p.requires_grad)
...
def _compare(settings:Settings,names:list[str])->None:
    ...
    comparison={"dataset":"cifar10","classes":names,"replacement":"both_full_stacks_optical4_token64_residual","teacher":teacher,"student":student,"accuracy_drop":{"top1":teacher["top1_accuracy"]-student["top1_accuracy"],"top5":teacher["top5_accuracy"]-student["top5_accuracy"]}}
