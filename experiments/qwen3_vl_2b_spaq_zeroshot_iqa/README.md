...
Generation uses left padding for the decoder-only model and a configurable system instruction that requests an exact numeric-only response. This is prompt formatting rather than dataset-specific training; responses that still ignore the required format remain visible in the raw JSONL cache.
