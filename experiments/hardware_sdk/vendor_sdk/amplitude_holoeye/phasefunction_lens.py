...
phaseData = np.float32(phaseModulation) * np.array((np.dot(np.ones([dataHeight, 1], np.float32), x2) + np.dot(y2, np.ones([1, dataWidth], np.float32))), dtype=np.float32) / np.float32(innerRadius * innerRadius)
