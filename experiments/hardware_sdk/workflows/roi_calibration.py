...
except ImportError:  # direct execution inside workflows/
    import sys
...
def _paths(config: dict[str, Any], config_path: Path) -> tuple[Path, Path]:
    ...
    masks = resolve_path(
        paths.get("masks_dir", "../artifacts/calibration/masks"), base
    )
    results = resolve_path(
        paths.get("results_dir", "../artifacts/calibration/results"), base
    )
...
def roi_boundary_source_points(config: dict[str, Any]) -> list[tuple[float, float]]:
    """Return ROI center plus the four exact ROI-corner coordinates.

    Unlike the coarse/fine detection patterns, these verification coordinates
    deliberately have no inward margin.  The corner-to-corner separation is
    therefore exactly the configured ROI width/height (956 pixels by default).
    """
    left, top, width, height = _amplitude_roi(config)
    return [
        (left + width / 2, top + height / 2),
...
def rectangle_marker(
    ...
    """Return one filled square centered on the requested SLM coordinate."""
    width, height = (int(size_wh[0]), int(size_wh[1]))
    if marker_size <= 0 or not 0 <= value <= 255:
        raise ValueError("marker_size must be positive and value must be in [0,255]")
    half = marker_size / 2
    x0 = max(0, int(round(center_xy[0] - half)))
    y0 = max(0, int(round(center_xy[1] - half)))
    x1 = min(width, x0 + marker_size)
    y1 = min(height, y0 + marker_size)
    output = np.zeros((height, width), dtype=np.uint8)
    output[y0:y1, x0:x1] = np.uint8(value)
    return output
...
def generate_calibration_files(config: dict[str, Any], config_path: Path) -> dict[str, Any]:
    ...
    # Human-readable ROI verification patterns use the exact ROI boundary,
    # not the inward margin used by automatic coarse marker detection.
    boundary_points = roi_boundary_source_points(config)
    combined_points = np.zeros((amp_size[1], amp_size[0]), dtype=np.uint8)
    combined_rectangles = np.zeros_like(combined_points)
    for point in boundary_points:
        combined_points = np.maximum(
            combined_points, gaussian_marker(amp_size, point, marker_size, sigma)
        )
        combined_rectangles = np.maximum(
            combined_rectangles, rectangle_marker(amp_size, point, marker_size)
        )
    Image.fromarray(combined_points, mode="L").save(
        amplitude_dir / "verify_roi_5points.bmp"
    )
    # Compatibility filename now contains the corrected exact-ROI pattern.
    Image.fromarray(combined_points, mode="L").save(
        amplitude_dir / "verify_5points.bmp"
    )
    Image.fromarray(combined_rectangles, mode="L").save(
        amplitude_dir / "verify_roi_5rectangles.bmp"
    )
    roi_outline = np.zeros_like(combined_points)
    ...
    line_width = max(2, int(config["calibration"].get("roi_outline_width_px", 4)))
    roi_outline[y0 : y0 + line_width, x0:x1] = 255
    roi_outline[y1 - line_width : y1, x0:x1] = 255
    roi_outline[y0:y1, x0 : x0 + line_width] = 255
    roi_outline[y0:y1, x1 - line_width : x1] = 255
    Image.fromarray(roi_outline, mode="L").save(
        amplitude_dir / "verify_roi_outline.bmp"
    )
    for pattern_id, filename in (
        ("verify_roi_5points", "verify_roi_5points.bmp"),
        ("verify_roi_5rectangles", "verify_roi_5rectangles.bmp"),
        ("verify_roi_outline", "verify_roi_outline.bmp"),
    ):
        rows.append(
            dict(
                pattern_id=pattern_id,
                calibration_type="verification",
                source_x=float(config["amplitude_roi"]["center_x"]),
                source_y=float(config["amplitude_roi"]["center_y"]),
                gray_value=255,
                amplitude_filename=f"amplitude/{filename}",
                phase_filename=phase_name,
            )
        )
    ...
    report = {
        ...
        "roi_boundary_points": [[float(x), float(y)] for x, y in boundary_points],
