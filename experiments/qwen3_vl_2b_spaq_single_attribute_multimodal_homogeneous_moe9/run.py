...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="SPAQ Qwen3-VL-2B DeepStack-aware homogeneous optical-MoE multimodal distillation")
...
def _model_report(model: torch.nn.Module, replacement: Any, settings: Settings):
    ...
    trainable = vision["trainable_parameters"] + head_params + (language["trainable_parameters"] if settings.student_language_mode == "optical_moe" else 0)
