...
from . import QUALITY_SCORE_MAX, QUALITY_SCORE_MIN


def denormalize_scores(values: torch.Tensor | float) -> torch.Tensor | float:
    scale = QUALITY_SCORE_MAX - QUALITY_SCORE_MIN
    if isinstance(values, torch.Tensor):
        return values * scale + QUALITY_SCORE_MIN
    return float(values) * scale + QUALITY_SCORE_MIN
...
def score_metrics(predictions: torch.Tensor, targets: torch.Tensor) -> dict[str, float]:
    ...
    original_targets = denormalize_scores(targets)
    original_predictions = denormalize_scores(predictions)
    assert isinstance(original_targets, torch.Tensor)
    assert isinstance(original_predictions, torch.Tensor)
    report = regression_metrics(original_targets.tolist(), original_predictions.tolist())
    report.update({"prediction_min_normalized": float(predictions.min()),
                   ...
                   "prediction_min_original": float(original_predictions.min()),
                   "prediction_max_original": float(original_predictions.max()),
                   "target_scale_min": QUALITY_SCORE_MIN,
                   "target_scale_max": QUALITY_SCORE_MAX})
    return report
...
def _reference_disjoint_head_split(
    dataset: Dataset[Any],
    fraction: float,
    seed: int,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Split the teacher-head subset by reference content when metadata permits.

    KADID contains 125 distorted versions of each reference. A sample-random
    split would let the linear teacher head see the same reference content on
    both sides and inflate its internal validation result.
    """

    records = getattr(dataset, "records", None)
    if not records:
        return _split_indices(len(dataset), fraction, seed)
    references = sorted({str(record.reference_id) for record in records})
    if len(references) < 2:
        return _split_indices(len(dataset), fraction, seed)
    ...
    validation_count = min(
        max(round(len(references) * fraction), 1),
        len(references) - 1,
    )
    validation_references = {
        references[position] for position in order[:validation_count]
    }
    train_indices = [
        index
        for index, record in enumerate(records)
        if str(record.reference_id) not in validation_references
    ]
    validation_indices = [
        index
        for index, record in enumerate(records)
        if str(record.reference_id) in validation_references
    ]
    if not train_indices or not validation_indices:
        raise RuntimeError("Reference-disjoint teacher-head split produced an empty subset")
    return (
...
def teacher_inference(head: nn.Module, store: TeacherCacheStore, dataset: Dataset[Any], settings: Any,
    ...
    report = {**score_metrics(predictions, targets), "dataset": "KADID-10k", "task": settings.task_name,
    ...
    if predictions_path:
        ...
        for index, target, prediction in zip(indices.tolist(), targets.tolist(), predictions.tolist()):
            ...
            rows.append({
                **metadata,
                "sample_index": index,
                "true_score": denormalize_scores(target),
                "predicted_score": denormalize_scores(prediction),
                "absolute_error": abs(prediction - target)
                * (QUALITY_SCORE_MAX - QUALITY_SCORE_MIN),
            })
...
def save_student_parts(output_dir: Path, replacement: Any, head: nn.Module, tag: str, epoch: int, metrics: dict[str, Any]):
    ...
    torch.save({
        "state_dict": replacement.vision_surrogate.state_dict(),
        "attention_prelude": replacement.prelude_state_dict(),
        "transformer_alignment": replacement.alignment_specification(),
        **metadata,
    }, root / f"vision_moe_{tag}.pt")
    if replacement.language_mode == "optical_moe":
...
def save_student_inference(report: dict[str, Any], settings: Any, replacement: Any, predictions_path: Path | None):
    ...
    if settings.visualization_enabled and settings.save_scatter_plot and predictions_path and predictions_path.is_file():
        ...
        save_scatter([float(row["true_score"]) for row in rows], [float(row["predicted_score"]) for row in rows],
                     settings.output_dir / "figures" / "student_scatter.png", "KADID-10k optical multimodal student", settings.task_name)
