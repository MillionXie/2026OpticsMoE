...
from .cache_paths import cache_identity_digest, processor_cache_root
...
def expected_processor_metadata(split: str, samples: int, settings: Any) -> dict[str, Any]:
    return {
        ...
        "split": split, "sample_count": int(samples), "dataset": "kadid10k_mos",
...
def build_processor_cache(split: str, processor: Any, loader: Any, dataset_size: int, settings: Any) -> Path:
    root = processor_cache_root(settings); manifest_path = root / f"{split}.pt"
