...
def multimodal_forward_features(model: nn.Module, inputs: Mapping[str, torch.Tensor]) -> torch.Tensor:
    fallback_name = "_kadid_electronic_router_optical_last_hidden"
    ...
    # Qwen3-VL's causal-LM wrapper otherwise projects every sequence position
    # to the full vocabulary even though this experiment consumes only the
    # final-norm hook.  Keeping one logit position leaves the multimodal hidden
    # path unchanged while avoiding a large, unused vocabulary projection.
    outputs = model(
        **inputs,
