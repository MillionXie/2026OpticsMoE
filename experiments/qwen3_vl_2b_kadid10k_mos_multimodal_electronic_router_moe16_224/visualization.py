...
def save_training_curves(history: Sequence[dict[str, Any]], path: Path) -> None:
    ...
    axes[0].plot(epochs, [row["loss_vision"] for row in history], label="vision hidden")
    axes[0].plot(epochs, [row["loss_answer"] for row in history], label="answer hidden")
    ...
    axes[1].set_title("MAE (KADID 1-5 scale)"); axes[1].legend()
...
def save_scatter(true_scores: Sequence[float], predicted_scores: Sequence[float], path: Path,
    ...
    axis.plot([1, 5], [1, 5], "k--", linewidth=1)
    axis.set(xlim=(1, 5), ylim=(1, 5), xlabel=f"True {task_name}",
