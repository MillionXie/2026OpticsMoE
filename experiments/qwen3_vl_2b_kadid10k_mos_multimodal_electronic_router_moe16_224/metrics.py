...
def regression_metrics(true_scores: Sequence[float], predicted_scores: Sequence[float]) -> dict[str, float]:
    ...
    return {
        ...
        "within_0_1_accuracy": float(np.mean(absolute <= 0.1)),
        "within_0_25_accuracy": float(np.mean(absolute <= 0.25)),
        "within_0_5_accuracy": float(np.mean(absolute <= 0.5)),
        "within_1_accuracy": float(np.mean(absolute <= 1.0)),
