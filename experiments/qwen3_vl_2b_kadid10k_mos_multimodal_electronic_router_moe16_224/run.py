...
from .datasets import DatasetBundle, load_kadid10k, make_indexed_loader
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "KADID-10k DMOS Qwen3-VL-2B regression distillation with "
            "electronic top-k amplitude routing and homogeneous optical MoE16-224"
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    _dirs(settings.output_dir); preparation = ensure_kadid10k_dataset(settings); write_json(settings.output_dir / "data_preparation.json", preparation)
    data = load_kadid10k(settings, persist_split=True); settings.resolved_annotations_file = data.metadata["annotation_file"]
    ...
    if args.phase in {"download", "prepare_data"}:
        print(
            f"KADID-10k ready: task={settings.task_name} train={len(data.train)} "
            f"test={len(data.test)} references="
            f"{data.metadata['reference_count_train']}/{data.metadata['reference_count_test']}",
            flush=True,
        )
        return 0
...
def _stores(settings: Settings, data: DatasetBundle):
    ...
    for split, dataset in (("train", data.train), ("test", data.test)):
        expected = {"split": split, "sample_count": len(dataset), "dataset": settings.dataset,
                    ...
                    "quality_score_max": settings.quality_score_max,
...
def _model_report(model: torch.nn.Module, replacement: Any, settings: Settings):
    ...
    write_json(settings.output_dir / "model.json", {
        ...
        "task": settings.task_name,
        "original_target_scale": [settings.quality_score_min, settings.quality_score_max],
