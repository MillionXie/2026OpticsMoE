...
所有命令从仓库根目录执行。正式实验只运行固定 Top-2 光路由的 alpha 梯度；不要再启动 LSP、电子 Router、Top-1 或 Top-4。`alpha65` 只在 `alpha50` 达标后用于搜索更高光贡献上限。
...
python -m experiments.qwen3_vl_2b_lgvq_o2_109_highalpha_vqa --config experiments/qwen3_vl_2b_lgvq_o2_109_highalpha_vqa/configs/release/alpha65.yaml --phase preflight
```

只有三份报告全部为 `status=ready` 才进入正式训练。

## 4. 正式训练：每个 alpha 梯度各跑一次

推荐把 alpha20 放在 AutoDL，把 alpha35、alpha50 放在主服务器两张空闲 4090；确认 alpha50 达标后，再用第三张空闲 4090 跑 alpha65。所有实验同 seed，不重复跑无关对照。
...
CUDA_VISIBLE_DEVICES=3 nohup python -u -m experiments.qwen3_vl_2b_lgvq_o2_109_highalpha_vqa \
  --config experiments/qwen3_vl_2b_lgvq_o2_109_highalpha_vqa/configs/release/alpha65.yaml \
  --phase train \
  > experiments/qwen3_vl_2b_lgvq_o2_109_highalpha_vqa/logs/alpha65.log 2>&1 &
```
...
runs/o2_109_alphaXX/best_reference_compliant_checkpoint.pt
...
alpha20、alpha50、alpha65 只需替换配置和目录名。正式移交时同时保留 `test_metrics.json`、`test_predictions.csv`、`fusion_diagnostics.json` 和 `router_diagnostics.json`。
