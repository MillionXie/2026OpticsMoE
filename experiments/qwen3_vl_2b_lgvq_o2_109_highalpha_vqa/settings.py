...
class OpticalGeometry:
    """Verified hardware plane: four 232 lanes inside one 478 active ROI."""
    ...
    def validate(self, *, formal: bool = True) -> None:
        ...
        if formal and asdict(self) != {
            ...
            raise ValueError(
                "Formal O2-109 runs lock canvas/active/lane/expert/pitch to "
                "518/478/232/109/123"
            )
...
class ExperimentSettings:
    ...
    language_router_detector_intervals: tuple[tuple[int, int], tuple[int, int]] = (
        (164, 223),
        (255, 314),
    )
    router_input_shift_pixels: int = 8
    router_phase_shift_pixels: int = 8
    router_ccd_shift_pixels: int = 8
    router_phase_dropout_block_size: int = 4
    ...
    fusion_alpha_min: float = 0.20
    fusion_alpha_max: float = 0.85
    fusion_alpha_initial: float = 0.25
    fusion_rms_epsilon: float = 1.0e-6
    quality_head_width: int = 256
    quality_head_dropout: float = 0.15
    epochs: int = 50
    batch_size: int = 8
    num_workers: int = 2
    ...
    correlation_loss_weight: float = 0.30
    optical_alignment_loss_weight: float = 0.10
    router_balance_weight: float = 0.02
    router_importance_weight: float = 0.002
    test_interval_epochs: int = 5
    prompt: str = ""
    feature_contract: str = "qwen3vl_full_visual_main_merger_196x2048_v1"
    ...
    def architecture_label(self) -> str:
        alpha = (
            f"{self.fusion_alpha_min:.2f}_{self.fusion_alpha_max:.2f}"
            .replace(".", "p")
        )
        return (
            "qwen3vl2b_lgvq_fullvision_hardware518_o2_expert109_"
            f"powerl2_correctedste_alpha{alpha}_dualhead_v1"
        )
    ...
    def validate(self) -> None:
        ...
        if self.router_backend != "optical" or self.top_k != 2:
            raise ValueError("Formal hardware VQA uses optical routing with top_k=2 only")
        if self.router_weight_normalization != "power_l2":
            raise ValueError("Hardware O2 routing requires power_l2")
        ...
        if any(
            ...
            raise ValueError("Vision router detector intervals must lie inside each 232 lane")
        if int(intervals[0][1]) > int(intervals[1][0]):
            raise ValueError("Router detector intervals must not overlap")
        language_intervals = self.language_router_detector_intervals
        if len(language_intervals) != 2 or any(
            len(interval) != 2 for interval in language_intervals
        ):
            raise ValueError("Language router intervals must contain two [start,end] pairs")
        if any(
            not 0 <= int(start) < int(end) <= self.geometry.active_size
            for start, end in language_intervals
        ):
            raise ValueError("Language router intervals must lie inside the 478 ROI")
        if int(language_intervals[0][1]) > int(language_intervals[1][0]):
            raise ValueError("Language router intervals must not overlap")
        ...
        if not 0 < self.language_token_count + self.frame_count <= self.geometry.expert_size:
            raise ValueError("Language cache must fit the 109-row optical input field")
        if not self.synthetic and (
            self.input_width != 2048
            or self.feature_contract
            != "qwen3vl_full_visual_main_merger_196x2048_v1"
        ):
            raise ValueError(
                "Formal run requires full Qwen Vision main-merger [196,2048] cache"
            )
        if any(
        ...
        if not 0.0 <= self.fusion_alpha_min < self.fusion_alpha_initial < self.fusion_alpha_max < 1.0:
            raise ValueError("Balanced alpha must satisfy 0<=min<initial<max<1")
        if not self.synthetic and self.fusion_alpha_min < 0.20:
            raise ValueError(
                "Formal high-alpha runs require every optical fusion alpha >= 0.20"
            )
        if self.fusion_rms_epsilon <= 0:
            raise ValueError("fusion.rms_epsilon must be positive")
        if self.phase_parameterization != "sigmoid":
            raise ValueError("Formal phase parameterization must be 2pi*sigmoid(raw_phase)")
        if self.quality_head_width <= 0 or not 0.0 <= self.quality_head_dropout < 1.0:
            raise ValueError("Quality-head width/dropout are invalid")
        if self.correlation_loss_weight < 0.0 or self.optical_alignment_loss_weight < 0.0:
            raise ValueError("Additional quality losses must be nonnegative")
...
def load_settings(path: str | Path, *, synthetic: bool = False) -> ExperimentSettings:
    ...
    value = ExperimentSettings(
        ...
        initialization_checkpoint=_path(get("initialization", "checkpoint"), config_path),
        qwen_model_path=_path(get("initialization", "qwen_model_path"), config_path),
        target=target,
        router_backend=str(get("router", "backend", "optical")),
        ...
        input_width=int(get("model", "input_width", 2048)),
        ...
        router_detector_intervals=tuple(
            ...
            for interval in get("router", "detector_intervals", [[79, 108], [124, 153]])
        ),
        language_router_detector_intervals=tuple(
            tuple(map(int, interval))
            for interval in get(
                "router", "language_detector_intervals", [[164, 223], [255, 314]]
            )
        ),
        router_input_shift_pixels=int(get("router", "input_shift_pixels", 8)),
        router_phase_shift_pixels=int(get("router", "phase_shift_pixels", 8)),
        router_ccd_shift_pixels=int(get("router", "ccd_shift_pixels", 8)),
        router_phase_dropout_block_size=int(get("router", "phase_dropout_block_size", 4)),
        ...
        fusion_alpha_min=float(get("fusion", "alpha_min", 0.20)),
        fusion_alpha_max=float(get("fusion", "alpha_max", 0.85)),
        fusion_alpha_initial=float(get("fusion", "alpha_initial", 0.25)),
        fusion_rms_epsilon=float(get("fusion", "rms_epsilon", 1.0e-6)),
        quality_head_width=int(get("model", "quality_head_width", 256)),
        quality_head_dropout=float(get("model", "quality_head_dropout", 0.15)),
        ...
        correlation_loss_weight=float(get("loss", "correlation_weight", 0.30)),
        optical_alignment_loss_weight=float(
            get("loss", "optical_alignment_weight", 0.10)
        ),
        ...
        feature_contract=str(
            get("data", "feature_contract", "qwen3vl_full_visual_main_merger_196x2048_v1")
        ),
