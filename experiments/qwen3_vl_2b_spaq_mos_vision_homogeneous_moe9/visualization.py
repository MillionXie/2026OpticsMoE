...
def save_training_curves(history: Sequence[dict[str, Any]], path: Path) -> None:
    ...
    axes[0].plot(epochs, [row["loss_regression"] for row in history], label="MOS")
...
def save_scatter(true_scores: Sequence[float], predicted_scores: Sequence[float], path: Path, title: str) -> None:
    plt = _pyplot()
    figure, axis = plt.subplots(figsize=(6, 6))
    axis.scatter(true_scores, predicted_scores, s=12, alpha=0.45)
    axis.plot([0, 100], [0, 100], "k--", linewidth=1)
    axis.set(xlim=(0, 100), ylim=(0, 100), xlabel="True MOS", ylabel="Predicted MOS", title=title)
...
def save_debug_example(directory: Path, image: Any, sample_index: int, true_score: float,
                       ...
                       teacher_hidden: torch.Tensor, epoch: int) -> None:
    plt = _pyplot()
    ...
    write_json(directory / "metadata.json", {
        "epoch": epoch, "sample_index": sample_index, "task": "MOS",
