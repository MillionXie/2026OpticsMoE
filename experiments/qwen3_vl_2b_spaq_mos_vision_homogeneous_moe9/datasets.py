...
class SPAQMOSDataset(Dataset[tuple[Image.Image, float]]):
    """RGB SPAQ images paired with MOS normalized to [0,1]."""

    def __init__(self, records: Sequence[SPAQImageRecord]) -> None:
    ...
    def sample_metadata(self, index: int) -> dict[str, Any]:
        ...
        return {"image_name": record.image_name, "image_path": str(record.image_path),
                "score": float(record.scores[TASK_NAME]), "normalized_score": self.targets[index]}
...
def load_spaq(settings: Settings, persist_split: bool = True) -> DatasetBundle:
    ...
    task_columns = {TASK_NAME: _required_column(columns, TASK_COLUMN_ALIASES[TASK_NAME], "MOS score")}
    ...
    return DatasetBundle(
        train=SPAQMOSDataset(train_records),
        test=SPAQMOSDataset(test_records),
...
def targets_of(dataset: Dataset[Any]) -> list[float]:
    ...
    raise TypeError("Dataset does not expose normalized MOS targets")
