...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Qwen3-VL-2B SPAQ MOS electronic-teacher to homogeneous optical-MoE vision distillation"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.phase in {"teacher_train", "teacher_predictions", "teacher_logits", "teacher_inference", "compare"}:
        ...
        if args.phase == "teacher_train":
            train_teacher_head(stores["train"], stores["test"], settings, device); return 0
        ...
        if args.phase in {"teacher_predictions", "teacher_logits"}:
            if args.phase == "teacher_logits":
                print("[compatibility] teacher_logits means cached scalar teacher MOS predictions in this regression task", flush=True)
            generate_teacher_predictions(head, stores, settings, device); return 0
        if args.phase == "teacher_inference":
            teacher_inference(head, stores["test"], settings, device); return 0
...
def _load_stores(settings: Settings, data: DatasetBundle) -> dict[str, TeacherCacheStore]:
    ...
    for split, dataset in (("train", data.train), ("test", data.test)):
        ...
        expected = {"split": split, "sample_count": len(dataset), "dataset": "spaq_mos", "task": "MOS",
                    "data_root": str(settings.data_root), "model_id": settings.model_id,
...
def _write_model_report(model: torch.nn.Module, replacement: VisionStackReplacement, settings: Settings) -> None:
    ...
    write_json(settings.output_dir / "model.json", {
        ...
        "teacher": "complete electronic Qwen3-VL-2B vision stack + normalized linear MOS head",
        "student": "frozen Qwen vision stem + homogeneous optical MoE9x5 + identical MOS head",
...
def _compare(settings: Settings) -> None:
    ...
    write_json(settings.output_dir / "metrics" / "comparison.json", {
        "dataset": "SPAQ", "task": "MOS", "teacher": teacher, "student": student,
