...
class Settings:
    ...
    detector_size: int
    loss_eps: float
    template_mse_loss_weight: float
    detector_ce_loss_weight: float
    ...
    @property
    def propagation_guard(self) -> int:
        return (self.propagation_grid_size - self.canvas_size) // 2
    ...
    def validate(self) -> None:
        ...
        if self.propagation_grid_size < self.canvas_size or (
            self.propagation_grid_size - self.canvas_size
        ) % 2:
            raise ValueError(
                "propagation_grid_size must center the physical canvas exactly"
            )
        ...
        if abs(self.detector_distance_m - 0.10) > 1.0e-12:
            raise ValueError("Formal phase-to-CCD propagation is 10 cm")
        ...
        if self.amplitude_invert_before_export:
            raise ValueError(
                "Formal amplitude export requires corrected polarity: "
                "255=transmissive, 0=blocking, invert_before_export=false"
            )
        ...
        if min(
            ...
            self.loss_eps,
            self.template_mse_loss_weight,
            self.gradient_clip_norm,
        ) <= 0:
            raise ValueError("Learning rates, eps and gradient clip must be positive")
        if self.detector_ce_loss_weight < 0.0:
            raise ValueError("detector_ce_loss_weight must be nonnegative")
        ...
        if min(
            ...
            self.demo_samples_per_class,
            self.evaluation_samples_per_class,
        ) <= 0:
...
def load_settings(path: str | Path) -> Settings:
    ...
    settings = Settings(
        ...
        output_dir=_resolve(d("output_dir", "../../runs/mnist4_single_layer_17um_10cm"), base),
        ...
        propagation_grid_size=int(d("optics.propagation_grid_size", 1024)),
        active_size=int(d("optics.active_size", 478)),
        input_size=int(d("optics.input_size", 400)),
        detector_distance_m=float(d("optics.detector_distance_m", 0.10)),
        ...
        template_mse_loss_weight=float(d("loss.template_mse_weight", 1.0)),
        detector_ce_loss_weight=float(d("loss.detector_ce_weight", 0.25)),
        ...
        amplitude_invert_before_export=bool(d("hardware.amplitude_slm.invert_before_export", False)),
        ...
        demo_samples_per_class=int(d("hardware.demo_samples_per_class", 10)),
        evaluation_samples_per_class=int(
            d("hardware.evaluation_samples_per_class", 100)
        ),
        hardware_export_subdir=str(
            d("hardware.export_subdir", "hardware_export_10cm_normal_polarity")
        ),
    )
    settings.validate()
    return settings
