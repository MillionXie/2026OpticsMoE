...
def train_student(model: nn.Module, replacement: Any, head: nn.Module, train_dataset: Dataset[Any],
    ...
    sam_enabled = bool(settings.sam_enabled and settings.sam_apply_to_flickr_finetune)
    sam = SAMController(optimizer, [*main, *head.parameters(), *routers], settings.sam_rho,
                        settings.sam_adaptive) if sam_enabled else None
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=settings.epochs) if settings.scheduler_type == "cosine" else None
    history: list[dict[str, Any]] = []; best = float("-inf")
    for epoch in range(1, settings.epochs + 1):
        ...
        for batch_index, (cpu_inputs, labels, _indices, teacher_targets, teacher_batch_logits) in enumerate(loader, 1):
            ...
            teacher_taps = [target.to(device=device, dtype=torch.float32, non_blocking=True)
                            for target in teacher_targets["vision_taps"]]
            ...
            def forward_pass():
                ...
                answer, _ = pool_answer_hidden_state(hidden, inputs["attention_mask"]); logits = head(answer)
                student_taps = [*replacement.vision_surrogate.tap_outputs, replacement.vision_surrogate.last_output]
                if len(student_taps) != len(teacher_taps): raise RuntimeError("Student/teacher vision tap count mismatch")
                loss_vision = torch.stack([normalized_packed_group_mse(student, teacher, token_counts)
                                           for student, teacher in zip(student_taps, teacher_taps)]).mean()
                ...
                total = (settings.loss_hidden_weight * loss_vision + settings.loss_answer_weight * loss_answer +
                ...
                values = (total, loss_vision, loss_answer, loss_logit, loss_classification,
                          router["vision_balance"], router["language_balance"],
                          router["vision_importance"], router["language_importance"])
                return total, values, logits

            optimizer.zero_grad(set_to_none=True)
            loss_total, values, logits = forward_pass()
            logged_values = torch.stack([value.detach().double() for value in values])
            logged_logits = logits.detach()
            vr = replacement.vision_surrogate.core.last_routing
            vr_selected = vr["selected_mask"].detach().float(); vr_weights = vr["weights"].detach()
            if settings.student_language_mode == "optical_moe":
                lr = replacement.language_surrogate.core.last_routing
                lr_selected = lr["selected_mask"].detach().float(); lr_weights = lr["weights"].detach()
            loss_total.backward()
            if sam is not None:
                sam.first_step(zero_grad=True); second_loss, _second_values, _second_logits = forward_pass()
                second_loss.backward(); sam.second_step(zero_grad=True)
            else: optimizer.step()
            batch_size = len(labels); seen += batch_size; totals += logged_values * batch_size
            logits_all.append(logged_logits); labels_all.append(labels.detach())
            v_selection += vr_selected.sum(0); v_weights += vr_weights.sum(0)
            if settings.student_language_mode == "optical_moe":
                l_selection += lr_selected.sum(0); l_weights += lr_weights.sum(0)
        ...
        row = {"epoch": epoch, **{f"loss_{key}": value for key, value in mean_losses.items()},
               ...
               **{f"test_{key}": value for key, value in test_report.items() if isinstance(value, (int, float))},
               ...
               "sam_enabled": sam_enabled, "sam_rho": settings.sam_rho if sam_enabled else None,
               "selection_split": "test", "selection_metric": "auroc"}
...
def _validate_alignment_checkpoint(payload: dict[str, Any], replacement: Any, path: Path) -> None:
    ...
    identity_keys = (
        "native_pre_attention_enabled", "native_pre_norm_enabled", "native_pre_attention_trainable", "transformer_residual_enabled",
        "vision_attention_source_layer", "language_attention_source_layer",
        "logical_optical_stages", "physical_layers_per_logical_stage", "total_physical_layers",
    )
    if saved is None:
