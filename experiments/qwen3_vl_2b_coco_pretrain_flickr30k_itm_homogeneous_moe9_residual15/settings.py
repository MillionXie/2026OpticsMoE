...
from . import DATASET_REPO_ID, GENERIC_DATASET_REPO_ID, GENERIC_PROMPT_TEMPLATE, MODEL_ID, PROMPT_TEMPLATE


PROJECT_DIR = Path(__file__).resolve().parent
PATH_FIELDS = {"data_root", "generic_data_root", "output_dir", "cache_dir"}
...
class Settings:
    ...
    experiment_name: str = "qwen3_vl_2b_coco_pretrain_flickr30k_itm_homogeneous_moe9_residual15"
    ...
    output_dir: Path = PROJECT_DIR / "runs" / "qwen3_vl_2b_coco_pretrain_flickr30k_itm_homogeneous_moe9_residual15"
    generic_pretrain_enabled: bool = True
    generic_dataset: str = "coco_multimodal_hidden_distillation"
    generic_dataset_source: str = "huggingface"
    generic_dataset_repo_id: str = GENERIC_DATASET_REPO_ID
    generic_dataset_revision: str | None = None
    generic_data_root: Path = PROJECT_DIR.parent.parent / "data" / "coco"
    generic_annotations_file: str = "annotations/captions_train2017.json"
    generic_images_dir: str = "train2017"
    generic_split_values: tuple[str, ...] = ("train", "restval")
    generic_prompt_template: str = GENERIC_PROMPT_TEMPLATE
    generic_image_limit: int | None = None
    generic_train_samples_per_epoch: int | None = None
    generic_epochs: int = 10
    generic_learning_rate: float = 5e-4
    generic_router_learning_rate: float = 1e-3
    generic_loss_vision_weight: float = 1.0
    generic_loss_answer_weight: float = 1.0
    generic_router_balance_weight: float = 0.03
    generic_router_importance_weight: float = 0.0
    generic_manifest_digest: str | None = None
    generic_dataset_fingerprint: str | None = None
    finetune_from_generic_pretrain: bool = True
    ...
    native_pre_norm_enabled: bool = True
    ...
    logical_optical_stages: int = 5
    physical_layers_per_logical_stage: int = 3
    expert_layers: int = 15
    ...
    sam_apply_to_generic_pretrain: bool = True
    sam_apply_to_flickr_finetune: bool = True
    sam_rho: float = 0.05
    ...
    def validate(self) -> None:
        ...
        if self.generic_dataset_source not in {"huggingface", "local_coco2017"}:
            raise ValueError("generic_pretrain.dataset_source must be huggingface or local_coco2017")
        if self.finetune_from_generic_pretrain and not self.generic_pretrain_enabled:
            raise ValueError("fine_tuning.load_generic_pretrain_checkpoint requires generic_pretrain.enabled")
        if not self.generic_dataset_repo_id or not self.generic_prompt_template.count("{caption}") == 1:
            raise ValueError("Generic dataset repo id is required and generic prompt must contain one {caption}")
        ...
        if self.native_pre_attention_enabled and not self.native_pre_norm_enabled:
            raise ValueError("native attention prelude requires its native pre/post-attention norms")
        ...
        if self.logical_optical_stages != 5:
            raise ValueError("Qwen DeepStack replacement requires exactly five logical optical stages")
        if self.physical_layers_per_logical_stage < 1:
            raise ValueError("physical_layers_per_logical_stage must be positive")
        if self.expert_layers != self.logical_optical_stages * self.physical_layers_per_logical_stage:
            raise ValueError("expert_layers must equal logical_optical_stages * physical_layers_per_logical_stage")
        ...
        if any(stage < 1 or stage > self.logical_optical_stages for stage in self.vision_tap_stages):
            raise ValueError("vision tap stage is outside the five logical optical stages")
        ...
        if self.sam_enabled and self.phase_dropout_enabled:
            raise ValueError("SAM requires phase dropout disabled so both SAM passes use the same optical model")
        ...
        positive = ("feature_batch_size", "student_batch_size", "inference_batch_size", "head_batch_size",
                    ...
                    "epochs", "generic_epochs", "log_interval_batches", "checkpoint_interval_epochs", "visualization_interval_epochs")
        ...
        for name in ("train_image_limit", "test_image_limit", "train_samples_per_epoch",
                     "train_samples_per_class_per_epoch", "generic_image_limit", "generic_train_samples_per_epoch"):
        ...
        for name in ("loss_hidden_weight", "loss_answer_weight", "loss_logit_weight", "loss_classification_weight",
                     "router_balance_weight", "router_importance_weight", "generic_loss_vision_weight",
                     "generic_loss_answer_weight", "generic_router_balance_weight", "generic_router_importance_weight"):
            if float(getattr(self, name)) < 0:
                raise ValueError(f"{name} must be non-negative")
        if (self.smooth_l1_beta <= 0 or self.learning_rate <= 0 or self.generic_learning_rate <= 0 or
                self.generic_router_learning_rate <= 0 or self.student_head_learning_rate <= 0):
...
NESTED_FIELDS: dict[tuple[str, ...], str] = {
    ...
    ("generic_pretrain", "enabled"): "generic_pretrain_enabled",
    ("generic_pretrain", "dataset", "name"): "generic_dataset",
    ("generic_pretrain", "dataset", "source"): "generic_dataset_source",
    ("generic_pretrain", "dataset", "repo_id"): "generic_dataset_repo_id",
    ("generic_pretrain", "dataset", "revision"): "generic_dataset_revision",
    ("generic_pretrain", "dataset", "data_root"): "generic_data_root",
    ("generic_pretrain", "dataset", "annotations_file"): "generic_annotations_file",
    ("generic_pretrain", "dataset", "images_dir"): "generic_images_dir",
    ("generic_pretrain", "dataset", "split_values"): "generic_split_values",
    ("generic_pretrain", "dataset", "image_limit"): "generic_image_limit",
    ("generic_pretrain", "dataset", "samples_per_epoch"): "generic_train_samples_per_epoch",
    ("generic_pretrain", "prompt_template"): "generic_prompt_template",
    ("generic_pretrain", "epochs"): "generic_epochs",
    ("generic_pretrain", "learning_rate"): "generic_learning_rate",
    ("generic_pretrain", "router_learning_rate"): "generic_router_learning_rate",
    ("generic_pretrain", "loss", "vision_hidden_weight"): "generic_loss_vision_weight",
    ("generic_pretrain", "loss", "answer_hidden_weight"): "generic_loss_answer_weight",
    ("generic_pretrain", "loss", "router_balance_weight"): "generic_router_balance_weight",
    ("generic_pretrain", "loss", "router_importance_weight"): "generic_router_importance_weight",
    ("generic_pretrain", "resolved_manifest_digest"): "generic_manifest_digest",
    ("generic_pretrain", "resolved_dataset_fingerprint"): "generic_dataset_fingerprint",
    ("fine_tuning", "load_generic_pretrain_checkpoint"): "finetune_from_generic_pretrain",
    ...
    ("student", "transformer_block_alignment", "native_pre_norm_enabled"): "native_pre_norm_enabled",
    ...
    ("moe", "depth", "logical_stages"): "logical_optical_stages",
    ("moe", "depth", "physical_layers_per_logical_stage"): "physical_layers_per_logical_stage",
    ("moe", "depth", "total_physical_layers"): "expert_layers",
    ...
    ("optimizer", "sam", "apply_to_generic_pretrain"): "sam_apply_to_generic_pretrain",
    ("optimizer", "sam", "apply_to_flickr_finetune"): "sam_apply_to_flickr_finetune",
    ("optimizer", "sam", "rho"): "sam_rho", ("optimizer", "sam", "adaptive"): "sam_adaptive",
