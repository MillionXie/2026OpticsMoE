...
class HomogeneousMoEOpticalCore(nn.Module):
    ...
    def __init__(self, hidden_size: int, max_tokens: int, settings: Any) -> None:
        ...
        self.logical_stages = int(settings.logical_optical_stages)
        self.physical_layers_per_logical_stage = int(settings.physical_layers_per_logical_stage)
        self.total_physical_layers = int(settings.expert_layers)
    ...
    def run_logical_stage(self, logical_stage: int, field: torch.Tensor,
                          routing: dict[str, torch.Tensor]) -> torch.Tensor:
        """Run one Qwen-facing stage as a group of consecutive physical planes."""
        if not 0 <= logical_stage < self.logical_stages:
            raise IndexError(f"logical optical stage {logical_stage} is outside [0,{self.logical_stages})")
        start = logical_stage * self.physical_layers_per_logical_stage
        stop = start + self.physical_layers_per_logical_stage
        for physical_index in range(start, stop):
            field = self.run_stage(physical_index, field, routing)
        return field
    ...
    def parameter_breakdown(self) -> dict[str, int]:
        ...
        return {"optical_phase_parameters": phase, "router_parameters": router,
                ...
                "logical_optical_stages": self.logical_stages,
                "physical_layers_per_logical_stage": self.physical_layers_per_logical_stage,
                "total_physical_layers": self.total_physical_layers}
...
class VisionDeepStackHomogeneousMoE(nn.Module):
    ...
    def compute(self, hidden_states: torch.Tensor, cu_seqlens: torch.Tensor | None,
        ...
        for logical_index in range(self.core.logical_stages):
            field = self.core.run_logical_stage(logical_index, field, routing)
            stage = logical_index + 1
...
class LanguageDeepStackHomogeneousMoE(nn.Module):
    ...
    def set_deepstack_injection_count(self, count: int) -> None:
        if count < 0 or count >= self.core.logical_stages:
            raise ValueError(f"DeepStack injection count must be in [0,{self.core.logical_stages - 1}], got {count}")
    ...
    def forward_stage(self, stage: int, hidden_states: torch.Tensor,
        ...
        self.field = self.core.run_logical_stage(stage, self.field, self.routing)
        packed = self.core.read_hidden(self.field, self.lengths, hidden_states.dtype,
                                       final=stage == self.core.logical_stages - 1)
        ...
        if stage == self.core.logical_stages - 1: self.last_output = output
        return output
