...
def _validate_source(settings) -> str:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Caltech101 four-layer truncated-biased-Gaussian CCD study"
    )
    ...
    try:
        if args.phase == "train":
            ...
            write_json(
                ...
                {
                    "mode": "strict_checkpoint_resume_weights_only",
                    "path": str(settings.continuation_checkpoint),
                    "sha256": digest,
                    "optimizer_state": "reset",
                    "noise_variant": settings.noise_variant,
                },
            )
            train_optical_retrieval(
                loaded,
