...
@dataclass(frozen=True)
class DatasetBundle:
    train: "SPAQTaskDataset"
    test: "SPAQTaskDataset"
...
class SPAQTaskDataset(Dataset[dict[str, Any]]):
    """Virtual image-task pairs; source image files are never duplicated."""

    def __init__(self, records: Sequence[SPAQImageRecord], prompts: dict[str, str]) -> None:
        ...
        self.samples = [
            SPAQTaskSample(
                ...
                score=float(record.scores[task]),
                normalized_score=float(record.scores[task]) / 100.0,
            )
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        return {
            ...
            "score": sample.score,
            "normalized_score": sample.normalized_score,
            "sample_index": index,
        }
...
def load_spaq(settings: Settings, persist_split: bool = True) -> DatasetBundle:
    ...
    task_columns = {
        task: _required_column(columns, aliases, f"{task} score")
        for task, aliases in TASK_COLUMN_ALIASES.items()
    }
    ...
    prompts = dict(settings.task_prompts or {})
    metadata = {
        "dataset": "SPAQ",
        ...
        "train_task_samples": len(train_records) * len(TASK_NAMES),
        "test_task_samples": len(test_records) * len(TASK_NAMES),
    ...
    return DatasetBundle(
        train=SPAQTaskDataset(train_records, prompts),
        test=SPAQTaskDataset(test_records, prompts),
...
def spaq_collate(batch: Sequence[dict[str, Any]]) -> dict[str, Any]:
    return {
        "images": [item["image"] for item in batch],
        "image_names": [item["image_name"] for item in batch],
        "image_paths": [item["image_path"] for item in batch],
        "tasks": [item["task"] for item in batch],
        "task_indices": torch.tensor([item["task_index"] for item in batch], dtype=torch.long),
        "prompts": [item["prompt"] for item in batch],
        "scores": torch.tensor([item["score"] for item in batch], dtype=torch.float32),
        "normalized_scores": torch.tensor(
            [item["normalized_score"] for item in batch], dtype=torch.float32
        ),
...
def _has_required_columns(columns: Sequence[str]) -> bool:
    try:
        ...
        for task, aliases in TASK_COLUMN_ALIASES.items():
            _required_column(columns, aliases, f"{task} score")
    except RuntimeError:
        return False
    return True
...
def _annotation_error(data_root: Path, inspections: Sequence[tuple[Path, list[str], Any]]) -> str:
    ...
    return (
        ...
        "column plus MOS, Brightness, Colorfulness, and Contrast. No labels were guessed. "
