"""Cache a frozen, attention-free ResNet18 layer-3 convolutional front.

This is an architecture-screening artifact only.  It does not alter the
formal model unless a later explicit configuration injects the cached tensor
inside the electronic route before optical stage one.
"""
...
def build(
    *, frame_cache: Path, output: Path, batch_size: int, device: str
    ...
    # Stem + layer1/2/3 outputs [N,256,14,14]. No pooling head, attention,
    # Transformer, classifier, or layer4 is retained.
    front = nn.Sequential(
        source.conv1,
        source.bn1,
        source.relu,
    ...
    tokens = torch.empty(frames.shape[0], 4, 196, 256, dtype=torch.float16)
    with torch.inference_mode():
        for start in range(0, frames.shape[0], batch_size):
            ...
            value = value.flatten(2).transpose(1, 2).reshape(count, 4, 196, 256)
            tokens[start:stop].copy_(value.cpu().half())
            print(f"[resnet18-layer3] {stop}/{frames.shape[0]}", flush=True)
    payload = {
        "schema_version": 1,
        ...
        "front": "resnet18 stem+layer1+layer2+layer3; convolution only",
