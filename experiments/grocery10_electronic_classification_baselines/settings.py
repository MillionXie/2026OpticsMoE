...
class Settings:
    ...
    def validate(self) -> None:
        ...
        if len(self.selected_skus) != 10 or len(set(self.selected_skus)) != 10:
            raise ValueError("Direct Grocery classification requires 10 unique SKUs")
        if self.gallery_images_per_sku != 1:
            raise ValueError("Grocery Store supplies one iconic gallery image per SKU")
        for name in (
            "image_size",
            "batch_size",
        ...
        for name in ("backbone_learning_rate", "head_learning_rate"):
